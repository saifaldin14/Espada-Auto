...
This document describes how the Espada ACP (Agent Client Protocol) bridge works,
how it maps ACP sessions to Gateway sessions, and how IDEs should invoke it.

## Overview

`espada acp` exposes an ACP agent over stdio and forwards prompts to a running
Espada Gateway over WebSocket. It keeps ACP session ids mapped to Gateway
...
drive a Espada Gateway session.

Quick steps:

1. Run a Gateway (local or remote).
2. Configure the Gateway target (`gateway.remote.url` + auth) or pass flags.
3. Point the IDE to run `espada acp` over stdio.

Example config:

```bash
espada config set gateway.remote.url wss://gateway-host:18789
espada config set gateway.remote.token <token>
```

Example run:

```bash
espada acp --url wss://gateway-host:18789 --token <token>
```
...
```bash
espada acp --session agent:main:main
espada acp --session agent:design:main
espada acp --session agent:qa:bug-123
```
...
In Zed, open the Agent panel and select “Espada ACP” to start a thread.

## Execution Model

- ACP client spawns `espada acp` and speaks ACP messages over stdio.
...
```bash
espada acp --session agent:main:main
espada acp --session-label "support inbox"
espada acp --reset-session
```
...
`espada acp` resolves the Gateway URL and auth from CLI flags or config:
