...
function restoreEnv(snapshot: EnvSnapshot) {
  ...
  restoreKey("ESPADA_STATE_DIR", snapshot.stateDir);
}
...
function setTempHome(base: string) {
  ...
  process.env.ESPADA_STATE_DIR = path.join(base, ".espada");
...
export async function withTempHome<T>(
  ...
  const base = await fs.mkdtemp(path.join(os.tmpdir(), opts.prefix ?? "espada-test-home-"));
  ...
  setTempHome(base);
  await fs.mkdir(path.join(base, ".espada", "agents", "main", "sessions"), { recursive: true });
  if (opts.env) {
