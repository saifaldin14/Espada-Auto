...
const spawnGatewayInstance = async (name: string): Promise<GatewayInstance> => {
  ...
  const homeDir = await fs.mkdtemp(path.join(os.tmpdir(), `espada-e2e-${name}-`));
  const configDir = path.join(homeDir, ".espada");
    ...
    child = spawn(
      ...
      ],
      {
        ...
        env: {
          ...process.env,
          HOME: homeDir,
          ESPADA_CONFIG_PATH: configPath,
          ESPADA_STATE_DIR: stateDir,
          ESPADA_GATEWAY_TOKEN: "",
          ESPADA_GATEWAY_PASSWORD: "",
          ESPADA_SKIP_CHANNELS: "1",
          ESPADA_SKIP_BROWSER_CONTROL_SERVER: "1",
...
const waitForNodeStatus = async (inst: GatewayInstance, nodeId: string, timeoutMs = 10_000) => {
  ...
  while (Date.now() < deadline) {
    const list = (await runCliJson(
      ...
      {
        ESPADA_GATEWAY_TOKEN: inst.gatewayToken,
        ESPADA_GATEWAY_PASSWORD: "",
      },
    )) as NodeListPayload;
    ...
    async () => {
      ...
      const [healthA, healthB] = (await Promise.all([
        runCliJson(["health", "--json", "--timeout", "10000"], {
          ESPADA_GATEWAY_PORT: String(gwA.port),
          ESPADA_GATEWAY_TOKEN: gwA.gatewayToken,
          ESPADA_GATEWAY_PASSWORD: "",
        }),
        runCliJson(["health", "--json", "--timeout", "10000"], {
          ESPADA_GATEWAY_PORT: String(gwB.port),
          ESPADA_GATEWAY_TOKEN: gwB.gatewayToken,
          ESPADA_GATEWAY_PASSWORD: "",
        }),
