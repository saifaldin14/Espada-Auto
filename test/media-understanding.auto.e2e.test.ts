...
import type { EspadaConfig } from "../src/config/config.js";
...
const makeTempMedia = async (ext: string) => {
  const dir = await makeTempDir("espada-media-e2e-");
...
describe("media understanding auto-detect (e2e)", () => {
  ...
  it("uses sherpa-onnx-offline when available", async () => {
    ...
    try {
      const binDir = await makeTempDir("espada-bin-sherpa-");
      const modelDir = await makeTempDir("espada-sherpa-model-");
      ...
      const cfg: EspadaConfig = { tools: { media: { audio: {} } } };
  ...
  it("uses whisper-cli when sherpa is missing", async () => {
    const snapshot = envSnapshot();
    try {
      const binDir = await makeTempDir("espada-bin-whispercpp-");
      const modelDir = await makeTempDir("espada-whispercpp-model-");
      tempPaths.push(binDir, modelDir);
  ...
  it("uses gemini CLI for images when available", async () => {
    const snapshot = envSnapshot();
    try {
      const binDir = await makeTempDir("espada-bin-gemini-");
      ...
      const cfg: EspadaConfig = { tools: { media: { image: {} } } };
