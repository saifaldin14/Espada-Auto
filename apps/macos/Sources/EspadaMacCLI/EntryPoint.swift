...
struct EspadaMacCLI {
    static func main() async {
        ...
        default:
            fputs("espada-mac: unknown command\n", stderr)
            printUsage()
            exit(1)
        }
    }
...
private func printUsage() {
    ...
    Usage:
      ...
      espada-mac discover [--timeout <ms>] [--json] [--include-local]
      espada-mac wizard [--url <ws://host:port>] [--token <token>] [--password <password>]
                          [--mode <local|remote>] [--workspace <path>] [--json]

    Examples:
      espada-mac connect
      espada-mac connect --url ws://127.0.0.1:18789 --json
      espada-mac discover --timeout 3000 --json
      espada-mac wizard --mode local
    """)
}
