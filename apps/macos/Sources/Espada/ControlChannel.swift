...
struct ControlAgentEvent: Codable, Sendable, Identifiable {
    ...
    let stream: String
    let ts: Double
    let data: [String: EspadaProtocol.AnyCodable]
    let summary: String?
}
...
final class ControlChannel {
    ...
    {
        do {
            let rawParams = params?.reduce(into: [String: EspadaKit.AnyCodable]()) {
                $0[$1.key] = EspadaKit.AnyCodable($1.value.base)
            }
    ...
    private func friendlyGatewayMessage(_ error: Error) -> String {
           ...
           urlErr.code == .dataNotAllowed // used for WS close 1008 auth failures
        {
            ...
            let tokenKey = CommandResolver.connectionModeIsRemote()
                ...
                : "gateway.auth.token"
            return
                "Gateway rejected token; set \(tokenKey) (or ESPADA_GATEWAY_TOKEN) " +
    ...
    private static func bridgeToProtocolArgs(
        _ value: EspadaProtocol.AnyCodable?) -> [String: EspadaProtocol.AnyCodable]?
    {
        guard let value else { return nil }
        if let dict = value.value as? [String: EspadaProtocol.AnyCodable] {
            return dict
        }
        if let dict = value.value as? [String: EspadaKit.AnyCodable],
           let data = try? JSONEncoder().encode(dict),
           let decoded = try? JSONDecoder().decode([String: EspadaProtocol.AnyCodable].self, from: data)
        {
            return decoded
        }
...
extension Notification.Name {
    static let controlHeartbeat = Notification.Name("espada.control.heartbeat")
    static let controlAgentEvent = Notification.Name("espada.control.agent")
}
