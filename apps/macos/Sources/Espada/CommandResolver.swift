...
enum CommandResolver {
    private static let projectRootDefaultsKey = "espada.gatewayProjectRootPath"
    private static let helperName = "espada"

    static func gatewayEntrypoint(in root: URL) -> String? {
        let distEntry = root.appendingPathComponent("dist/index.js").path
        if FileManager().isReadableFile(atPath: distEntry) { return distEntry }
        let binEntry = root.appendingPathComponent("bin/espada.js").path
    ...
    static func projectRoot() -> URL {
        ...
        let fallback = FileManager().homeDirectoryForCurrentUser
            .appendingPathComponent("Projects/espada")
    ...
    static func preferredPaths(home: URL, current: [String], projectRoot: URL) -> [String] {
        ...
        #endif
        let espadaPaths = self.espadaManagedPaths(home: home)
        if !espadaPaths.isEmpty {
            extras.insert(contentsOf: espadaPaths, at: 1)
        }
        extras.insert(contentsOf: self.nodeManagerBinPaths(home: home), at: 1 + espadaPaths.count)
        var seen = Set<String>()
        // Preserve order while stripping duplicates so PATH lookups remain deterministic.
        return (extras + current).filter { seen.insert($0).inserted }
    }

    private static func espadaManagedPaths(home: URL) -> [String] {
        let base = home.appendingPathComponent(".espada")
    ...
    static func espadaExecutable(searchPaths: [String]? = nil) -> String? {
        self.findExecutable(named: self.helperName, searchPaths: searchPaths)
    }

    static func projectEspadaExecutable(projectRoot: URL? = nil) -> String? {
        #if DEBUG
    ...
    static func nodeCliPath() -> String? {
        let candidate = self.projectRoot().appendingPathComponent("bin/espada.js").path
        return FileManager().isReadableFile(atPath: candidate) ? candidate : nil
    }

    static func hasAnyEspadaInvoker(searchPaths: [String]? = nil) -> Bool {
        if self.espadaExecutable(searchPaths: searchPaths) != nil { return true }
        ...
        searchPaths: [String]? = nil) -> [String]
    {
        ...
        case let .success(runtime):
            ...
            if let espadaPath = self.projectEspadaExecutable(projectRoot: root) {
                return [espadaPath, subcommand] + extraArgs
            }
            ...
            if let pnpm = self.findExecutable(named: "pnpm", searchPaths: searchPaths) {
                ...
                return [pnpm, "--silent", "espada", subcommand] + extraArgs
            }
            if let espadaPath = self.espadaExecutable(searchPaths: searchPaths) {
                return [espadaPath, subcommand] + extraArgs
            }

            let missingEntry = """
            espada entrypoint missing (looked for dist/index.js or bin/espada.js); run pnpm build.
            """
    ...
    // Existing callers still refer to espadaCommand; keep it as node alias.
    static func espadaCommand(
        subcommand: String,
        extraArgs: [String] = [],
        defaults: UserDefaults = .standard,
        configRoot: [String: Any]? = nil,
    ...
    private static func sshNodeCommand(subcommand: String, extraArgs: [String], settings: RemoteSettings) -> [String]? {
        ...
        // Run the real espada CLI on the remote host.
        let exportedPath = [
            "/opt/homebrew/bin",
            "/usr/local/bin",
            "/usr/bin",
        ...
        let projectSection = if userPRJ.isEmpty {
            """
            DEFAULT_PRJ="$HOME/Projects/espada"
        ...
        \(cliSection)
        \(projectSection)
        if command -v espada >/dev/null 2>&1; then
          CLI="$(command -v espada)"
          espada \(quotedArgs);
        ...
        elif [ -n "${PRJ:-}" ] && [ -f "$PRJ/bin/espada.js" ]; then
          if command -v node >/dev/null 2>&1; then
            CLI="node $PRJ/bin/espada.js"
            node "$PRJ/bin/espada.js" \(quotedArgs);
          else
            echo "Node >=22 required on remote host"; exit 127;
          fi
        elif command -v pnpm >/dev/null 2>&1; then
          CLI="pnpm --silent espada"
          pnpm --silent espada \(quotedArgs);
        else
          echo "espada CLI missing on remote host"; exit 127;
        fi
        """
        let options: [String] = [
            "-o", "BatchMode=yes",
    ...
    {
        let root = configRoot ?? EspadaConfigFile.loadDict()
