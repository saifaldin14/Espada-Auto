...
enum DebugActions {
    private static let verboseDefaultsKey = "espada.debug.verboseMain"
    private static let sessionMenuLimit = 12
    private static let onboardingSeenKey = "espada.onboardingSeen"
    ...
    static func sendTestNotification() async {
        _ = await NotificationManager().send(title: "Espada", body: "Test notification", sound: nil)
    }
    ...
    private static func resolveSessionStorePath() -> String {
        ...
        let configURL = FileManager().homeDirectoryForCurrentUser
            .appendingPathComponent(".espada/espada.json")
        guard
