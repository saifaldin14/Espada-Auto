...
enum CLIInstaller {
        ...
        fileManager: FileManager) -> String?
    {
        for basePath in searchPaths {
            let candidate = URL(fileURLWithPath: basePath).appendingPathComponent("espada").path
    ...
    static func install(statusHandler: @escaping @MainActor @Sendable (String) async -> Void) async {
        ...
        await statusHandler("Installing espada CLI…")
        ...
        if response.success {
            ...
            let summary = installedVersion.map { "Installed espada \($0)." } ?? "Installed espada."
