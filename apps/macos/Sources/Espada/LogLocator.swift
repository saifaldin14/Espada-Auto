...
enum LogLocator {
    private static var logDir: URL {
        if let override = ProcessInfo.processInfo.environment["ESPADA_LOG_DIR"], !override.isEmpty {
            return URL(fileURLWithPath: override)
        }

        return URL(fileURLWithPath: "/tmp/espada")
    }

    private static var stdoutLog: URL {
        logDir.appendingPathComponent("espada-stdout.log")
    }

    private static var gatewayLog: URL {
        logDir.appendingPathComponent("espada-gateway.log")
    }
    ...
    /// Returns the newest log file under /tmp/espada/ (rolling or stdout), or nil if none exist.
    static func bestLogFile() -> URL? {
        ...
        return files
            .filter { $0.lastPathComponent.hasPrefix("espada") && $0.pathExtension == "log" }
            .max { lhs, rhs in
