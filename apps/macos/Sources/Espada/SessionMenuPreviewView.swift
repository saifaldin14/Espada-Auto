...
enum SessionMenuPreviewLoader {
    ...
    private static func requestPreview(
        keys: [String],
        maxItems: Int) async throws -> EspadaSessionsPreviewPayload
    {
    ...
    private static func snapshot(
        from entry: EspadaSessionPreviewEntry,
    ...
    private static func cache(payload: EspadaSessionsPreviewPayload, maxItems: Int) async {
    ...
    private static func previewItems(
        from payload: EspadaChatHistoryPayload,
        maxItems: Int) -> [SessionPreviewItem]
    {
        let boundedItems = self.normalizeMaxItems(maxItems)
        let raw: [EspadaKit.AnyCodable] = payload.messages ?? []
    ...
    private static func decodeMessages(_ raw: [EspadaKit.AnyCodable]) -> [EspadaChatMessage] {
        raw.compactMap { item in
            guard let data = try? JSONEncoder().encode(item) else { return nil }
            return try? JSONDecoder().decode(EspadaChatMessage.self, from: data)
        }
    }
    ...
    private static func previewText(for message: EspadaChatMessage) -> String? {
    ...
    private static func isToolCall(_ message: EspadaChatMessage) -> Bool {
    ...
    private static func toolNames(for message: EspadaChatMessage) -> [String] {
        var names: [String] = []
    ...
    private static func mediaSummary(for message: EspadaChatMessage) -> String? {
