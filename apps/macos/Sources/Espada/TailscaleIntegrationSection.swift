...
struct TailscaleIntegrationSection: View {
    ...
    private var authFields: some View {
        ...
        Text("Stored in ~/.espada/espada.json. Prefer ESPADA_GATEWAY_PASSWORD for production.")
            .font(.caption)
            .foregroundStyle(.secondary)
    ...
    private func applySettings() async {
        ...
        if self.connectionMode == .local, !self.isPaused {
            self.statusMessage = "Saved to ~/.espada/espada.json. Restarting gateway…"
        } else {
            self.statusMessage = "Saved to ~/.espada/espada.json. Restart the gateway to apply."
        }
