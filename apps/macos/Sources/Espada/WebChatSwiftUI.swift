...
struct MacGatewayChatTransport: EspadaChatTransport, Sendable {
    ...
    func listSessions(limit: Int?) async throws -> EspadaChatSessionsListResponse {
        ...
        let data = try await GatewayConnection.shared.request(
            ...
            params: params,
            timeoutMs: 15000)
        return try JSONDecoder().decode(EspadaChatSessionsListResponse.self, from: data)
    }
    ...
    static func mapPushToTransportEvent(_ push: GatewayPush) -> EspadaChatTransportEvent? {
        switch push {
        case let .snapshot(hello):
            let ok = (try? JSONDecoder().decode(
                EspadaGatewayHealthOK.self,
...
final class WebChatSwiftUIWindowController {
    ...
    private let hosting: NSHostingController<EspadaChatView>
    ...
    init(sessionKey: String, presentation: WebChatPresentation, transport: any EspadaChatTransport) {
        self.sessionKey = sessionKey
        self.presentation = presentation
        let vm = EspadaChatViewModel(sessionKey: sessionKey, transport: transport)
        let accent = Self.color(fromHex: AppStateStore.shared.seamColorHex)
        self.hosting = NSHostingController(rootView: EspadaChatView(
            viewModel: vm,
    ...
    private static func makeContentController(
        for presentation: WebChatPresentation,
        hosting: NSHostingController<EspadaChatView>) -> NSViewController
    {
