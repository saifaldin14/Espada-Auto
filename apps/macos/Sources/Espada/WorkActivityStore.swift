...
final class WorkActivityStore {
    ...
    func handleTool(
        ...
        name: String?,
        meta: String?,
        args: [String: EspadaProtocol.AnyCodable]?)
    {
    ...
    private static func buildLabel(
        name: String?,
        meta: String?,
        args: [String: EspadaProtocol.AnyCodable]?) -> String
    {
    ...
    private static func wrapToolArgs(_ args: [String: EspadaProtocol.AnyCodable]?) -> EspadaKit.AnyCodable? {
        guard let args else { return nil }
        let converted: [String: Any] = args.mapValues { self.unwrapJSONValue($0.value) }
        return EspadaKit.AnyCodable(converted)
    }

    private static func unwrapJSONValue(_ value: Any) -> Any {
        if let dict = value as? [String: EspadaProtocol.AnyCodable] {
            return dict.mapValues { self.unwrapJSONValue($0.value) }
        }
        if let array = value as? [EspadaProtocol.AnyCodable] {
