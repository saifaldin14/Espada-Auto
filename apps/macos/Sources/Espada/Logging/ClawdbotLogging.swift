...
enum EspadaLogging {
    ...
    private static let didBootstrap: Void = {
        LoggingSystem.bootstrap { label in
            ...
            let osHandler = EspadaOSLogHandler(subsystem: subsystem, category: category)
            let fileHandler = EspadaFileLogHandler(label: label)
...
extension Logging.Logger {
    init(subsystem: String, category: String) {
        EspadaLogging.bootstrapIfNeeded()
        let label = EspadaLogging.makeLabel(subsystem: subsystem, category: category)
...
struct EspadaOSLogHandler: LogHandler {
...
struct EspadaFileLogHandler: LogHandler {
    let label: String
    var metadata: Logger.Metadata = [:]
        ...
        line: UInt)
    {
        ...
        let (subsystem, category) = EspadaLogging.parseLabel(self.label)
