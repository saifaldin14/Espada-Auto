...
enum GatewayEnvironment {
    ...
    static func gatewayPort() -> Int {
        if let raw = ProcessInfo.processInfo.environment["ESPADA_GATEWAY_PORT"] {
            let trimmed = raw.trimmingCharacters(in: .whitespacesAndNewlines)
            if let parsed = Int(trimmed), parsed > 0 { return parsed }
        }
        if let configPort = EspadaConfigFile.gatewayPort(), configPort > 0 {
            return configPort
        }
    ...
    static func check() -> GatewayEnvironmentStatus {
        ...
        case let .success(runtime):
            let gatewayBin = CommandResolver.espadaExecutable()
            ...
            if gatewayBin == nil, projectEntrypoint == nil {
                return GatewayEnvironmentStatus(
                    ...
                    gatewayVersion: nil,
                    requiredGateway: expectedString,
                    message: "espada CLI not found in PATH; install the CLI.")
            }
    ...
    private static func preferredGatewayBind() -> String? {
        ...
        if let env = ProcessInfo.processInfo.environment["ESPADA_GATEWAY_BIND"] {
    ...
    static func installGlobal(versionString: String?, statusHandler: @escaping @Sendable (String) -> Void) async {
        ...
        let (label, cmd): (String, [String]) =
            if let npm {
                ("npm", [npm, "install", "-g", "espada@\(target)"])
            } else if let pnpm {
                ("pnpm", [pnpm, "add", "-g", "espada@\(target)"])
            } else if let bun {
                ("bun", [bun, "add", "-g", "espada@\(target)"])
            } else {
                ("npm", ["npm", "install", "-g", "espada@\(target)"])
            }

        statusHandler("Installing espada@\(target) via \(label)…")
        ...
        if response.success {
            statusHandler("Installed espada@\(target)")
        } else {
