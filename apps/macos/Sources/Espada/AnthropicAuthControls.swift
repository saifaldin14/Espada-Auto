...
struct AnthropicAuthControls: View {
    ...
    @State private var oauthStatus: EspadaOAuthStore.AnthropicOAuthStatus = EspadaOAuthStore.anthropicOAuthStatus()
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 10) {
            ...
            HStack(spacing: 10) {
                ...
                .disabled(!FileManager().fileExists(atPath: EspadaOAuthStore.oauthURL().path))
    ...
    private func refresh() {
        let imported = EspadaOAuthStore.importLegacyAnthropicOAuthIfNeeded()
        self.oauthStatus = EspadaOAuthStore.anthropicOAuthStatus()
        if imported != nil {
    ...
    private func finishOAuth() async {
        ...
        do {
            ...
            try EspadaOAuthStore.saveAnthropicOAuth(creds)
            self.refresh()
            self.pkce = nil
            self.code = ""
            self.statusText = "Connected. Espada can now use Claude via OAuth."
        } catch {
...
extension AnthropicAuthControls {
    init(
        ...
        oauthStatus: EspadaOAuthStore.AnthropicOAuthStatus,
