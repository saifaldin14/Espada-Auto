...
@Suite struct InstancesStoreTests {
    ...
    func presenceEventPayloadDecodesViaJSONEncoder() {
        ...
        let entry: [String: EspadaProtocol.AnyCodable] = [
            "host": .init("gw"),
            "ip": .init("10.0.0.1"),
            "version": .init("2.0.0"),
            "mode": .init("gateway"),
        ...
        let payloadMap: [String: EspadaProtocol.AnyCodable] = [
            "presence": .init([EspadaProtocol.AnyCodable(entry)]),
        ]
        let payload = EspadaProtocol.AnyCodable(payloadMap)
