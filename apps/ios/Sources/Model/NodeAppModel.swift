...
final class NodeAppModel {
    ...
    private func handleCanvasA2UIAction(body: [String: Any]) async {
        ...
        guard let name = EspadaCanvasA2UIAction.extractActionName(userAction) else { return }
        let actionId: String = {
        ...
        let contextJSON = EspadaCanvasA2UIAction.compactJSON(userAction["context"])
        let sessionKey = self.mainSessionKey

        let messageContext = EspadaCanvasA2UIAction.AgentMessageContext(
            actionName: name,
            session: .init(key: sessionKey, surfaceId: surfaceId),
            component: .init(id: sourceComponentId, host: host, instanceId: instanceId),
            contextJSON: contextJSON)
        let message = EspadaCanvasA2UIAction.formatAgentMessage(messageContext)
        ...
        let js = EspadaCanvasA2UIAction.jsDispatchA2UIActionStatus(actionId: actionId, ok: ok, error: errorText)
        do {
    ...
    private func resolveA2UIHostURL() async -> String? {
        ...
        return base.appendingPathComponent("__espada__/a2ui/").absoluteString + "?platform=ios"
    }
    ...
    func requestLocationPermissions(mode: EspadaLocationMode) async -> Bool {
    ...
    private func sendAgentRequest(link: AgentDeepLink) async throws {
        ...
        // (Key-based unattended auth is handled on macOS for espada:// links.)
    ...
    private func handleInvoke(_ req: BridgeInvokeRequest) async -> BridgeInvokeResponse {
        ...
        do {
            switch command {
            case EspadaLocationCommand.get.rawValue:
                return try await self.handleLocationInvoke(req)
            case EspadaCanvasCommand.present.rawValue,
                 EspadaCanvasCommand.hide.rawValue,
                 EspadaCanvasCommand.navigate.rawValue,
                 EspadaCanvasCommand.evalJS.rawValue,
                 EspadaCanvasCommand.snapshot.rawValue:
                return try await self.handleCanvasInvoke(req)
            case EspadaCanvasA2UICommand.reset.rawValue,
                 EspadaCanvasA2UICommand.push.rawValue,
                 EspadaCanvasA2UICommand.pushJSONL.rawValue:
                return try await self.handleCanvasA2UIInvoke(req)
            case EspadaCameraCommand.list.rawValue,
                 EspadaCameraCommand.snap.rawValue,
                 EspadaCameraCommand.clip.rawValue:
                return try await self.handleCameraInvoke(req)
            case EspadaScreenCommand.record.rawValue:
                return try await self.handleScreenRecordInvoke(req)
            default:
                return BridgeInvokeResponse(
                    id: req.id,
                    ok: false,
                    error: EspadaNodeError(code: .invalidRequest, message: "INVALID_REQUEST: unknown command"))
            }
        } catch {
            ...
            return BridgeInvokeResponse(
                id: req.id,
                ok: false,
                error: EspadaNodeError(code: .unavailable, message: error.localizedDescription))
        }
    }
    ...
    private func handleLocationInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        ...
        let params = (try? Self.decodeParams(EspadaLocationGetParams.self, from: req.paramsJSON)) ??
            EspadaLocationGetParams()
        ...
        let payload = EspadaLocationPayload(
    ...
    private func handleCanvasInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        switch req.command {
        case EspadaCanvasCommand.present.rawValue:
            let params = (try? Self.decodeParams(EspadaCanvasPresentParams.self, from: req.paramsJSON)) ??
                EspadaCanvasPresentParams()
        ...
        case EspadaCanvasCommand.hide.rawValue:
            return BridgeInvokeResponse(id: req.id, ok: true)
        case EspadaCanvasCommand.navigate.rawValue:
            let params = try Self.decodeParams(EspadaCanvasNavigateParams.self, from: req.paramsJSON)
            self.screen.navigate(to: params.url)
            return BridgeInvokeResponse(id: req.id, ok: true)
        case EspadaCanvasCommand.evalJS.rawValue:
            let params = try Self.decodeParams(EspadaCanvasEvalParams.self, from: req.paramsJSON)
            let result = try await self.screen.eval(javaScript: params.javaScript)
            let payload = try Self.encodePayload(["result": result])
            return BridgeInvokeResponse(id: req.id, ok: true, payloadJSON: payload)
        case EspadaCanvasCommand.snapshot.rawValue:
            let params = try? Self.decodeParams(EspadaCanvasSnapshotParams.self, from: req.paramsJSON)
    ...
    private func handleCanvasA2UIInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        ...
        case EspadaCanvasA2UICommand.reset.rawValue:
            ...
            (() => {
              if (!globalThis.espadaA2UI) return JSON.stringify({ ok: false, error: "missing espadaA2UI" });
              return JSON.stringify(globalThis.espadaA2UI.reset());
            })()
            """)
            return BridgeInvokeResponse(id: req.id, ok: true, payloadJSON: json)
        case EspadaCanvasA2UICommand.push.rawValue, EspadaCanvasA2UICommand.pushJSONL.rawValue:
            let messages: [AnyCodable]
            if command == EspadaCanvasA2UICommand.pushJSONL.rawValue {
                let params = try Self.decodeParams(EspadaCanvasA2UIPushJSONLParams.self, from: req.paramsJSON)
                messages = try EspadaCanvasA2UIJSONL.decodeMessagesFromJSONL(params.jsonl)
            } else {
                do {
                    let params = try Self.decodeParams(EspadaCanvasA2UIPushParams.self, from: req.paramsJSON)
            ...
            let messagesJSON = try EspadaCanvasA2UIJSONL.encodeMessagesJSONArray(messages)
            let js = """
            (() => {
              try {
                if (!globalThis.espadaA2UI) return JSON.stringify({ ok: false, error: "missing espadaA2UI" });
                const messages = \(messagesJSON);
                return JSON.stringify(globalThis.espadaA2UI.applyMessages(messages));
              } catch (e) {
    ...
    private func handleCameraInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        ...
        case EspadaCameraCommand.list.rawValue:
            ...
            return BridgeInvokeResponse(id: req.id, ok: true, payloadJSON: payload)
        case EspadaCameraCommand.snap.rawValue:
            self.showCameraHUD(text: "Taking photo…", kind: .photo)
            self.triggerCameraFlash()
            let params = (try? Self.decodeParams(EspadaCameraSnapParams.self, from: req.paramsJSON)) ??
                EspadaCameraSnapParams()
            ...
            return BridgeInvokeResponse(id: req.id, ok: true, payloadJSON: payload)
        case EspadaCameraCommand.clip.rawValue:
            let params = (try? Self.decodeParams(EspadaCameraClipParams.self, from: req.paramsJSON)) ??
                EspadaCameraClipParams()
    ...
    private func handleScreenRecordInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        let params = (try? Self.decodeParams(EspadaScreenRecordParams.self, from: req.paramsJSON)) ??
            EspadaScreenRecordParams()
...
private extension NodeAppModel {
    func locationMode() -> EspadaLocationMode {
        let raw = UserDefaults.standard.string(forKey: "location.enabledMode") ?? "off"
        return EspadaLocationMode(rawValue: raw) ?? .off
    }
