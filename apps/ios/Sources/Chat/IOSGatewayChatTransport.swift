...
struct IOSGatewayChatTransport: EspadaChatTransport, Sendable {
    ...
    func listSessions(limit: Int?) async throws -> EspadaChatSessionsListResponse {
        ...
        return try JSONDecoder().decode(EspadaChatSessionsListResponse.self, from: res)
    }
    ...
    func requestHistory(sessionKey: String) async throws -> EspadaChatHistoryPayload {
        ...
        return try JSONDecoder().decode(EspadaChatHistoryPayload.self, from: res)
    }
        ...
        attachments: [EspadaChatAttachmentPayload]) async throws -> EspadaChatSendResponse
    {
        struct Params: Codable {
            ...
            var message: String
            var thinking: String
            var attachments: [EspadaChatAttachmentPayload]?
            var timeoutMs: Int
        ...
        return try JSONDecoder().decode(EspadaChatSendResponse.self, from: res)
    }

    func requestHealth(timeoutMs: Int) async throws -> Bool {
        let seconds = max(1, Int(ceil(Double(timeoutMs) / 1000.0)))
        let res = try await self.gateway.request(method: "health", paramsJSON: nil, timeoutSeconds: seconds)
        return (try? JSONDecoder().decode(EspadaGatewayHealthOK.self, from: res))?.ok ?? true
    }
                ...
                for await evt in stream {
                    ...
                    case "health":
                        ...
                        let ok = (try? GatewayPayloadDecoding.decode(
                            payload,
                            as: EspadaGatewayHealthOK.self))?.ok ?? true
                        continuation.yield(.health(ok: ok))
                    case "chat":
                        guard let payload = evt.payload else { break }
                        if let chatPayload = try? GatewayPayloadDecoding.decode(
                            payload,
                            as: EspadaChatEventPayload.self)
                        {
                    ...
                    case "agent":
                        ...
                        if let agentPayload = try? GatewayPayloadDecoding.decode(
                            payload,
                            as: EspadaAgentEventPayload.self)
                        {
