...
final class GatewayConnectionController {
    ...
    private func currentCaps() -> [String] {
        var caps = [EspadaCapability.canvas.rawValue, EspadaCapability.screen.rawValue]
        ...
        if cameraEnabled { caps.append(EspadaCapability.camera.rawValue) }

        let voiceWakeEnabled = UserDefaults.standard.bool(forKey: VoiceWakePreferences.enabledKey)
        if voiceWakeEnabled { caps.append(EspadaCapability.voiceWake.rawValue) }

        let locationModeRaw = UserDefaults.standard.string(forKey: "location.enabledMode") ?? "off"
        let locationMode = EspadaLocationMode(rawValue: locationModeRaw) ?? .off
        if locationMode != .off { caps.append(EspadaCapability.location.rawValue) }
    ...
    private func currentCommands() -> [String] {
        var commands: [String] = [
            ...
            EspadaSystemCommand.notify.rawValue,
            EspadaSystemCommand.which.rawValue,
            EspadaSystemCommand.run.rawValue,
            EspadaSystemCommand.execApprovalsGet.rawValue,
            EspadaSystemCommand.execApprovalsSet.rawValue,
        ]

        let caps = Set(self.currentCaps())
        if caps.contains(EspadaCapability.camera.rawValue) {
            commands.append(EspadaCameraCommand.list.rawValue)
            commands.append(EspadaCameraCommand.snap.rawValue)
            commands.append(EspadaCameraCommand.clip.rawValue)
        }
        if caps.contains(EspadaCapability.location.rawValue) {
            commands.append(EspadaLocationCommand.get.rawValue)
        }
