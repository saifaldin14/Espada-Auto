...
private final class TestChatTransport: @unchecked Sendable, EspadaChatTransport {
    private let state = TestChatTransportState()
    private let historyResponses: [EspadaChatHistoryPayload]
    private let sessionsResponses: [EspadaChatSessionsListResponse]

    private let stream: AsyncStream<EspadaChatTransportEvent>
    private let continuation: AsyncStream<EspadaChatTransportEvent>.Continuation

    init(
        historyResponses: [EspadaChatHistoryPayload],
        sessionsResponses: [EspadaChatSessionsListResponse] = [])
    {
        self.historyResponses = historyResponses
        self.sessionsResponses = sessionsResponses
        var cont: AsyncStream<EspadaChatTransportEvent>.Continuation!
    ...
    func requestHistory(sessionKey: String) async throws -> EspadaChatHistoryPayload {
        ...
        return self.historyResponses.last ?? EspadaChatHistoryPayload(
    ...
    func sendMessage(
        ...
        attachments _: [EspadaChatAttachmentPayload]) async throws -> EspadaChatSendResponse
    {
        await self.state.sentRunIdsAppend(idempotencyKey)
        return EspadaChatSendResponse(runId: idempotencyKey, status: "ok")
    }
    ...
    func listSessions(limit _: Int?) async throws -> EspadaChatSessionsListResponse {
        ...
        return self.sessionsResponses.last ?? EspadaChatSessionsListResponse(
            ts: nil,
            path: nil,
            count: 0,
            defaults: nil,
    ...
    func emit(_ evt: EspadaChatTransportEvent) {
...
@Suite struct ChatViewModelTests {
    @Test func streamsAssistantAndClearsOnFinal() async throws {
        ...
        let history1 = EspadaChatHistoryPayload(
            sessionKey: "main",
            sessionId: sessionId,
            messages: [],
            thinkingLevel: "off")
        let history2 = EspadaChatHistoryPayload(
            sessionKey: "main",
            sessionId: sessionId,
            messages: [
                AnyCodable([
        ...
        let vm = await MainActor.run { EspadaChatViewModel(sessionKey: "main", transport: transport) }
    ...
    @Test func clearsStreamingOnExternalFinalEvent() async throws {
        let sessionId = "sess-main"
        let history = EspadaChatHistoryPayload(
            sessionKey: "main",
            sessionId: sessionId,
            messages: [],
            thinkingLevel: "off")
    ...
    @Test func sessionChoicesPreferMainAndRecent() async throws {
        ...
        let history = EspadaChatHistoryPayload(
            ...
            messages: [],
            thinkingLevel: "off")
        let sessions = EspadaChatSessionsListResponse(
            ts: now,
            path: nil,
            count: 4,
            defaults: nil,
    ...
    @Test func sessionChoicesIncludeCurrentWhenMissing() async throws {
        ...
        let transport = TestChatTransport(
            historyResponses: [history],
            sessionsResponses: [sessions])
        let vm = await MainActor.run { EspadaChatViewModel(sessionKey: "custom", transport: transport) }
        await MainActor.run { vm.load() }
        try await waitUntil("sessions loaded") { await MainActor.run { !vm.sessions.isEmpty } }
