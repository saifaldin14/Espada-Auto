...
import "@a2ui/lit/ui";
import { themeContext } from "@espada/a2ui-theme-context";
...
class EspadaA2UIHost extends LitElement {
  static properties = {
    surfaces: { state: true },
  ...
  static styles = css`
    :host {
      ...
      padding:
        var(--espada-a2ui-inset-top, 0px)
        var(--espada-a2ui-inset-right, 0px)
        var(--espada-a2ui-inset-bottom, 0px)
        var(--espada-a2ui-inset-left, 0px);
    }
    ...
    #surfaces {
      ...
      height: 100%;
      overflow: auto;
      padding-bottom: var(--espada-a2ui-scroll-pad-bottom, 0px);
    }
    ...
    .status {
      ...
      top: var(--espada-a2ui-status-top, 12px);
      display: inline-flex;
      align-items: center;
      gap: 8px;
      padding: 8px 10px;
    ...
    .toast {
      ...
      left: 50%;
      transform: translateX(-50%);
      bottom: var(--espada-a2ui-toast-bottom, 12px);
      display: inline-flex;
      align-items: center;
      gap: 8px;
      padding: 8px 10px;
    ...
    .empty {
      ...
      left: 50%;
      transform: translateX(-50%);
      top: var(--espada-a2ui-empty-top, var(--espada-a2ui-status-top, 12px));
      text-align: center;
      opacity: 0.8;
      padding: 10px 12px;
      pointer-events: none;
  ...
  connectedCallback() {
    ...
    globalThis.espadaA2UI = api;
    globalThis.espadaA2UI = api;
    this.addEventListener("a2uiaction", (evt) => this.#handleA2UIAction(evt));
    this.#statusListener = (evt) => this.#handleActionStatus(evt);
    globalThis.addEventListener("espada:a2ui-action-status", this.#statusListener);
    this.#syncSurfaces();
  }
  ...
  disconnectedCallback() {
    ...
    if (this.#statusListener) {
      globalThis.removeEventListener("espada:a2ui-action-status", this.#statusListener);
  ...
  #handleA2UIAction(evt) {
    ...
    globalThis.__espadaLastA2UIAction = userAction;

    const handler =
      globalThis.webkit?.messageHandlers?.espadaCanvasA2UIAction ??
      globalThis.webkit?.messageHandlers?.espadaCanvasA2UIAction ??
      globalThis.espadaCanvasA2UIAction ??
      globalThis.espadaCanvasA2UIAction;
    if (handler?.postMessage) {
      try {
        // WebKit message handlers support structured objects; Android's JS interface expects strings.
        if (
          handler === globalThis.espadaCanvasA2UIAction ||
          handler === globalThis.espadaCanvasA2UIAction
        ) {
...
customElements.define("espada-a2ui-host", EspadaA2UIHost);
