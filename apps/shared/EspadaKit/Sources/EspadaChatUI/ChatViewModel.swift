...
private let chatUILogger = Logger(subsystem: "bot.molt", category: "EspadaChatUI")

@MainActor
@Observable
public final class EspadaChatViewModel {
    public private(set) var messages: [EspadaChatMessage] = []
    ...
    public var attachments: [EspadaPendingAttachment] = []
    ...
    public private(set) var pendingToolCalls: [EspadaChatPendingToolCall] = []
    public private(set) var sessions: [EspadaChatSessionEntry] = []
    private let transport: any EspadaChatTransport
    ...
    private var pendingToolCallsById: [String: EspadaChatPendingToolCall] = [:] {
        didSet {
    ...
    public init(sessionKey: String, transport: any EspadaChatTransport) {
    ...
    public var sessionChoices: [EspadaChatSessionEntry] {
        let now = Date().timeIntervalSince1970 * 1000
        let cutoff = now - (24 * 60 * 60 * 1000)
        let sorted = self.sessions.sorted { ($0.updatedAt ?? 0) > ($1.updatedAt ?? 0) }
        var seen = Set<String>()
        var recent: [EspadaChatSessionEntry] = []
        for entry in sorted {
        ...
        var result: [EspadaChatSessionEntry] = []
    ...
    public func removeAttachment(_ id: EspadaPendingAttachment.ID) {
    ...
    private static func decodeMessages(_ raw: [AnyCodable]) -> [EspadaChatMessage] {
        let decoded = raw.compactMap { item in
            (try? ChatPayloadDecoding.decode(item, as: EspadaChatMessage.self))
        }
        return Self.dedupeMessages(decoded)
    }

    private static func dedupeMessages(_ messages: [EspadaChatMessage]) -> [EspadaChatMessage] {
        var result: [EspadaChatMessage] = []
    ...
    private static func dedupeKey(for message: EspadaChatMessage) -> String? {
    ...
    private func performSend() async {
        ...
        var userContent: [EspadaChatMessageContent] = [
        ...
        let encodedAttachments = self.attachments.map { att -> EspadaChatAttachmentPayload in
            EspadaChatAttachmentPayload(
                type: att.type,
    ...
    private func placeholderSession(key: String) -> EspadaChatSessionEntry {
        EspadaChatSessionEntry(
            key: key,
            kind: nil,
            displayName: nil,
            surface: nil,
    ...
    private func handleTransportEvent(_ evt: EspadaChatTransportEvent) {
        switch evt {
        case let .health(ok):
            self.healthOK = ok
        case .tick:
    ...
    private func handleChatEvent(_ chat: EspadaChatEventPayload) {
    ...
    private func handleAgentEvent(_ evt: EspadaAgentEventPayload) {
        ...
        case "tool":
            ...
            if phase == "start" {
                ...
                self.pendingToolCallsById[toolCallId] = EspadaChatPendingToolCall(
    ...
    private static func previewImage(data: Data) -> EspadaPlatformImage? {
        #if canImport(AppKit)
        NSImage(data: data)
        #elseif canImport(UIKit)
        UIImage(data: data)
