...
@MainActor
struct EspadaChatComposer: View {
    @Bindable var viewModel: EspadaChatViewModel
        ...
        .background {
            ...
            if self.style == .standard {
                let shape = UnevenRoundedRectangle(
                    ...
                    style: .continuous)
                shape
                    .fill(EspadaChatTheme.composerBackground)
                    .overlay(shape.strokeBorder(EspadaChatTheme.composerBorder, lineWidth: 1))
        ...
        ScrollView(.horizontal, showsIndicators: false) {
            HStack(spacing: 6) {
                ForEach(
                    ...
                    id: \EspadaPendingAttachment.id)
                { (att: EspadaPendingAttachment) in
                    HStack(spacing: 6) {
                        if let img = att.preview {
                            EspadaPlatformImageFactory.image(img)
                                .resizable()
                                .scaledToFill()
    ...
    private var editor: some View {
        ...
        .background(
            RoundedRectangle(cornerRadius: 12, style: .continuous)
                .fill(EspadaChatTheme.composerField)
                .overlay(
                    RoundedRectangle(cornerRadius: 12, style: .continuous)
                        .strokeBorder(EspadaChatTheme.composerBorder)))
    ...
    private var connectionPill: some View {
        ...
        .padding(.horizontal, 8)
        .padding(.vertical, 4)
        .background(EspadaChatTheme.subtleCard)
        .clipShape(Capsule())
    }
