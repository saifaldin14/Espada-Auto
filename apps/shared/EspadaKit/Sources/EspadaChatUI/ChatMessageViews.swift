...
struct ChatMessageBubble: View {
    let message: EspadaChatMessage
    let style: EspadaChatView.Style
...
private struct ChatMessageBody: View {
    ...
    var body: some View {
        ...
        let textColor = self.isUser ? EspadaChatTheme.userText : EspadaChatTheme.assistantText
    ...
    private var inlineAttachments: [EspadaChatMessageContent] {
    ...
    private var toolCalls: [EspadaChatMessageContent] {
        self.message.content.filter { content in
    ...
    private var inlineToolResults: [EspadaChatMessageContent] {
        self.message.content.filter { content in
            let kind = (content.type ?? "").lowercased()
    ...
    private var bubbleFillColor: Color {
        if self.isUser {
            return self.userAccent ?? EspadaChatTheme.userBubble
        }
        if self.style == .onboarding {
            return EspadaChatTheme.onboardingAssistantBubble
        }
        return EspadaChatTheme.assistantBubble
    }
    ...
    private var bubbleBorderColor: Color {
        ...
        if self.style == .onboarding {
            return EspadaChatTheme.onboardingAssistantBorder
        }
...
private struct AttachmentRow: View {
    let att: EspadaChatMessageContent
    let isUser: Bool
    ...
    var body: some View {
        HStack(spacing: 8) {
            ...
            Text(self.att.fileName ?? "Attachment")
                .font(.footnote)
                .lineLimit(1)
                .foregroundStyle(self.isUser ? EspadaChatTheme.userText : EspadaChatTheme.assistantText)
            Spacer()
        }
        .padding(10)
...
private struct ToolCallCard: View {
    let content: EspadaChatMessageContent
    let isUser: Bool
    ...
    var body: some View {
        ...
        .background(
            RoundedRectangle(cornerRadius: 12, style: .continuous)
                .fill(EspadaChatTheme.subtleCard)
                .overlay(
                    RoundedRectangle(cornerRadius: 12, style: .continuous)
    ...
    var body: some View {
        ...
        .background(
            RoundedRectangle(cornerRadius: 16, style: .continuous)
                .fill(EspadaChatTheme.assistantBubble))
        .overlay(
            RoundedRectangle(cornerRadius: 16, style: .continuous)
                .strokeBorder(Color.white.opacity(0.08), lineWidth: 1))
...
struct ChatPendingToolsBubble: View {
    let toolCalls: [EspadaChatPendingToolCall]
        ...
        VStack(alignment: .leading, spacing: 10) {
            ForEach(segments) { segment in
                ...
                ChatMarkdownRenderer(
                    ...
                    variant: self.markdownVariant,
                    font: font,
                    textColor: EspadaChatTheme.assistantText)
            }
        }
    }
}
