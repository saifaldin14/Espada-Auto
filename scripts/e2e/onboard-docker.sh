...
docker run --rm -t "$IMAGE_NAME" bash -lc '
  ...
  cat > /tmp/espada-bin/trash <<'"'"'TRASH'"'"'
#!/usr/bin/env bash
set -euo pipefail
...
TRASH
  ...
  run_wizard_cmd() {
    ...
    input_fifo="$(mktemp -u "/tmp/espada-onboard-${case_name}.XXXXXX")"
    mkfifo "$input_fifo"
    local log_path="/tmp/espada-onboard-${case_name}.log"
  ...
  make_home() {
    mktemp -d "/tmp/espada-e2e-$1.XXXXXX"
  }
...
NODE
  ...
  run_case_reset() {
    ...
    cat > "$HOME/.espada/espada.json" <<'"'"'JSON'"'"'
{
