...
IMAGE_NAME="espada-doctor-install-switch-e2e"
...
docker run --rm -t "$IMAGE_NAME" bash -lc '
  ...
  export PATH="/tmp/espada-bin:$PATH"
  mkdir -p /tmp/espada-bin

  cat > /tmp/espada-bin/systemctl <<"SYSTEMCTL"
#!/usr/bin/env bash
set -euo pipefail
...
SYSTEMCTL
  chmod +x /tmp/espada-bin/systemctl

  cat > /tmp/espada-bin/loginctl <<"LOGINCTL"
#!/usr/bin/env bash
set -euo pipefail
...
exit 0
LOGINCTL
  chmod +x /tmp/espada-bin/loginctl
  ...
  npm_bin="/tmp/npm-prefix/bin/espada"
  npm_entry="/tmp/npm-prefix/lib/node_modules/espada/espada.mjs"
  git_entry="/app/espada.mjs"
  ...
  run_flow() {
    ...
    echo "== Flow: $name =="
    home_dir=$(mktemp -d "/tmp/espada-switch-${name}.XXXXXX")
    export HOME="$home_dir"
    export USER="testuser"

    eval "$install_cmd"

    unit_path="$HOME/.config/systemd/user/espada-gateway.service"
    if [ ! -f "$unit_path" ]; then
