...
IMAGE_NAME="espada-gateway-network-e2e"

PORT="18789"
TOKEN="e2e-$(date +%s)-$$"
NET_NAME="espada-net-e2e-$$"
GW_NAME="espada-gateway-e2e-$$"
...
echo "Starting gateway container..."
	docker run --rm -d \
	  --name "$GW_NAME" \
	  --network "$NET_NAME" \
	  -e "ESPADA_GATEWAY_TOKEN=$TOKEN" \
	  -e "ESPADA_SKIP_CHANNELS=1" \
	  -e "ESPADA_SKIP_GMAIL_WATCHER=1" \
	  -e "ESPADA_SKIP_CRON=1" \
	  -e "ESPADA_SKIP_CANVAS_HOST=1" \
	  "$IMAGE_NAME" \
