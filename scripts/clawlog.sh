...
show_usage() {
    cat << EOF
clawlog - Espada Logging Utility

USAGE:
    clawlog [OPTIONS]

DESCRIPTION:
    View Espada logs with full details (bypasses Apple's privacy redaction).
    Requires sudo access configured for /usr/bin/log command.

LOG FLOW ARCHITECTURE:
    Espada logs flow through the macOS unified log (subsystem: bot.molt).
