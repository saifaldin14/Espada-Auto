#!/bin/bash
# Setup Espada Auth Management System
...
echo "  2. Subscribe to a topic (e.g., 'espada-alerts')"
echo ""
...
# Phone notification
echo ""
echo "  Espada message: Send warning via Espada itself"
echo "Enter your phone number for alerts (or leave blank to skip):"
read -r PHONE_NUMBER

# Update service file
SERVICE_FILE="$SCRIPT_DIR/systemd/espada-auth-monitor.service"
...
cp "$SCRIPT_DIR/systemd/espada-auth-monitor.service" ~/.config/systemd/user/
cp "$SCRIPT_DIR/systemd/espada-auth-monitor.timer" ~/.config/systemd/user/
systemctl --user daemon-reload
systemctl --user enable --now espada-auth-monitor.timer
...
echo "   scp $SCRIPT_DIR/termux-quick-auth.sh phone:~/.shortcuts/EspadaAuth"
echo "   scp $SCRIPT_DIR/termux-auth-widget.sh phone:~/.shortcuts/EspadaAuth-Full"
echo ""
echo "4. Make them executable on phone:"
echo "   ssh phone 'chmod +x ~/.shortcuts/Espada*'"
echo ""
echo "5. Add Termux:Widget to your home screen"
echo "6. Tap the widget to see your auth scripts"
echo ""
echo "The quick widget (EspadaAuth) shows status and opens auth URL if needed."
echo "The full widget (EspadaAuth-Full) provides guided re-auth flow."
echo ""
...
echo "  - Auth monitor: systemctl --user status espada-auth-monitor.timer"
echo ""
echo "Quick commands:"
