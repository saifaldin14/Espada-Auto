...
# Script to replace all instances of Espada/espada with Espada/espada
# Usage: ./scripts/rebrand-espada-to-espada.sh
...
FILE_PATTERN="*.ts *.tsx *.js *.jsx *.json *.md *.mdx *.yml *.yaml *.toml *.xml *.plist *.swift *.kt *.kts *.gradle *.sh *.py *.html *.css *.txt"
...
# Fix any directory references that shouldn't be changed
echo "🔧 Fixing workspace directory name..."
find . -type f \( -name "*.ts" -o -name "*.tsx" -o -name "*.js" -o -name "*.jsx" -o -name "*.json" -o -name "*.md" -o -name "*.mdx" \) \
  -not -path "./node_modules/*" \
  -not -path "./.git/*" \
  -print0 | xargs -0 sed -i '' 's/espada-main/espada-main/g'

# Fix package name - it should remain "espada"
