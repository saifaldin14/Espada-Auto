...
ZIP=${1:?"Usage: $0 Espada-<ver>.zip"}
FEED_URL=${2:-"https://raw.githubusercontent.com/espada/espada/main/appcast.xml"}
...
if [[ -z "$VERSION" ]]; then
  if [[ "$ZIP_NAME" =~ ^Espada-([0-9]+(\.[0-9]+){1,2}([-.][^.]*)?)\.zip$ ]]; then
...
DOWNLOAD_URL_PREFIX=${SPARKLE_DOWNLOAD_URL_PREFIX:-"https://github.com/espada/espada/releases/download/v${VERSION}/"}
