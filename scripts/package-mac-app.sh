...
# Build and bundle Espada into a minimal .app we can open.
# Outputs to dist/Espada.app

ROOT_DIR="$(cd "$(dirname "$0")/.." && pwd)"
APP_ROOT="$ROOT_DIR/dist/Espada.app"
...
SPARKLE_FEED_URL="${SPARKLE_FEED_URL:-https://raw.githubusercontent.com/espada/espada/main/appcast.xml}"
AUTO_CHECKS=true
...
INFO_PLIST_SRC="$ROOT_DIR/apps/macos/Sources/Espada/Resources/Info.plist"
...
/usr/libexec/PlistBuddy -c "Set :EspadaBuildTimestamp ${BUILD_TS}" "$APP_ROOT/Contents/Info.plist" || true
/usr/libexec/PlistBuddy -c "Set :EspadaGitCommit ${GIT_COMMIT}" "$APP_ROOT/Contents/Info.plist" || true
...
echo "🚚 Copying binary"
cp "$BIN_PRIMARY" "$APP_ROOT/Contents/MacOS/Espada"
if [[ "${#BUILD_ARCHS[@]}" -gt 1 ]]; then
  BIN_INPUTS=()
  for arch in "${BUILD_ARCHS[@]}"; do
    BIN_INPUTS+=("$(bin_for_arch "$arch")")
  done
  /usr/bin/lipo -create "${BIN_INPUTS[@]}" -output "$APP_ROOT/Contents/MacOS/Espada"
fi
chmod +x "$APP_ROOT/Contents/MacOS/Espada"
# SwiftPM outputs ad-hoc signed binaries; strip the signature before install_name_tool to avoid warnings.
/usr/bin/codesign --remove-signature "$APP_ROOT/Contents/MacOS/Espada" 2>/dev/null || true
...
cp "$ROOT_DIR/apps/macos/Sources/Espada/Resources/Espada.icns" "$APP_ROOT/Contents/Resources/Espada.icns"

echo "📦 Copying device model resources"
rm -rf "$APP_ROOT/Contents/Resources/DeviceModels"
cp -R "$ROOT_DIR/apps/macos/Sources/Espada/Resources/DeviceModels" "$APP_ROOT/Contents/Resources/DeviceModels"
...
echo "📦 Copying EspadaKit resources"
ESPADAKIT_BUNDLE="$(build_path_for_arch "$PRIMARY_ARCH")/$BUILD_CONFIG/EspadaKit_EspadaKit.bundle"
if [ -d "$ESPADAKIT_BUNDLE" ]; then
  rm -rf "$APP_ROOT/Contents/Resources/EspadaKit_EspadaKit.bundle"
  cp -R "$ESPADAKIT_BUNDLE" "$APP_ROOT/Contents/Resources/EspadaKit_EspadaKit.bundle"
else
  echo "WARN: EspadaKit resource bundle not found at $ESPADAKIT_BUNDLE (continuing)" >&2
fi
...
echo "⏹  Stopping any running Espada"
killall -q Espada 2>/dev/null || true
