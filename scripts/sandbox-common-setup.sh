...
BASE_IMAGE="${BASE_IMAGE:-espada-sandbox:bookworm-slim}"
TARGET_IMAGE="${TARGET_IMAGE:-espada-sandbox-common:bookworm-slim}"
...
If you want a clean re-create, remove old sandbox containers:
  docker rm -f \$(docker ps -aq --filter label=espada.sandbox=1)
NOTE
