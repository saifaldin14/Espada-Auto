...
STATUS=$(ssh -o ConnectTimeout=5 "$SERVER" '$HOME/espada/scripts/claude-auth-status.sh simple' 2>&1)
...
case "$STATUS" in
    ...
    *EXPIRED*|*MISSING*)
        ...
        sleep 2
        termux-notification -t "Espada Re-Auth" -c "After getting key, run: ssh $SERVER '~/espada/scripts/mobile-reauth.sh'" --id espada-auth
        ;;
    *)
