#!/usr/bin/env bash
# Reset Espada like Trimmy: kill running instances, rebuild, repackage, relaunch, verify.

set -euo pipefail

ROOT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
APP_BUNDLE="${ESPADA_APP_BUNDLE:-}"
APP_PROCESS_PATTERN="Espada.app/Contents/MacOS/Espada"
DEBUG_PROCESS_PATTERN="${ROOT_DIR}/apps/macos/.build/debug/Espada"
LOCAL_PROCESS_PATTERN="${ROOT_DIR}/apps/macos/.build-local/debug/Espada"
RELEASE_PROCESS_PATTERN="${ROOT_DIR}/apps/macos/.build/release/Espada"
LAUNCH_AGENT="${HOME}/Library/LaunchAgents/bot.molt.mac.plist"
LOCK_KEY="$(printf '%s' "${ROOT_DIR}" | shasum -a 256 | cut -c1-8)"
LOCK_DIR="${TMPDIR:-/tmp}/espada-restart-${LOCK_KEY}"
LOCK_PID_FILE="${LOCK_DIR}/pid"
WAIT_FOR_LOCK=0
LOG_PATH="${ESPADA_RESTART_LOG:-/tmp/espada-restart.log}"
NO_SIGN=0
SIGN=0
AUTO_DETECT_SIGNING=1
GATEWAY_WAIT_SECONDS="${ESPADA_GATEWAY_WAIT_SECONDS:-0}"
LAUNCHAGENT_DISABLE_MARKER="${HOME}/.espada/disable-launchagent"
ATTACH_ONLY=1
...
for arg in "$@"; do
  case "${arg}" in
    ...
    --help|-h)
      ...
      log ""
      log "Env:"
      log "  ESPADA_GATEWAY_WAIT_SECONDS=0  Wait time before gateway port check (unsigned only)"
      log ""
      log "Unsigned recovery:"
      log "  node espada.mjs daemon install --force --runtime node"
      log "  node espada.mjs daemon restart"
      log ""
      log "Reset unsigned overrides:"
      log "  rm ~/.espada/disable-launchagent"
      log ""
...
log "==> Killing existing Espada instances"
kill_all_espada
stop_launch_agent
...
run_step "swift build" bash -lc "cd '${ROOT_DIR}/apps/macos' && swift build -q --product Espada"
...
choose_app_bundle() {
  ...
  if [[ -d "/Applications/Espada.app" ]]; then
    APP_BUNDLE="/Applications/Espada.app"
    return 0
  fi

  if [[ -d "${ROOT_DIR}/dist/Espada.app" ]]; then
    APP_BUNDLE="${ROOT_DIR}/dist/Espada.app"
    if [[ ! -d "${APP_BUNDLE}/Contents/Frameworks/Sparkle.framework" ]]; then
      fail "dist/Espada.app missing Sparkle after packaging"
    fi
    return 0
  fi

  fail "App bundle not found. Set ESPADA_APP_BUNDLE to your installed Espada.app"
}
...
if [ "$NO_SIGN" -eq 1 ] && [ "$ATTACH_ONLY" -ne 1 ]; then
  run_step "install gateway launch agent (unsigned)" bash -lc "cd '${ROOT_DIR}' && node espada.mjs daemon install --force --runtime node"
  run_step "restart gateway daemon (unsigned)" bash -lc "cd '${ROOT_DIR}' && node espada.mjs daemon restart"
  ...
  GATEWAY_PORT="$(
    node -e '
      ...
      try {
        const raw = fs.readFileSync(path.join(process.env.HOME, ".espada", "espada.json"), "utf8");
