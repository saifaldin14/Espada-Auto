...
PACKAGE_NAME="${ESPADA_INSTALL_PACKAGE:-$DEFAULT_PACKAGE}"
if [[ "$PACKAGE_NAME" == "espada" ]]; then
  ALT_PACKAGE_NAME="espada"
elif [[ "$PACKAGE_NAME" == "espada" ]]; then
  ALT_PACKAGE_NAME="espada"
else
  ALT_PACKAGE_NAME="espada"
fi
...
if [[ "$SKIP_PREVIOUS" == "1" ]]; then
  echo "==> Skip preinstall previous (ESPADA_INSTALL_SMOKE_SKIP_PREVIOUS=1)"
else
...
if [[ -n "${ESPADA_INSTALL_LATEST_OUT:-}" ]]; then
  printf "%s" "$LATEST_VERSION" > "$ESPADA_INSTALL_LATEST_OUT"
fi
