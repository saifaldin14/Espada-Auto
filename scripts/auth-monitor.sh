...
#
# Suggested cron: */30 * * * * /home/admin/espada/scripts/auth-monitor.sh
#
# Environment variables:
#   NOTIFY_PHONE - Phone number to send Espada notification (e.g., +1234567890)
#   NOTIFY_NTFY  - ntfy.sh topic for push notifications (e.g., espada-alerts)
...
STATE_FILE="$HOME/.espada/auth-monitor-state"
...
send_notification() {
    ...
    # Send via Espada if phone configured and auth still valid
    if [ -n "$NOTIFY_PHONE" ]; then
        # Check if we can still use espada
        if "$SCRIPT_DIR/claude-auth-status.sh" simple 2>/dev/null | grep -q "OK\|EXPIRING"; then
            echo "Sending via Espada to $NOTIFY_PHONE..."
            espada send --to "$NOTIFY_PHONE" --message "$message" 2>/dev/null || true
        fi
    fi
...
if [ "$DIFF_MS" -lt 0 ]; then
    send_notification "Claude Code auth EXPIRED! Espada is down. Run: ssh l36 '~/espada/scripts/mobile-reauth.sh'" "urgent"
    exit 1
