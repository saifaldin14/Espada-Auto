...
const compiler = env.ESPADA_TS_COMPILER === "tsc" ? "tsc" : "tsgo";
...
const shouldBuild = () => {
  if (env.ESPADA_FORCE_BUILD === "1") return true;
...
const logRunner = (message) => {
  if (env.ESPADA_RUNNER_LOG === "0") return;
  process.stderr.write(`[espada] ${message}\n`);
};

const runNode = () => {
  const nodeProcess = spawn(process.execPath, ["espada.mjs", ...args], {
    cwd,
    env,
    stdio: "inherit",
  });
