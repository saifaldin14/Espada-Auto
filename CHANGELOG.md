...
### Changes
- Rebrand: rename the npm package/CLI to `espada`, add a `espada` compatibility shim, and move extensions to the `@espada/*` scope.
- Commands: group /help and /commands output with Telegram paging. (#2504) Thanks @hougangdev.
- macOS: limit project-local `node_modules/.bin` PATH preference to debug builds (reduce PATH hijacking risk).
- macOS: finish Espada app rename for macOS sources, bundle identifiers, and shared kit paths. (#2844) Thanks @fal3.
- Branding: update launchd labels, mobile bundle IDs, and logging subsystems to bot.molt (legacy com.espada migrations). Thanks @thewilloftheshadow.
...
- Docs: update exe.dev install instructions. (#https://github.com/espada/espada/pull/3047) Thanks @zackerthescar.
...
- CLI: add `espada system` for system events + heartbeat controls; remove standalone `wake`. (commit 71203829d) https://docs.molt.bot/cli/system
- CLI: add live auth probes to `espada models status` for per-profile verification. (commit 40181afde) https://docs.molt.bot/cli/models
- CLI: restart the gateway by default after `espada update`; add `--no-restart` to skip it. (commit 2c85b1b40)
...
- CLI: auth probe output polish (table output, inline errors, reduced noise, and wrap fixes in `espada models status`). (commit da3f2b489, commit 00ae21bed, commit 31e59cd58, commit f7dc27f2d, commit 438e782f8, commit 886752217, commit aabe0bed3, commit 81535d512, commit c63144ab1)
...
- Doctor: honor ESPADA_GATEWAY_TOKEN for auth checks and security audit token reuse. (#1448) Thanks @azade-c.
...
- CLI: flatten node service commands under `espada node` and remove `service node` docs.
- CLI: move gateway service commands under `espada gateway` and add `gateway probe` for reachability.
- Sessions: add per-channel reset overrides via `session.resetByChannel`. (#1353) Thanks @cash-echo-bot.
- Agents: add identity avatar config support and Control UI avatar rendering. (#1329, #1424) Thanks @dlauer.
- UI: show per-session assistant identity in the Control UI. (#1420) Thanks @robbyczgw-cla.
- CLI: add `espada update wizard` for interactive channel selection and restart prompts. https://docs.molt.bot/cli/update
...
- ACP: add `espada acp` for IDE integrations. https://docs.molt.bot/cli/acp
- ACP: add `espada acp client` interactive harness for debugging. https://docs.molt.bot/cli/acp
...
- Plugins: sync plugin sources on channel switches and update npm-installed plugins during `espada update`.
- Plugins: share npm plugin update logic between `espada update` and `espada plugins update`.
...
- Exec approvals: migrate approvals to `~/.espada/exec-approvals.json` with per-agent allowlists + skill auto-allow toggle, and add approvals UI + node exec lifecycle events. https://docs.molt.bot/tools/exec-approvals
- Nodes: add headless node host (`espada node start`) for `system.run`/`system.which`. https://docs.molt.bot/cli/node
...
- Build: use tsgo for dev/watch builds by default (opt out with `ESPADA_TS_COMPILER=tsc`).
...
### Breaking
- **BREAKING:** Reject invalid/unknown config entries and refuse to start the gateway for safety. Run `espada doctor --fix` to repair, then update plugins (`espada plugins update`) if you use any.

### Fixes
- Discovery: shorten Bonjour DNS-SD service type to `_espada-gw._tcp` and update discovery clients/docs.
...
- CLI: keep `espada logs` output resilient to broken pipes while preserving progress output.
...
- Memory: show total file counts + scan issues in `espada memory status`.
...
- Memory: probe sqlite-vec availability in `espada memory status`.
...
- Plugins: add Zalo Personal plugin (`@espada/zalouser`) and unify channel directory for plugins. (#1032) — thanks @suminhthanh. https://docs.molt.bot/plugins/zalouser
...
### Breaking
- **BREAKING:** `espada message` and message tool now require `target` (dropping `to`/`channelId` for destinations). (#1034) — thanks @tobalsan.
- **BREAKING:** Channel auth now prefers config over env for Discord/Telegram/Matrix (env is fallback only). (#1040) — thanks @thewilloftheshadow.
- **BREAKING:** Drop legacy `chatType: "room"` support; use `chatType: "channel"`.
- **BREAKING:** remove legacy provider-specific target resolution fallbacks; target resolution is centralized with plugin hints + directory lookups.
- **BREAKING:** `espada hooks` is now `espada webhooks`; hooks live under `espada hooks`. https://docs.molt.bot/cli/webhooks
- **BREAKING:** `espada plugins install <path>` now copies into `~/.espada/extensions` (use `--link` to keep path-based loading).
...
- Directory: unify `espada directory` across channels and plugin channels.
...
- Hooks: add hook pack installs (npm/path/zip/tar) with `espada.hooks` manifests and `espada hooks install/update`.
...
- CLI: speed up `espada sandbox-explain` by avoiding heavy plugin imports when normalizing channel ids.
...
### Highlights
- Plugins: add provider auth registry + `espada models auth login` for plugin-driven OAuth/API key flows.
...
- **BREAKING:** Microsoft Teams is now a plugin; install `@espada/msteams` via `espada plugins install @espada/msteams`.
- **BREAKING:** Channel auth now prefers config over env for Discord/Telegram/Matrix (env is fallback only). (#1040) — thanks @thewilloftheshadow.

### Changes
- UI/Apps: move channel/config settings to schema-driven forms and rename Connections → Channels. (#1040) — thanks @thewilloftheshadow.
- CLI: set process titles to `espada-<command>` for clearer process listings.
...
- Security: warn on weak model tiers (Haiku, below GPT-5, below Claude 4.5) in `espada security audit`.
- Apps: store node auth tokens encrypted (Keychain/SecurePrefs).
- Daemon: share profile/state-dir resolution across service helpers and honor `ESPADA_STATE_DIR` for Windows task scripts.
...
- Auth/Status: keep auth profiles sticky per session (rotate on compaction/new), surface provider usage headers in `/status` and `espada models status`, and update docs.
- CLI: add `--json` output for `espada daemon` lifecycle/install commands.
...
- Fix: make `espada update` auto-update global installs when installed via a package manager.
- Fix: list model picker entries as provider/model pairs for explicit selection. (#970) — thanks @mcinteerj.
- Fix: align OpenAI image-gen defaults with DALL-E 3 standard quality and document output formats. (#880) — thanks @mkbehr.
- Fix: persist `gateway.mode=local` after selecting Local run mode in `espada configure`, even if no other sections are chosen.
...
- Security: expanded `espada security audit` (+ `--fix`), detect-secrets CI scan, and a `SECURITY.md` reporting policy.

### Changes
- Docs: clarify per-agent auth stores, sandboxed skill binaries, and elevated semantics.
- Docs: add FAQ entries for missing provider auth after adding agents and Gemini thinking signature errors.
- Agents: add optional auth-profile copy prompt on `agents add` and improve auth error messaging.
- Security: expand `espada security audit` checks (model hygiene, config includes, plugin allowlists, exposure matrix) and extend `--fix` to tighten more sensitive state paths.
...
- Browser: add Chrome extension relay takeover mode (toolbar button), plus `espada browser extension install/path` and remote browser control (standalone server + token auth).
...
- Gateway/Dev: ensure `pnpm gateway:dev` always uses the dev profile config + state (`~/.espada-dev`).
...
- Memory: new `espada memory` CLI plus `memory_search`/`memory_get` tools with snippets + line ranges; index stored under `~/.espada/memory/{agentId}.sqlite` with watch-on-by-default.
...
- Gateway/CLI: add Tailscale binary discovery, custom bind mode, and probe auth retry; add `espada dashboard` auto-open flow; default native slash commands to `"auto"` with per-provider overrides. (#740) — thanks @jeffersonwarrior.
...
### Installer
- Install: run `espada doctor --non-interactive` after git installs/updates and nudge daemon restarts when detected.
...
- Onboarding/Auth: honor `ESPADA_AGENT_DIR` / `PI_CODING_AGENT_DIR` when writing auth profiles (MiniMax). (#829) — thanks @roshanasingh4.
...
- Gateway/CLI: honor `ESPADA_LAUNCHD_LABEL` / `ESPADA_SYSTEMD_UNIT` overrides; `agents.list` respects explicit config; reduce noisy loopback WS logs during tests; run `espada doctor --non-interactive` during updates. (#781) — thanks @ronyrus.
...
- Plugins: add `espada plugins install` (path/tgz/npm), plus `list|info|enable|disable|doctor` UX.
...
- macOS: prompt to install the global `espada` CLI when missing in local mode; install via `molt.bot/install-cli.sh` (no onboarding) and use external launchd/CLI instead of the embedded gateway runtime.
...
- Installer UX: support `ESPADA_NO_ONBOARD=1` for non-interactive installs; fix npm prefix on Linux and auto-install git.
...
- Plugins: treat `plugins.load.paths` directory entries as package roots when they contain `package.json` + `espada.extensions`; load plugin packages from config dirs; extract archives without system tar.
- Config: expand `~` in `ESPADA_CONFIG_PATH` and common path-like config fields (including `plugins.load.paths`); guard invalid `$include` paths. (#731) — thanks @pasogott.
...
### Highlights
- CLI: `espada status` now table-based + shows OS/update/gateway/daemon/agents/sessions; `status --all` adds a full read-only debug report (tables, log tails, Tailscale summary, and scan progress via OSC-9 + spinner).
...
- CLI: add `espada update` (safe-ish git checkout update) + `--update` shorthand. (#673) — thanks @fm1randa.
...
- CLI: add `espada reset` and `espada uninstall` flows (interactive + non-interactive) plus docker cleanup smoke test.
...
- CLI/Gateway: clarify that `espada gateway status` reports RPC health (connect + RPC) and shows RPC failures separately from connect failures.
...
- Docs: make `espada status` the first diagnostic step, clarify `status --deep` behavior, and document `/whoami` + `/id`.
...
- Sandbox: add `espada sandbox explain` (effective policy inspector + fix-it keys); improve “sandbox jail” tool-policy/elevated errors with actionable config key paths; link to docs.
...
- CLI: respect `ESPADA_STATE_DIR` for node pairing + voice wake settings storage. (#664) — thanks @azade-c.
...
- Gateway/Agent: accept image attachments on `agent` (multimodal message) and add live gateway image probe (`ESPADA_LIVE_GATEWAY_IMAGE_PROBE=1`).
- CLI: `espada sessions` now includes `elev:*` + `usage:*` flags in the table output.
- CLI/Pairing: accept positional provider for `pairing list|approve` (npm-run compatible); update docs/bot hints.
- Branding: normalize legacy casing/branding to “Espada” (CLI, status, docs).
- Auto-reply: fix native `/model` not updating the actual chat session (Telegram/Slack/Discord). (#646)
- Doctor: offer to run `espada update` first on git installs (keeps doctor output aligned with latest).
- Doctor: avoid false legacy workspace warning when install dir is `~/espada`. (#660)
...
### Breaking
- CLI: `espada message` now subcommands (`message send|poll|...`) and requires `--provider` unless only one provider configured.
...
- Models/Auth: setup-token + token auth profiles; `espada models auth order {get,set,clear}`; per-agent auth candidates in `/model status`; OAuth expiry checks in doctor/status.
...
- Config/Onboarding: inline env vars; OpenAI API key flow to shared `~/.espada/.env`; Opus 4.5 default prompt for Anthropic auth; QuickStart auto-install gateway (Node-only) + provider picker tweaks + skip-systemd flags; TUI bootstrap prompt (`tui --message`); remove Bun runtime choice.
...
- Gateway/Daemon/Doctor: atomic config writes; repair gateway service entrypoint + install switches; non-interactive legacy migrations; systemd unit alignment + KillMode=process; node bridge keepalive/pings; Launch at Login persistence; bundle EspadaKit resources + Swift 6.2 compat dylib; relay version check + remove smoke test; regen Swift GatewayModels + keep agent provider string; cron jobId alias + channel alias migration + main session key normalization; heartbeat Telegram accountId resolution; avoid WhatsApp fallback for internal runs; gateway listener error wording; serveBaseUrl param; honor gateway --dev; fix wide-area discovery updates; align agents.defaults schema; provider account metadata in daemon status; refresh Carbon patch for gateway fixes; restore doctor prompter initialValue handling.
...
- **SECURITY (update ASAP):** inbound DMs are now **locked down by default** on Telegram/WhatsApp/Signal/iMessage/Discord/Slack.
  ...
  - Approve requests via `espada pairing list <provider>` + `espada pairing approve <provider> <code>`.
...
- **Gateway/CLI UX:** `espada logs`, cron list colors/aliases, docs search, agents list/add/delete flows, status usage snapshots, runtime/auth source display, and `/status`/commands auth unification.
...
- Env: load global `$ESPADA_STATE_DIR/.env` (`~/.espada/.env`) as a fallback after CWD `.env`.
...
- CLI: run `espada agent` via the Gateway by default; use `--local` to force embedded mode.
