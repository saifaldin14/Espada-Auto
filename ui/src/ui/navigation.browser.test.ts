...
describe("control UI routing", () => {
  ...
  it("infers nested base paths", async () => {
    const app = mountApp("/apps/espada/cron");
    await app.updateComplete;

    expect(app.basePath).toBe("/apps/espada");
    expect(app.tab).toBe("cron");
    expect(window.location.pathname).toBe("/apps/espada/cron");
  });

  it("honors explicit base path overrides", async () => {
    window.__ESPADA_CONTROL_UI_BASE_PATH__ = "/espada";
    const app = mountApp("/espada/sessions");
    await app.updateComplete;

    expect(app.basePath).toBe("/espada");
    expect(app.tab).toBe("sessions");
    expect(window.location.pathname).toBe("/espada/sessions");
  });
