...
export async function handleWhatsAppStart(host: EspadaApp, force: boolean) {
  await startWhatsAppLogin(host, force);
  await loadChannels(host, true);
}

export async function handleWhatsAppWait(host: EspadaApp) {
  await waitWhatsAppLogin(host);
  await loadChannels(host, true);
}

export async function handleWhatsAppLogout(host: EspadaApp) {
  await logoutWhatsApp(host);
  await loadChannels(host, true);
}

export async function handleChannelConfigSave(host: EspadaApp) {
  await saveConfig(host);
  await loadConfig(host);
  await loadChannels(host, true);
}

export async function handleChannelConfigReload(host: EspadaApp) {
  await loadConfig(host);
  await loadChannels(host, true);
}
...
function resolveNostrAccountId(host: EspadaApp): string {
...
export function handleNostrProfileCancel(host: EspadaApp) {
...
export function handleNostrProfileToggleAdvanced(host: EspadaApp) {
  const state = host.nostrProfileFormState;
  if (!state) return;
  host.nostrProfileFormState = {
    ...state,
...
export async function handleNostrProfileSave(host: EspadaApp) {
  const state = host.nostrProfileFormState;
...
export async function handleNostrProfileImport(host: EspadaApp) {
  const state = host.nostrProfileFormState;
