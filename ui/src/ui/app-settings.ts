...
import type { EspadaApp } from "./app";
...
export async function refreshActiveTab(host: SettingsHost) {
  ...
  if (host.tab === "instances") await loadPresence(host as unknown as EspadaApp);
  if (host.tab === "sessions") await loadSessions(host as unknown as EspadaApp);
  if (host.tab === "cron") await loadCron(host);
  if (host.tab === "skills") await loadSkills(host as unknown as EspadaApp);
  if (host.tab === "nodes") {
    await loadNodes(host as unknown as EspadaApp);
    await loadDevices(host as unknown as EspadaApp);
    await loadConfig(host as unknown as EspadaApp);
    await loadExecApprovals(host as unknown as EspadaApp);
  }
  if (host.tab === "chat") {
  ...
  if (host.tab === "config") {
    await loadConfigSchema(host as unknown as EspadaApp);
    await loadConfig(host as unknown as EspadaApp);
  }
  if (host.tab === "debug") {
    await loadDebug(host as unknown as EspadaApp);
    host.eventLog = host.eventLogBuffer;
  }
  if (host.tab === "logs") {
    host.logsAtBottom = true;
    await loadLogs(host as unknown as EspadaApp, { reset: true });
    scheduleLogsScroll(
...
export function inferBasePath() {
  ...
  const configured = window.__ESPADA_CONTROL_UI_BASE_PATH__;
...
export async function loadOverview(host: SettingsHost) {
  await Promise.all([
    loadChannels(host as unknown as EspadaApp, false),
    loadPresence(host as unknown as EspadaApp),
    loadSessions(host as unknown as EspadaApp),
    loadCronStatus(host as unknown as EspadaApp),
    loadDebug(host as unknown as EspadaApp),
  ]);
}

export async function loadChannelsTab(host: SettingsHost) {
  await Promise.all([
    loadChannels(host as unknown as EspadaApp, true),
    loadConfigSchema(host as unknown as EspadaApp),
    loadConfig(host as unknown as EspadaApp),
  ]);
}
...
export async function loadCron(host: SettingsHost) {
  await Promise.all([
    loadChannels(host as unknown as EspadaApp, false),
    loadCronStatus(host as unknown as EspadaApp),
    loadCronJobs(host as unknown as EspadaApp),
  ]);
}
