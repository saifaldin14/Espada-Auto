...
read_when:
  ...
  - Investigating tsx/esbuild loader crashes in Espada
---

# Node + tsx "__name is not a function" crash

## Summary
Running Espada via Node with `tsx` fails at startup with:

```
[espada] Failed to start CLI: TypeError: __name is not a function
...
- Before that (Bun path), `espada status` and `gateway:watch` worked.
...
- Use Node + tsc watch, then run compiled output:
  ...
  node --watch espada.mjs status
  ```
- Confirmed locally: `pnpm exec tsc -p tsconfig.json` + `node espada.mjs status` works on Node 25.
