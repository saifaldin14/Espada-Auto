...
- **Ask default:** `on-miss`.
- **Approvals store:** `~/.espada/exec-approvals.json` (JSON, no legacy migration).
- **Runner:** headless system service; UI app hosts a Unix socket for approvals.
- **Node identity:** use existing `nodeId`.
- **Socket auth:** Unix socket + token (cross-platform); split later if needed.
- **Node host state:** `~/.espada/node.json` (node id + pairing token).
...
Path: `~/.espada/exec-approvals.json`
...
```json
{
  "version": 1,
  "socket": {
    "path": "~/.espada/exec-approvals.sock",
...
### IPC
- Unix socket at `~/.espada/exec-approvals.sock` (0600).
