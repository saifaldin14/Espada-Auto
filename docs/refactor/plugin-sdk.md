...
Delivery:
- Publish as `@espada/plugin-sdk` (or export from core under `espada/plugin-sdk`).
- Semver with explicit stability guarantees.

### 2) Plugin Runtime (execution surface, injected)
Scope: everything that touches core runtime behavior.
Accessed via `EspadaPluginApi.runtime` so plugins never import `src/**`.
...
export type PluginRuntime = {
  channel: {
    text: {
      ...
      resolveTextChunkLimit(cfg: EspadaConfig, channel: string, accountId?: string): number;
      hasControlCommand(text: string, cfg: EspadaConfig): boolean;
    };
    reply: {
    ...
    mentions: {
      buildMentionRegexes(cfg: EspadaConfig, agentId?: string): RegExp[];
      matchesMentionPatterns(text: string, regexes: RegExp[]): boolean;
    };
    groups: {
      resolveGroupPolicy(cfg: EspadaConfig, channel: string, accountId: string, groupId: string): {
    ...
    debounce: {
      ...
      resolveInboundDebounceMs(cfg: EspadaConfig, channel: string): number;
    };
    commands: {
  ...
  state: {
    resolveStateDir(cfg: EspadaConfig): string;
  };
};
```
...
### Phase 0: scaffolding
- Introduce `@espada/plugin-sdk`.
- Add `api.runtime` to `EspadaPluginApi` with the surface above.
...
- Plugins declare a required runtime range (e.g., `espadaRuntime: ">=2026.2.0"`).
