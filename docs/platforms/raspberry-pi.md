---
summary: "Espada on Raspberry Pi (budget self-hosted setup)"
read_when:
  - Setting up Espada on a Raspberry Pi
  - Running Espada on ARM devices
  - Building a cheap always-on personal AI
---

# Espada on Raspberry Pi

## Goal

Run a persistent, always-on Espada Gateway on a Raspberry Pi for **~$35-80** one-time cost (no monthly fees).
...
# Update config
espada config set gateway.bind tailnet
...
Most Espada features work on ARM64, but some external binaries may need ARM builds:
...
sudo systemctl is-enabled espada

# Enable if not
sudo systemctl enable espada

# Start on boot
sudo systemctl start espada
```
...
```bash
# Check logs
journalctl -u espada --no-pager -n 100

# Common fix: rebuild
cd ~/espada  # if using hackable install
npm run build
sudo systemctl restart espada
```
