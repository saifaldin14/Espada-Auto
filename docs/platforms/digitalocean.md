---
summary: "Espada on DigitalOcean (simple paid VPS option)"
read_when:
  - Setting up Espada on DigitalOcean
  - Looking for cheap VPS hosting for Espada
---

# Espada on DigitalOcean

## Goal

Run a persistent Espada Gateway on DigitalOcean for **$6/month** (or $4/mo with reserved pricing).
...
# Check service
systemctl --user status espada-gateway.service

# View logs
journalctl --user -u espada-gateway.service -f
```
...
### WhatsApp
```bash
espada channels login whatsapp
# Scan QR code
```
...
journalctl -u espada --no-pager -n 50
```
