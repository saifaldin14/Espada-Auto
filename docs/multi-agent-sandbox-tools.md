...
For debugging “why is this blocked?”, see [Sandbox vs Tool Policy vs Elevated](/gateway/sandbox-vs-tool-policy-vs-elevated) and `espada sandbox explain`.
...
Legacy `agent.*` configs are migrated by `espada doctor`; prefer `agents.defaults` + `agents.list` going forward.
...
2. **Verify sandbox containers:**
   ```exec
   docker ps --filter "label=espada.sandbox=1"
   ```
...
4. **Monitor logs:**
   ```exec
   tail -f "${ESPADA_STATE_DIR:-$HOME/.espada}/logs/gateway.log" | grep -E "routing|sandbox|tools"
   ```
