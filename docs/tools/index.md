---
summary: "Agent tool surface for Espada (browser, canvas, nodes, message, cron) replacing legacy `espada-*` skills"
read_when:
  - Adding or modifying agent tools
  - Retiring or changing `espada-*` skills
---

# Tools (Espada)

Espada exposes **first-class agent tools** for browser, canvas, nodes, and cron.
These replace the old `espada-*` skills: the tools are typed, no shelling,
and the agent should rely on them directly.

## Disabling tools

You can globally allow/deny tools via `tools.allow` / `tools.deny` in `espada.json`
...
- If `tools.allow` only references unknown or unloaded plugin tool names, Espada logs a warning and ignores the allowlist so core tools stay available.
...
- `host=node` can target a macOS companion app or a headless node host (`espada node run`).
...
Notes:
- Requires a Brave API key (recommended: `espada configure --section web`, or set `BRAVE_API_KEY`).
...
- Quick smoke: `espada nodes canvas a2ui push --node <id> --text "Hello from A2UI"`.
...
Core actions:
- `restart` (authorizes + sends `SIGUSR1` for in-process restart; `espada gateway` restart in-place)
