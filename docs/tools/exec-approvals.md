...
- **gateway host** → `espada` process on the gateway machine
...
`~/.espada/exec-approvals.json` directly.

CLI: `espada approvals` supports gateway or node editing (see [Approvals CLI](/cli/approvals)).
