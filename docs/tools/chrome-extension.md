---
summary: "Chrome extension: let Espada drive your existing Chrome tab"
read_when:
...
The Espada Chrome extension lets the agent control your **existing Chrome tabs** (your normal Chrome window) instead of launching a separate espada-managed Chrome profile.
...
Espada then controls the attached tab through the normal `browser` tool surface (selecting the right profile).
...
The extension ships inside the Espada release (npm package) as static files. There is no separate “build” step.

After upgrading Espada:
- Re-run `espada browser extension install` to refresh the installed files under your Espada state directory.
- Chrome → `chrome://extensions` → click “Reload” on the extension.

## Use it (no extra config)

Espada ships with a built-in browser profile named `chrome` that targets the extension relay on the default port.
...
- Open the tab you want Espada to control.
...
- `ON`: attached; Espada can drive that tab.
...
Debugging: `espada sandbox explain`
...
`espada browser extension path` prints the **installed** on-disk directory containing the extension files.

The CLI intentionally does **not** print a `node_modules` path. Always run `espada browser extension install` first to copy the extension to a stable location under your Espada state directory.
...
- **This is not isolated** like the dedicated espada-managed profile.
