---
summary: "Fix Chrome/Brave/Edge/Chromium CDP startup issues for Espada browser control on Linux"
...
On Ubuntu (and many Linux distros), the default Chromium installation is a **snap package**. Snap's AppArmor confinement interferes with how Espada spawns and monitors the browser process.
...
Then update your Espada config (`~/.espada/espada.json`):
...
If you must use snap Chromium, configure Espada to attach to a manually-started browser:
...
You’re using the `chrome` profile (extension relay). It expects the Espada
