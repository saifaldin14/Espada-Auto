...
- `/usage cost` in chats: local cost summary aggregated from Espada session logs.
- CLI: `espada status --usage` prints a full per-provider breakdown.
- CLI: `espada channels list` prints the same usage snapshot alongside provider config (use `--no-usage` to skip).
