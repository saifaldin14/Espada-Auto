...
- **Session store** (chat history + routing state) under `~/.espada/agents/<agentId>/sessions`.
...
available from `~/.espada/skills`. See [Skills: per-agent vs shared](/tools/skills#per-agent-vs-shared-skills).
...
If you do nothing, Espada runs a single agent:
...
```js
{
  ...
  channels: {
    whatsapp: {
      accounts: {
        personal: {
          // Optional override. Default: ~/.espada/credentials/whatsapp/personal
          // authDir: "~/.espada/credentials/whatsapp/personal",
        },
        biz: {
