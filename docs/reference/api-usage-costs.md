...
Espada features that can generate provider usage or paid API calls.
...
- `espada status --usage` and `espada channels list` show provider **usage windows**
...
Espada can pick up credentials from:
...
### 8) Model scan / probe
`espada models scan` can probe OpenRouter models and uses `OPENROUTER_API_KEY` when
probing is enabled.
