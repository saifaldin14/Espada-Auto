...
read_when:
  - "You installed Espada but `espada` is “command not found”"
...
Espada’s runtime baseline is **Node 22+**.

If you can run `npm install -g espada@latest` but later see `espada: command not found`, it’s almost always a **PATH** issue: the directory where npm puts global binaries isn’t on your shell’s PATH.
