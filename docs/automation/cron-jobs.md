...
- Jobs persist under `~/.espada/cron/` so restarts don’t lose schedules.
...
## Storage & history
- Job store: `~/.espada/cron/jobs.json` (Gateway-managed JSON).
- Run history: `~/.espada/cron/runs/<jobId>.jsonl` (JSONL, auto-pruned).
...
espada cron add --name "Ops sweep" --cron "0 6 * * *" --session isolated --message "Check ops queue" --agent ops

# Switch or clear the agent on an existing job
espada cron edit <jobId> --agent ops
espada cron edit <jobId> --clear-agent
```
```
...
```bash
espada system event --mode now --text "Next heartbeat: check battery."
```

## Gateway API surface
- `cron.list`, `cron.status`, `cron.add`, `cron.update`, `cron.remove`
- `cron.run` (force or due), `cron.runs`
For immediate system events without a job, use [`espada system event`](/cli/system).

## Troubleshooting

### “Nothing runs”
- Check cron is enabled: `cron.enabled` and `ESPADA_SKIP_CRON`.
