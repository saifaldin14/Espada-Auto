...
Broadcast groups are evaluated after channel allowlists and group activation rules. In WhatsApp groups, this means broadcasts happen when Espada would normally reply (for example: on mention, depending on your group settings).
...
**Result:** When Espada would reply in this chat, it will run all three agents.
...
Workspace: /Users/pascal/espada-alfred/
...
Workspace: /Users/pascal/espada-baerbel/
Tools: read only
```
...
**Debug:**
```bash
tail -f ~/.espada/logs/gateway.log | grep broadcast
```
