---
summary: "All configuration options for ~/.espada/espada.json with examples"
read_when:
  - Adding or modifying config fields
---
# Configuration 🔧

Espada reads an optional **JSON5** config from `~/.espada/espada.json` (comments + trailing commas allowed).

If the file is missing, Espada uses safe-ish defaults (embedded Pi agent + per-sender sessions + workspace `~/espada`). You usually only need a config to:
...
Espada only accepts configurations that fully match the schema.
Unknown keys, malformed types, or invalid values cause the Gateway to **refuse to start** for safety.

When validation fails:
- The Gateway does not boot.
- Only diagnostic commands are allowed (for example: `espada doctor`, `espada logs`, `espada health`, `espada status`, `espada service`, `espada help`).
- Run `espada doctor` to see the exact issues.
- Run `espada doctor --fix` (or `--yes`) to apply migrations/repairs.
...
use `config.patch` or `espada config set`. Keep a backup of `~/.espada/espada.json`.
...
```bash
espada gateway call config.get --params '{}' # capture payload.hash
espada gateway call config.apply --params '{
...
```bash
espada gateway call config.get --params '{}' # capture payload.hash
espada gateway call config.patch --params '{
...
{
  agents: {
    defaults: { workspace: "~/espada" },
    list: [
      {
        id: "main",
        groupChat: { mentionPatterns: ["@espada", "reisponde"] }
      }
    ]
  },
  channels: {
...
{ "$include": "/etc/espada/base.json5" } // absolute
...
```json5
// ~/.espada/clients/mueller/agents.json5
[
...
```json5
// ~/.espada/clients/mueller/broadcast.json5
{
...
Espada reads env vars from the parent process (shell, launchd/systemd, CI, etc.).
...
Opt-in convenience: if enabled and none of the expected keys are set yet, Espada runs your login shell and imports only the missing expected keys (never overrides).
...
Env var equivalent:
- `ESPADA_LOAD_SHELL_ENV=1`
- `ESPADA_SHELL_ENV_TIMEOUT_MS=15000`
...
Espada stores **per-agent** auth profiles (OAuth + API keys) in:
- `<agentDir>/auth-profiles.json` (default: `~/.espada/agents/<agentId>/agent/auth-profiles.json`)

See also: [/concepts/oauth](/concepts/oauth)

Legacy OAuth imports:
- `~/.espada/credentials/oauth.json` (or `$ESPADA_STATE_DIR/credentials/oauth.json`)

The embedded Pi agent maintains a runtime cache at:
- `<agentDir>/auth.json` (managed automatically; don’t edit manually)

Legacy agent dir (pre multi-agent):
- `~/.espada/agent/*` (migrated by `espada doctor` into `~/.espada/agents/<defaultAgentId>/agent/*`)

Overrides:
- OAuth dir (legacy import only): `ESPADA_OAUTH_DIR`
- Agent dir (default agent root override): `ESPADA_AGENT_DIR` (preferred), `PI_CODING_AGENT_DIR` (legacy)

On first use, Espada imports `oauth.json` entries into `auth-profiles.json`.
...
If set, Espada derives defaults (only when you haven’t set them explicitly):
...
- Default log file: `/tmp/espada/espada-YYYY-MM-DD.log`
- If you want a stable path, set `logging.file` to `/tmp/espada/espada.log`.
  ...
  channels: {
    whatsapp: {
      accounts: {
        ...
        personal: {},
        biz: {
          // Optional override. Default: ~/.espada/credentials/whatsapp/biz
          // authDir: "~/.espada/credentials/whatsapp/biz",
        }
      }
    }
  }
...
- The legacy single-account Baileys auth dir is migrated by `espada doctor` into `whatsapp/default`.
...
```json5
{
  ...
  agents: {
    list: [
      { id: "main", groupChat: { mentionPatterns: ["@espada", "espada", "espada"] } }
    ]
  }
}
```
    ...
    list: [
      {
        ...
        groupChat: {
          ...
          mentionPatterns: ["reisponde", "@espada"]
        }
      }
    ]
  }
...
- `agents.list[]`: per-agent overrides.
  ...
  - `workspace`: default `~/espada-<agentId>` (for `main`, falls back to `agents.defaults.workspace`).
  - `agentDir`: default `~/.espada/agents/<agentId>/agent`.
...
```json5
{
  agents: {
    list: [
      { id: "home", default: true, workspace: "~/espada-home" },
      { id: "work", workspace: "~/espada-work" }
    ]
  },
  bindings: [
...
- `commands.config: true` enables `/config` (reads/writes `espada.json`).
...
Espada starts Telegram only when a `channels.telegram` config section exists. The bot token is resolved from `channels.telegram.botToken` (or `channels.telegram.tokenFile`), with `TELEGRAM_BOT_TOKEN` as a fallback for the default account.
  ...
  channels: {
    discord: {
      ...
      dm: {
        ...
        groupChannels: ["espada-dm"]          // optional group DM allowlist
      },
      guilds: {
...
Espada starts Slack when the provider is enabled and both tokens are set (via config or `SLACK_BOT_TOKEN` + `SLACK_APP_TOKEN`). Use `user:<id>` (DM) or `channel:<id>` when specifying delivery targets for cron/CLI commands.
...
Install it first: `espada plugins install @espada/mattermost` (or `./extensions/mattermost` from a git checkout).
...
Espada starts Mattermost when the account is configured (bot token + base URL) and enabled. The token + base URL are resolved from `channels.mattermost.botToken` + `channels.mattermost.baseUrl` or `MATTERMOST_BOT_TOKEN` + `MATTERMOST_URL` for the default account (unless `channels.mattermost.enabled` is `false`).
...
Espada spawns `imsg rpc` (JSON-RPC over stdio). No daemon or port required.
...
Optional repository root to show in the system prompt’s Runtime line. If unset, Espada
...
```json5
{
  agents: { defaults: { repoRoot: "~/Projects/espada" } }
}
```
...
When a file exceeds this limit, Espada logs a warning and injects a truncated
...
message envelopes). If unset, Espada uses the host timezone at runtime.
...
`[espada]`, so same-phone conversations stay legible.
...
`messages.messagePrefix`). Default stays **unchanged**: `"[espada]"` when
`channels.whatsapp.allowFrom` is empty, otherwise `""` (no prefix). When using
`"[espada]"`, Espada will instead use `[{identity.name}]` when the routed
...
Enable text-to-speech for outbound replies. When on, Espada generates audio
...
```json5
{
  messages: {
    tts: {
      ...
      maxTextLength: 4000,
      timeoutMs: 30000,
      prefsPath: "~/.espada/settings/tts.json",
      elevenlabs: {
...
`voiceAliases` lets Talk directives use friendly names (e.g. `"voice":"Espada"`).
...
Espada also ships a few built-in alias shorthands. Defaults only apply when the model
...
If you omit the provider, Espada currently assumes `anthropic` as a temporary
...
- `tools.web.search.apiKey` (recommended: set via `espada configure --section web`, or use `BRAVE_API_KEY` env var)
...
- `group:nodes`: `nodes`
- `group:espada`: all built-in Espada tools (excludes provider plugins)
...
- agent workspace access: `workspaceAccess: "none"` (default)
  - `"none"`: use a per-scope sandbox workspace under `~/.espada/sandboxes`
  ...
  agents: {
    defaults: {
      sandbox: {
        ...
        workspaceRoot: "~/.espada/sandboxes",
        docker: {
          image: "espada-sandbox:bookworm-slim",
          containerPrefix: "espada-sbx-",
          workdir: "/workspace",
          readOnlyRoot: true,
          ...
          apparmorProfile: "espada-sandbox",
          dns: ["1.1.1.1", "8.8.8.8"],
        ...
        browser: {
          enabled: false,
          image: "espada-sandbox-browser:bookworm-slim",
          containerPrefix: "espada-sbx-browser-",
          cdpPort: 9222,
          vncPort: 5900,
          noVncPort: 6080,
          headless: false,
...
Espada uses the **pi-coding-agent** model catalog. You can add custom providers
(LiteLLM, local OpenAI-compatible servers, Anthropic proxies, etc.) by writing
`~/.espada/agents/<agentId>/agent/models.json` or by defining the same schema inside your
Espada config under `models.providers`.
Provider-by-provider overview + examples: [/concepts/model-providers](/concepts/model-providers).

When `models.providers` is present, Espada writes/merges a `models.json` into
`~/.espada/agents/<agentId>/agent/` on startup:
...
OpenCode Zen is a multi-model gateway with per-model endpoints. Espada uses
...
- Shortcut: `espada onboard --auth-choice opencode-zen`.
...
Shortcut: `espada onboard --auth-choice zai-api-key`.
...
Notes:
- Set `MOONSHOT_API_KEY` in the environment or use `espada onboard --auth-choice moonshot-api-key`.
...
Notes:
- Set `KIMICODE_API_KEY` in the environment or use `espada onboard --auth-choice kimi-code-api-key`.
...
Notes:
- Set `SYNTHETIC_API_KEY` or use `espada onboard --auth-choice synthetic-api-key`.
...
Notes:
- Set `MINIMAX_API_KEY` environment variable or use `espada onboard --auth-choice minimax-api`.
...
- Override the agent config root with `ESPADA_AGENT_DIR` (or `PI_CODING_AGENT_DIR`)
  if you want `models.json` stored elsewhere (default: `~/.espada/agents/main/agent`).
...
```json5
{
  session: {
    ...
    // Default is already per-agent under ~/.espada/agents/<agentId>/sessions/sessions.json
    // You can override with {agentId} templating:
    store: "~/.espada/agents/{agentId}/sessions/sessions.json",
...
- `resetByType`: per-session overrides for `dm`, `group`, and `thread`.
  - If you only set legacy `session.idleMinutes` without any `reset`/`resetByType`, Espada stays in idle-only mode for backward compatibility.
...
overrides. Applies to **bundled** skills and `~/.espada/skills` (workspace skills
...
from `~/.espada/extensions`, `<workspace>/.espada/extensions`, plus any
...
### `browser` (espada-managed browser)

Espada can start a **dedicated, isolated** Chrome/Brave/Edge/Chromium instance for espada and expose a small loopback control service.
...
- Note: the control server is started by the running gateway (Espada.app menubar, or `espada gateway`).
...
```json5
{
  browser: {
    ...
    profiles: {
      espada: { cdpPort: 18800, color: "#FF4500" },
...
```json5
{
  gateway: {
    ...
    bind: "loopback",
    // controlUi: { enabled: true, basePath: "/espada" }
...
- Examples: `"/ui"`, `"/espada"`, `"/apps/espada"`.
...
- When a connection comes from one of these IPs, Espada uses `x-forwarded-for` (or `x-real-ip`) to determine the client IP for local pairing checks and HTTP auth/local checks.
- Only list proxies you fully control, and ensure they **overwrite** incoming `x-forwarded-for`.

Notes:
- `espada gateway` refuses to start unless `gateway.mode` is set to `local` (or you pass the override flag).
- `gateway.port` controls the single multiplexed port used for WebSocket + HTTP (control UI, hooks, A2UI).
- OpenAI Chat Completions endpoint: **disabled by default**; enable with `gateway.http.endpoints.chatCompletions.enabled: true`.
- Precedence: `--port` > `ESPADA_GATEWAY_PORT` > `gateway.port` > default `18789`.
...
- `gateway.auth.password` can be set here, or via `ESPADA_GATEWAY_PASSWORD` (recommended).
- `gateway.auth.allowTailscale` allows Tailscale Serve identity headers
  (`tailscale-user-login`) to satisfy auth when the request arrives on loopback
  with `x-forwarded-for`, `x-forwarded-proto`, and `x-forwarded-host`. Espada
...
macOS app behavior:
- Espada.app watches `~/.espada/espada.json` and switches modes live when `gateway.mode` or `gateway.remote.url` changes.
...
The Gateway watches `~/.espada/espada.json` (or `ESPADA_CONFIG_PATH`) and applies changes automatically.
...
Files watched:
- `~/.espada/espada.json` (or `ESPADA_CONFIG_PATH`)
...
- `ESPADA_STATE_DIR` (sessions/creds)
- `agents.defaults.workspace` (memories)
- `gateway.port` (unique per instance)

Convenience flags (CLI):
- `espada --dev …` → uses `~/.espada-dev` + shifts ports from base `19001`
- `espada --profile <name> …` → uses `~/.espada-<name>` (port via config/env/flags)
...
Example:
```bash
ESPADA_CONFIG_PATH=~/.espada/a.json \
ESPADA_STATE_DIR=~/.espada-a \
espada gateway --port 19001
```
...
Gmail helper config (used by `espada webhooks gmail setup` / `run`):

```json5
{
  hooks: {
    gmail: {
      account: "espada@gmail.com",
...
- Set `ESPADA_SKIP_GMAIL_WATCHER=1` to disable the auto-start (for manual runs).
...
Note: when `tailscale.mode` is on, Espada defaults `serve.path` to `/` so
...
Default root: `~/espada/canvas`  
Default port: `18793` (chosen to avoid the espada browser CDP port `18792`)  
The server listens on the **gateway bind host** (LAN or Tailnet) so nodes can reach it.

The server:
- serves files under `canvasHost.root`
- injects a tiny live-reload client into served HTML
- watches the directory and broadcasts reloads over a WebSocket endpoint at `/__espada/ws`
- auto-creates a starter `index.html` when the directory is empty (so you see something immediately)
- also serves A2UI at `/__espada__/a2ui/` and is advertised to nodes as `canvasHostUrl`
...
```json5
{
  bridge: {
    ...
    tls: {
      enabled: true,
      // Uses ~/.espada/bridge/tls/bridge-{cert,key}.pem when omitted.
      // certPath: "~/.espada/bridge/tls/bridge-cert.pem",
      // keyPath: "~/.espada/bridge/tls/bridge-key.pem"
    }
  }
}
```
...
Controls LAN mDNS discovery broadcasts (`_espada-gw._tcp`).
...
When enabled, the Gateway writes a unicast DNS-SD zone for `_espada-bridge._tcp` under `~/.espada/dns/` using the standard discovery domain `espada.internal.`

To make iOS/Android discover across networks (Vienna ⇄ London), pair this with:
- a DNS server on the gateway host serving `espada.internal.` (CoreDNS is recommended)
- Tailscale **split DNS** so clients resolve `espada.internal` via that server
