...
- During heartbeat runs, Espada treats `HEARTBEAT_OK` as an ack when it appears
...
- `session`: optional session key for heartbeat runs.
  ...
  - Explicit session key (copy from `espada sessions --json` or the [sessions CLI](/cli/sessions)).
...
If **all three** are false, Espada skips the heartbeat run entirely (no model call).
...
```bash
espada system event --text "Check for urgent follow-ups" --mode now
```
