---
summary: "Quick troubleshooting guide for common Espada failures"
read_when:
  - Investigating runtime issues or failures
---
# Troubleshooting 🔧

When Espada misbehaves, here's how to fix it.
...
|---|---|---|
| `espada status` | Local summary: OS + update, gateway reachability/mode, service, agents/sessions, provider config state | First check, quick overview |
| `espada status --all` | Full local diagnosis (read-only, pasteable, safe-ish) incl. log tail | When you need to share a debug report |
| `espada status --deep` | Runs gateway health checks (incl. provider probes; requires reachable gateway) | When “configured” doesn’t mean “working” |
| `espada gateway probe` | Gateway discovery + reachability (local + remote targets) | When you suspect you’re probing the wrong gateway |
| `espada channels status --probe` | Asks the running gateway for channel status (and optionally probes) | When gateway is reachable but channels misbehave |
| `espada gateway status` | Supervisor state (launchd/systemd/schtasks), runtime PID/exit, last gateway error | When the service “looks loaded” but nothing runs |
| `espada logs --follow` | Live logs (best signal for runtime issues) | When you need the actual failure reason |

**Sharing output:** prefer `espada status --all` (it redacts tokens). If you paste `espada status`, consider setting `ESPADA_SHOW_SECRETS=0` first (token previews).
...
- Or paste a setup-token on the **gateway host**:
  ```bash
  espada models auth setup-token --provider anthropic
  ```
...
```bash
espada models auth paste-token --provider anthropic
espada models status
```
...
**Logs:**
- Preferred: `espada logs --follow`
- File logs (always): `/tmp/espada/espada-YYYY-MM-DD.log` (or your configured `logging.file`)
- macOS LaunchAgent (if installed): `$ESPADA_STATE_DIR/logs/gateway.log` and `gateway.err.log`
- Linux systemd (if installed): `journalctl --user -u espada-gateway[-<profile>].service -n 200 --no-pager`
...
- Or set it directly:
  ```bash
  espada config set gateway.mode local
  ```

**If you meant to run a remote Gateway instead:**
- Set a remote URL and keep `gateway.mode=remote`:
  ```bash
  espada config set gateway.mode remote
  espada config set gateway.remote.url "wss://gateway.example.com"
  ```

**Ad-hoc/dev only:** pass `--allow-unconfigured` to start the gateway without
`gateway.mode=local`.

**No config file yet?** Run `espada setup` to create a starter config, then rerun
the gateway.
...
variables like `DISPLAY` should live in `~/.espada/.env` (loaded early by the
gateway).
...
service was installed with Bun or a version-managed Node path, run `espada doctor`
...
- then run `espada sandbox recreate --agent <id>` (or `--all`)
...
**Check:**
- `gateway.mode` must be `local` for `espada gateway` and the service.
- If you set `gateway.mode=remote`, the **CLI defaults** to a remote URL. The service can still be running locally, but your CLI may be probing the wrong place. Use `espada gateway status` to see the service’s resolved port + probe target (or pass `--url`).
- `espada gateway status` and `espada doctor` surface the **last gateway error** from logs when the service looks running but the port is closed.
- Non-loopback binds (`lan`/`tailnet`/`custom`, or `auto` when loopback is unavailable) require auth:
  `gateway.auth.token` (or `ESPADA_GATEWAY_TOKEN`).
- `gateway.remote.token` is for remote CLI calls only; it does **not** enable local auth.
- `gateway.token` is ignored; use `gateway.auth.token`.

**If `espada gateway status` shows a config mismatch**
...
- Fix: rerun `espada gateway install --force` from the same `--profile` / `ESPADA_STATE_DIR` you want the service to use.

**If `espada gateway status` reports service config issues**
- The supervisor config (launchd/systemd/schtasks) is missing current defaults.
- Fix: run `espada doctor` to update it (or `espada gateway install --force` for a full rewrite).
...
- Fix: set `gateway.auth.mode` + `gateway.auth.token` (or export `ESPADA_GATEWAY_TOKEN`) and restart the service.

**If `espada gateway status` says `bind=tailnet` but no tailnet interface was found**
...
If you upgraded from older installs, you might still have `~/espada` on disk.
...
Symptoms: `pwd` or file tools show `~/.espada/sandboxes/...` even though you
...
Espada intentionally rejects **older/insecure models** (especially those more
...
- If you’re unsure which models are available, run `espada models list` or
  `espada models scan` and choose a supported one.
...
grep -n "agents\\|groupChat\\|mentionPatterns\\|channels\\.whatsapp\\.groups\\|channels\\.telegram\\.groups\\|channels\\.imessage\\.groups\\|channels\\.discord\\.guilds" \
  "${ESPADA_CONFIG_PATH:-$HOME/.espada/espada.json}"
```
...
tail -f "$(ls -t /tmp/espada/espada-*.log | head -1)" | grep "blocked\\|skip\\|unauthorized"
```
...
```bash
espada logs --follow | grep "pairing request"
```
...
- ❌ `@espada` + image
- ✅ `@espada check this` + image

### Session Not Resuming

**Check 1:** Is the session file there?
```bash
ls -la ~/.espada/agents/<agentId>/sessions/
```
...
espada logs --limit 200 | grep "connection\\|disconnect\\|logout"
```
...
```bash
espada gateway --verbose
```

If you’re logged out / unlinked:

```bash
espada channels logout
trash "${ESPADA_STATE_DIR:-$HOME/.espada}/credentials" # if logout can't cleanly remove everything
espada channels login --verbose       # re-scan QR
```
...
```bash
grep "media\\|fetch\\|download" "$(ls -t /tmp/espada/espada-*.log | head -1)" | tail -20
```

### High Memory Usage

Espada keeps conversation history in memory.
...
Espada now refuses to start when the config contains unknown keys, malformed values, or invalid types.
...
Notes:
- `espada doctor` reports every invalid entry.
- `espada doctor --fix` applies migrations/repairs and rewrites the config.
- Diagnostic commands like `espada logs`, `espada health`, `espada status`, `espada gateway status`, and `espada gateway probe` still run even if the config is invalid.

### “All models failed” — what should I check first?

- **Credentials** present for the provider(s) being tried (auth profiles + env vars).
- **Model routing**: confirm `agents.defaults.model.primary` and fallbacks are models you can access.
- **Gateway logs** in `/tmp/espada/…` for the exact provider error.
- **Model status**: use `/model status` (chat) or `espada models status` (CLI).
...
```bash
curl -fsSL https://espada.dev/install.sh | bash -s -- --install-method git --no-onboard
```
...
5) Run `espada channels status --probe` for audit hints.
...
endpoint accepts a strict subset of JSON Schema. Espada scrubs/normalizes tool
...
If resetting doesn't work, change the `BUNDLE_ID` in [`scripts/package-mac-app.sh`](https://github.com/espada/espada/blob/main/scripts/package-mac-app.sh) (e.g., add a `.test` suffix) and rebuild. This forces macOS to treat it as a new app.
...
espada gateway status
espada gateway stop
# Or: launchctl bootout gui/$UID/bot.molt.gateway (replace with bot.molt.<profile>; legacy com.espada.* still works)
```
...
Ensure the global `espada` CLI is installed and matches the app version:
```bash
espada --version
npm install -g espada@<version>
```
...
#   ${ESPADA_CONFIG_PATH:-$HOME/.espada/espada.json} -> { logging: { level: "trace" } }
#
# Then run verbose commands to mirror debug output to stdout:
espada gateway --verbose
espada channels login --verbose
```
...
| Log | Location |
|-----|----------|
| Gateway file logs (structured) | `/tmp/espada/espada-YYYY-MM-DD.log` (or `logging.file`) |
| Gateway service logs (supervisor) | macOS: `$ESPADA_STATE_DIR/logs/gateway.log` + `gateway.err.log` (default: `~/.espada/logs/...`; profiles use `~/.espada-<profile>/logs/...`)<br />Linux: `journalctl --user -u espada-gateway[-<profile>].service -n 200 --no-pager`<br />Windows: `schtasks /Query /TN "Espada Gateway (<profile>)" /V /FO LIST` |
| Session files | `$ESPADA_STATE_DIR/agents/<agentId>/sessions/` |
| Media cache | `$ESPADA_STATE_DIR/media/` |
| Credentials | `$ESPADA_STATE_DIR/credentials/` |
...
trash "${ESPADA_STATE_DIR:-$HOME/.espada}"
espada channels login         # re-pair WhatsApp
espada gateway restart           # or: espada gateway
```
...
1. Check logs first: `/tmp/espada/` (default: `espada-YYYY-MM-DD.log`, or your configured `logging.file`)
