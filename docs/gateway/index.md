...
- Replaces the legacy `gateway` command. CLI entry point: `espada gateway`.
...
espada gateway --force
# dev loop (auto-reload on TS changes):
pnpm gateway:watch
```
- Config hot reload watches `~/.espada/espada.json` (or `ESPADA_CONFIG_PATH`).
...
- Starts a Canvas file server by default on `canvasHost.port` (default `18793`), serving `http://<gateway-host>:18793/__espada__/canvas/` from `~/espada/canvas`. Disable with `canvasHost.enabled=false` or `ESPADA_SKIP_CANVAS_HOST=1`.
...
- Gateway auth is required by default: set `gateway.auth.token` (or `ESPADA_GATEWAY_TOKEN`) or `gateway.auth.password`. Clients must send `connect.params.auth.token/password` unless using Tailscale Serve identity.
- The wizard now generates a token by default, even on loopback.
- Port precedence: `--port` > `ESPADA_GATEWAY_PORT` > `gateway.port` > default `18789`.
...
- macOS: `bot.molt.<profile>` (legacy `com.espada.*` may still exist)
- Linux: `espada-gateway-<profile>.service`
- Windows: `Espada Gateway (<profile>)`

Install metadata is embedded in the service config:
- `ESPADA_SERVICE_MARKER=espada`
- `ESPADA_SERVICE_KIND=gateway`
- `ESPADA_SERVICE_VERSION=<version>`
...
```bash
espada --dev setup
espada --dev gateway --allow-unconfigured
...
- `ESPADA_STATE_DIR=~/.espada-dev`
- `ESPADA_CONFIG_PATH=~/.espada-dev/espada.json`
- `ESPADA_GATEWAY_PORT=19001` (Gateway WS + HTTP)
- browser control service port = `19003` (derived: `gateway.port+2`, loopback only)
- `canvasHost.port=19005` (derived: `gateway.port+4`)
- `agents.defaults.workspace` default becomes `~/espada-dev` when you run `setup`/`onboard` under `--dev`.

Derived ports (rules of thumb):
- Base port = `gateway.port` (or `ESPADA_GATEWAY_PORT` / `--port`)
- browser control service port = base + 2 (loopback only)
- `canvasHost.port = base + 4` (or `ESPADA_CANVAS_HOST_PORT` / config override)
- Browser profile CDP ports auto-allocate from `browser.controlPort + 9 .. + 108` (persisted per profile).

Checklist per instance:
- unique `gateway.port`
- unique `ESPADA_CONFIG_PATH`
- unique `ESPADA_STATE_DIR`
- unique `agents.defaults.workspace`
- separate WhatsApp numbers (if using WA)

Service install per profile:
```bash
espada --profile main gateway install
espada --profile rescue gateway install
```

Example:
```bash
ESPADA_CONFIG_PATH=~/.espada/a.json ESPADA_STATE_DIR=~/.espada-a espada gateway --port 19001
ESPADA_CONFIG_PATH=~/.espada/b.json ESPADA_STATE_DIR=~/.espada-b espada gateway --port 19002
```
...
## Methods (initial set)
- `health` — full health snapshot (same shape as `espada health --json`).
...
- Use launchd to keep the service alive:
  - Program: path to `espada`
  - Arguments: `gateway`
  - KeepAlive: true
  - StandardOut/Err: file paths or `syslog`
- On failure, launchd restarts; fatal misconfig should keep exiting so the operator notices.
- LaunchAgents are per-user and require a logged-in session; for headless setups use a custom LaunchDaemon (not shipped).
  - `espada gateway install` writes `~/Library/LaunchAgents/bot.molt.gateway.plist`
    (or `bot.molt.<profile>.plist`; legacy `com.espada.*` is cleaned up).
  - `espada doctor` audits the LaunchAgent config and can update it to current defaults.
...
- If other gateway-like services are detected, the CLI warns unless they are Espada profile services.
  ...
  - Cleanup: `espada gateway uninstall` (current service) and `espada doctor` (legacy migrations).
- `gateway install` is a no-op when already installed; use `espada gateway install --force` to reinstall (profile/env/path changes).

Bundled mac app:
- Espada.app can bundle a Node-based gateway relay and install a per-user LaunchAgent labeled
  `bot.molt.gateway` (or `bot.molt.<profile>`; legacy `com.espada.*` labels still unload cleanly).
- To stop it cleanly, use `espada gateway stop` (or `launchctl bootout gui/$UID/bot.molt.gateway`).
- To restart, use `espada gateway restart` (or `launchctl kickstart -k gui/$UID/bot.molt.gateway`).
  - `launchctl` only works if the LaunchAgent is installed; otherwise use `espada gateway install` first.
  - Replace the label with `bot.molt.<profile>` when running a named profile.

## Supervision (systemd user unit)
Espada installs a **systemd user service** by default on Linux/WSL2. We
...
`espada gateway install` writes the user unit. `espada doctor` audits the
unit and can update it to match the current recommended defaults.

Create `~/.config/systemd/user/espada-gateway[-<profile>].service`:
```
[Unit]
Description=Espada Gateway (profile: <profile>, v<version>)
After=network-online.target
Wants=network-online.target

[Service]
ExecStart=/usr/local/bin/espada gateway --port 18789
Restart=always
RestartSec=5
Environment=ESPADA_GATEWAY_TOKEN=
...
```
systemctl --user enable --now espada-gateway[-<profile>].service
```

**Alternative (system service)** - for always-on or multi-user servers, you can
install a systemd **system** unit instead of a user unit (no lingering needed).
Create `/etc/systemd/system/espada-gateway[-<profile>].service` (copy the unit above,
switch `WantedBy=multi-user.target`, set `User=` + `WorkingDirectory=`), then:
```
sudo systemctl daemon-reload
sudo systemctl enable --now espada-gateway[-<profile>].service
```
...
## CLI helpers
- `espada gateway health|status` — request health/status over the Gateway WS.
- `espada message send --target <num> --message "hi" [--media ...]` — send via Gateway (idempotent for WhatsApp).
- `espada agent --message "hi" --to <num>` — run an agent turn (waits for final by default).
- `espada gateway call <method> --params '{"k":"v"}'` — raw method invoker for debugging.
- `espada gateway stop|restart` — stop/restart the supervised gateway service (launchd/systemd).
- Gateway helper subcommands assume a running gateway on `--url`; they no longer auto-spawn one.

## Migration guidance
- Retire uses of `espada gateway` and the legacy TCP control port.
