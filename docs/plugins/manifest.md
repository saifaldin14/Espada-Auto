...
read_when:
  - You are building a Espada plugin
  - You need to ship a plugin config schema or debug plugin validation errors
---
# Plugin manifest (espada.plugin.json)

Every plugin **must** ship a `espada.plugin.json` file in the **plugin root**.
Espada uses this manifest to validate configuration **without executing plugin
