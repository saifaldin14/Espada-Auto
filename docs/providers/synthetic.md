---
summary: "Use Synthetic's Anthropic-compatible API in Espada"
read_when:
...
Synthetic exposes Anthropic-compatible endpoints. Espada registers it as the
...
```bash
espada onboard --auth-choice synthetic-api-key
```
...
Note: Espada's Anthropic client appends `/v1` to the base URL, so use
