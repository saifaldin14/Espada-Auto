---
summary: "Use Venice AI privacy-focused models in Espada"
read_when:
  - You want privacy-focused inference in Espada
...
```bash
espada onboard --auth-choice venice-api-key
```
...
```bash
espada chat --model venice/llama-3.3-70b "Hello, are you working?"
```

## Model Selection

After setup, Espada shows all available Venice models. Pick based on your needs:
...
```bash
espada models set venice/claude-opus-45
espada models set venice/llama-3.3-70b
```

List all available models:

```bash
espada models list | grep venice
```

## Configure via `espada configure`

1. Run `espada configure`
2. Select **Model/auth**
3. Choose **Venice AI**
...
Espada automatically discovers models from the Venice API when `VENICE_API_KEY` is set. If the API is unreachable, it falls back to a static catalog.
...
espada chat --model venice/llama-3.3-70b

# Use Claude via Venice (anonymized)
espada chat --model venice/claude-opus-45

# Use uncensored model
espada chat --model venice/venice-uncensored

# Use vision model with image
espada chat --model venice/qwen3-vl-235b-a22b

# Use coding model
espada chat --model venice/qwen3-coder-480b-a35b-instruct
```
...
The Venice model catalog updates dynamically. Run `espada models list` to see currently available models. Some models may be temporarily offline.
