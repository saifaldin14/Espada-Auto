---
summary: "Model providers (LLMs) supported by Espada"
read_when:
...
Espada can use many LLM providers. Pick one, authenticate, then set the default
...
1) Authenticate with the provider (usually via `espada onboard`).
