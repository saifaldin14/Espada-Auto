...
Remote Gateway:
```bash
espada tui --url ws://<host>:<port> --token <gateway-token>
```
...
- Turn delivery on:
  ...
  - or start with `espada tui --deliver`
...
- Check the Gateway logs: `espada logs --follow`.
- Confirm the agent can run: `espada status` and `espada models status`.
- If you expect messages in a chat channel, enable delivery (`/deliver on` or `--deliver`).
- `--history-limit <n>`: History entries to load (default 200)

## Troubleshooting
- `disconnected`: ensure the Gateway is running and your `--url/--token/--password` are correct.
- No agents in picker: check `espada agents list` and your routing config.
