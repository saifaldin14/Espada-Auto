---
summary: "Top-level overview of Espada, features, and purpose"
read_when:
  - Introducing Espada to newcomers
---
# Espada 🦞
...
- **Guided setup (recommended):** [Wizard](/start/wizard) (`espada onboard`)
- **Open the dashboard (local Gateway):** http://127.0.0.1:18789/ (or http://localhost:18789/)

If the Gateway is running on the same computer, that link opens the browser Control UI
immediately. If it fails, start the Gateway first: `espada gateway`.
...
WhatsApp / Telegram / Discord / iMessage (+ plugins)
  ...
  │                           │    /__espada__/canvas/ (Canvas host)
  └───────────┬───────────────┘
              │
              ├─ Pi agent (RPC)
              ├─ CLI (espada …)
              ├─ Chat UI (SwiftUI)
              ├─ macOS app (Espada.app)
              ├─ iOS node via Gateway WS + pairing
              └─ Android node via Gateway WS + pairing
```

Most operations flow through the **Gateway** (`espada gateway`), a single long-running process that owns channel connections and the WebSocket control plane.
...
- **Loopback-first**: Gateway WS defaults to `ws://127.0.0.1:18789`.
  ...
  - For Tailnet access, run `espada gateway --bind tailnet --token ...` (token is required for non-loopback binds).
- **Nodes**: connect to the Gateway WebSocket (LAN/tailnet/SSH as needed); legacy TCP bridge is deprecated/removed.
- **Canvas host**: HTTP file server on `canvasHost.port` (default `18793`), serving `/__espada__/canvas/` for node WebViews; see [Gateway configuration](/gateway/configuration) (`canvasHost`).
...
Switching between npm and git installs later is easy: install the other flavor and run `espada doctor` to update the gateway service entrypoint.
...
If you don't have a global install yet, run the onboarding step via `pnpm espada ...` from the repo.
...
```bash
espada message send --target +15555550123 --message "Hello from Espada"
```

## Configuration (optional)

Config lives at `~/.espada/espada.json`.

- If you **do nothing**, Espada uses the bundled Pi binary in RPC mode with per-sender sessions.
...
**Espada = CLAW + TARDIS** — because every space lobster needs a time-and-space machine.
