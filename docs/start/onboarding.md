---
summary: "First-run onboarding flow for Espada (macOS app)"
read_when:
...
- Writes credentials to `~/.espada/credentials/oauth.json`
...
The app can install the global `espada` CLI via npm/pnpm so terminal
...
On the first agent run, Espada bootstraps a workspace (default `~/espada`):
...
```bash
espada webhooks gmail setup --account you@gmail.com
```
