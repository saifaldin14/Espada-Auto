...
Fastest chat: open the Control UI (no channel setup needed). Run `espada dashboard`
...
- **Recommended:** Brave Search API key for web search. Easiest path:
  `espada configure --section web` (stores `tools.web.search.apiKey`).
...
- OAuth credentials (legacy import): `~/.espada/credentials/oauth.json`
...
If you’re hacking on Espada itself, run from source:
...
```bash
node espada.mjs gateway --port 18789 --verbose
```
...
If `espada health` shows “no auth configured”, go back to the wizard and set OAuth/key auth — the agent won’t be able to respond without it.

Tip: `espada status --all` is the best pasteable, read-only debug report.
Health probes: `espada health` (or `espada status --deep`) asks the running gateway for a health snapshot.
