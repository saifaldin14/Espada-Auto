---
summary: "Research notes: offline memory system for Espada workspaces (Markdown source-of-truth + derived index)"
read_when:
  - Designing workspace memory (~/espada) beyond daily Markdown logs
  - Deciding: standalone CLI vs deep Espada integration
  - Adding offline recall + reflection (retain/recall/reflect)
---

# Workspace Memory v2 (offline): research notes

Target: Espada-style workspace (`agents.defaults.workspace`, default `~/espada`) where “memory” is stored as one Markdown file per day (`memory/YYYY-MM-DD.md`) plus a small set of stable files (e.g. `memory.md`, `SOUL.md`).
...
Keep `~/espada` as canonical human-readable memory.
...
- `memory.md` remains “small + core-ish”: the things you want Espada to see every session.
...
Recommendation: **deep integration in Espada**, but keep a separable core library.

### Why integrate into Espada?
- Espada already knows:
  - the workspace path (`agents.defaults.workspace`)
  - the session model + heartbeats
  - logging + troubleshooting patterns
- You want the agent itself to call the tools:
  - `espada memory recall "…" --k 25 --since 30d`
  - `espada memory reflect --since 7d`
