...
You can also set `ESPADA_VERBOSE=1` instead of the flag.
...
### `docs.espada.dev` shows an SSL error (Comcast/Xfinity)

Some Comcast/Xfinity connections block `docs.espada.dev` via Xfinity Advanced Security.
Disable Advanced Security or add `docs.espada.dev` to the allowlist, then retry.
...
- Check the allowlist: `espada config get agents.defaults.models`
...
If you can, include the relevant log tail from `espada logs --follow`.
