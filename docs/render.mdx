...
Deploy Espada on Render using Infrastructure as Code. The included `render.yaml` Blueprint defines your entire stack declaratively, service, disk, environment variables, so you can deploy with a single click and version your infrastructure alongside your code.
...
<a href="https://render.com/deploy?repo=https://github.com/espada/espada" target="_blank" rel="noreferrer">Deploy to Render</a>
...
repository configures everything needed to run Espada:
...
services:
  - type: web
    ...
    envVars:
      ...
      - key: ESPADA_STATE_DIR
        value: /data/.espada
      - key: ESPADA_WORKSPACE_DIR
        value: /data/workspace
      - key: ESPADA_GATEWAY_TOKEN
...
The web dashboard is available at `https://<your-service>.onrender.com/espada`.
...
If you use the original Espada repository, Render will not auto-deploy your Espada. To update it, run a manual Blueprint sync from the dashboard.
...
For Espada, vertical scaling is usually sufficient. Horizontal scaling requires sticky sessions or external state management.
...
This downloads a portable backup you can restore on any Espada host.
