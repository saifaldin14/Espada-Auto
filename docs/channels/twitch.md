...
read_when:
  - Setting up Twitch chat integration for Espada
---
# Twitch (plugin)

Twitch chat support via IRC connection. Espada connects as a Twitch user (bot account) to receive and send messages in channels.
...
```bash
espada plugins install @espada/twitch
```

Local checkout (when running from a git repo):

```bash
espada plugins install ./extensions/twitch
```
...
4) Configure the token:
   - Env: `ESPADA_TWITCH_ACCESS_TOKEN=...` (default account only)
...
```json5
{
  channels: {
    twitch: {
      enabled: true,
      username: "espada",              // Bot's Twitch account
      accessToken: "oauth:abc123...",    // OAuth Access Token (or use ESPADA_TWITCH_ACCESS_TOKEN env var)
...
```bash
ESPADA_TWITCH_ACCESS_TOKEN=oauth:abc123...
```
