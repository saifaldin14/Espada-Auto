...
Matrix is an open, decentralized messaging protocol. Espada connects as a Matrix **user**
...
```bash
espada plugins install @espada/matrix
```

Local checkout (when running from a git repo):

```bash
espada plugins install ./extensions/matrix
```
...
1) Install the Matrix plugin:
   - From npm: `espada plugins install @espada/matrix`
   - From a local checkout: `espada plugins install ./extensions/matrix`
...
3) Get an access token for the bot account:
   ...
   - Or set `channels.matrix.userId` + `channels.matrix.password`: Espada calls the same
     login endpoint, stores the access token in `~/.espada/credentials/matrix/credentials.json`,
...
- On first connection, Espada requests device verification from your other sessions.
- Verify the device in another Matrix client (Element, etc.) to enable key sharing.
- If the crypto module cannot be loaded, E2EE is disabled and encrypted rooms will not decrypt;
  Espada logs a warning.
...
`~/.espada/matrix/accounts/<account>/<homeserver>__<user>/<token-hash>/crypto/`
...
- Approve via:
  - `espada pairing list matrix`
  - `espada pairing approve matrix <CODE>`
...
- On startup, Espada resolves room/user names in allowlists to IDs and logs the mapping; unresolved entries are kept as typed.
