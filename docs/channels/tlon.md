...
Tlon is a decentralized messenger built on Urbit. Espada connects to your Urbit ship and can
...
```bash
espada plugins install @espada/tlon
```

Local checkout (when running from a git repo):

```bash
espada plugins install ./extensions/tlon
```
...
Use these with `espada message send` or cron delivery:
...
- Thread replies: if the inbound message is in a thread, Espada replies in-thread.
