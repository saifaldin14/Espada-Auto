...
```bash
espada plugins install @espada/nextcloud-talk
```

Local checkout (when running from a git repo):
```bash
espada plugins install ./extensions/nextcloud-talk
```
...
2) On your Nextcloud server, create a bot:
   ```bash
   ./occ talk:bot:install "Espada" "<shared-secret>" "<webhook-url>" --feature reaction
   ```
...
- Approve via:
  - `espada pairing list nextcloud-talk`
  - `espada pairing approve nextcloud-talk <CODE>`
