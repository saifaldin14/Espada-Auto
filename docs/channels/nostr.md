...
read_when:
  - You want Espada to receive DMs via Nostr
...
Nostr is a decentralized protocol for social networking. This channel enables Espada to receive and respond to encrypted direct messages (DMs) via NIP-04.

## Install (on demand)

### Onboarding (recommended)

- The onboarding wizard (`espada onboard`) and `espada channels add` list optional channel plugins.
...
```bash
espada plugins install @espada/nostr
```

Use a local checkout (dev workflows):

```bash
espada plugins install --link <path-to-espada>/extensions/nostr
```
  ...
  "channels": {
    "nostr": {
      ...
      "profile": {
        ...
        "nip05": "espada@example.com",
        "lud16": "espada@example.com"
      }
    }
  }
}
