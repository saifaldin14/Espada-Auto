...
let recRand = Math.random().toString(36).slice(2, 6);

function recId(): string {
  return `rec-${recEpoch}-${recRand}-${++recCounter}`;
}

/**
 * Reset the counter (for testing).
 * Uses a new epoch timestamp and random suffix to avoid ID collisions with
 * recommendations generated before the reset.
 */
export function resetRecommendationCounter(): void {
  recCounter = 0;
  recEpoch = Date.now();
  recRand = Math.random().toString(36).slice(2, 6);
}
