...
import type { OpaEngine, OpaEvaluationResult, OpaInput } from "./opa-engine.js";
...
export type GovernorConfig = {
  ...
  /** Optional external OPA/Rego policy engine for advanced policy evaluation. */
  opaEngine?: OpaEngine;
  /** Whether OPA evaluation failures should block (fail-closed) or allow (fail-open). */
  opaFailMode?: "open" | "closed";
};
...
export class ChangeGovernor {
  ...
  async interceptChange(params: {
    ...
    // Run OPA/Rego external policy evaluation (if configured)
    let opaResult: OpaEvaluationResult | null = null;
    if (this.config.opaEngine) {
      // Build a preliminary change request for OPA input (before final status)
      ...
      opaResult = await this.config.opaEngine.evaluate(opaInput);

      if (opaResult.ok && opaResult.violations.length > 0) {
        // Merge OPA violations into the policy violations list
        for (const v of opaResult.violations) {
          policyViolations.push(`[OPA/${v.ruleId}] ${v.message} (${v.severity})`);
        }
      } else if (!opaResult.ok && this.config.opaFailMode === "closed") {
        // OPA failed and we're in fail-closed mode — treat as violation
        policyViolations.push(
          `[OPA/error] OPA evaluation failed (fail-closed): ${opaResult.error ?? "unknown error"}`,
        );
      }
    }
    ...
    const request: ChangeRequest = {
      ...
      metadata: {
        ...
        ...(opaResult ? {
          opaEvaluated: true,
          opaOk: opaResult.ok,
          opaDurationMs: opaResult.durationMs,
          opaViolationCount: opaResult.violations.length,
          opaError: opaResult.error ?? null,
        } : {}),
      },
    };
  ...
  getSummary(since?: string, until?: string): GovernanceSummary {
    ...
    for (const req of requests) {
      ...
      if (req.metadata?.opaEvaluated && (req.metadata.opaViolationCount as number) > 0) {
        opaViolationCount++;
      }
    }
