...
metadata: {"espada":{"emoji":"🪶","homepage":"https://www.prose.md"}}
---
...
- **Task tool** in the upstream spec == Espada `sessions_spawn`
- **File I/O** == Espada `read`/`write`
- **Remote fetch** == Espada `web_fetch` (or `exec` with curl when POST is required)
