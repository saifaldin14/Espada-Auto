/**
 * Twitch channel plugin for Espada.
 *
...
import { buildChannelConfigSchema } from "espada/plugin-sdk";
 ...
 * for Espada. Supports message sending, receiving, access control, and
 * status monitoring.
 */
export const twitchPlugin: ChannelPlugin<TwitchAccountConfig> = {
  ...
  config: {
    ...
    listAccountIds: (cfg: EspadaConfig): string[] => listAccountIds(cfg),

    /** Resolve an account config by ID */
    resolveAccount: (cfg: EspadaConfig, accountId?: string | null): TwitchAccountConfig => {
    ...
    isConfigured: (_account: unknown, cfg: EspadaConfig): boolean => {
