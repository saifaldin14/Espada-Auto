...
import type { PluginRuntime } from "espada/plugin-sdk";
...
async function withTempStateDir<T>(fn: (dir: string) => Promise<T>) {
  const previous = process.env.ESPADA_STATE_DIR;
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "espada-nostr-"));
  process.env.ESPADA_STATE_DIR = dir;
  setNostrRuntime({
    state: {
      resolveStateDir: (env, homedir) => {
        const override = env.ESPADA_STATE_DIR?.trim();
        if (override) return override;
        return path.join(homedir(), ".espada");
      },
    },
  } as PluginRuntime);
  try {
    ...
    if (previous === undefined) delete process.env.ESPADA_STATE_DIR;
    else process.env.ESPADA_STATE_DIR = previous;
