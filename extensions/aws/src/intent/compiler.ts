...
export class IntentCompiler {
  /** SG IDs generated by compileSecurityInfrastructure, shared across phases */
  private sgIds: Record<string, string> = {};
  ...
  private async compileSecurityInfrastructure(
    ...
    // Generate stable IDs up front so cross-references work
    const sgWebId = `sg-web-${randomUUID().slice(0, 8)}`;
    const sgAppId = `sg-app-${randomUUID().slice(0, 8)}`;
    const sgDbId = `sg-db-${randomUUID().slice(0, 8)}`;

    // Store on instance so other compilation phases can reference them
    this.sgIds = { web: sgWebId, app: sgAppId, db: sgDbId };
    ...
    if (intent.tiers.some(t => t.type === 'api')) {
      resources.push({
        ...
        properties: {
          ...
          ingressRules: [
            { protocol: 'tcp', port: 8080, sourceSecurityGroup: sgWebId, description: 'From web tier' },
          ],
          egressRules: [
            { protocol: '-1', port: -1, cidr: '0.0.0.0/0', description: 'All outbound' },
          ],
        },
        dependencies: [sgWebId],
        estimatedCostUsd: 0,
        region,
    ...
    if (intent.tiers.some(t => t.type === 'database')) {
      resources.push({
        ...
        properties: {
          ...
          ingressRules: [
            { protocol: 'tcp', port: 5432, sourceSecurityGroup: sgAppId, description: 'PostgreSQL from app' },
          ],
          egressRules: [],
        },
        dependencies: [sgAppId],
        estimatedCostUsd: 0,
        region,
  ...
  private async compileDataLayer(
    ...
    for (const tier of intent.tiers) {
      ...
      } else if (tier.type === 'analytics') {
        const analyticsResources = await this.compileAnalyticsTier(tier, intent);
        resources.push(...analyticsResources);
      }
    }
  ...
  /**
   * Compile analytics tier (CloudWatch dashboard + Lambda for ETL + S3 data lake)
   */
  private async compileAnalyticsTier(
    tier: ApplicationTierIntent,
    intent: ApplicationIntent,
  ): Promise<PlannedResource[]> {
    const resources: PlannedResource[] = [];
    ...
    const dataLakeId = `s3-analytics-${randomUUID().slice(0, 8)}`;
    resources.push({
      id: dataLakeId,
      type: 's3-bucket',
      service: 's3',
      properties: {
        bucketName: `${intent.name}-analytics-${Date.now()}`,
        versioning: true,
        encryption: intent.security.encryptionAtRest ? 'AES256' : 'none',
        lifecycleRules: [
          {
    ...
    const etlLambdaId = `lambda-etl-${randomUUID().slice(0, 8)}`;
    resources.push({
      id: etlLambdaId,
      type: 'lambda-function',
      service: 'lambda',
      properties: {
        functionName: `${intent.name}-etl-processor`,
        runtime: 'nodejs20.x',
        handler: 'index.handler',
        memorySize: 512,
        timeout: 300, // 5 minutes for batch processing
        environment: {
          DATA_BUCKET: dataLakeId, // Resolved at execution time
          ENVIRONMENT: intent.environment,
        },
      },
    ...
    // SQS queue for analytics event ingestion
    resources.push({
      id: `sqs-analytics-${randomUUID().slice(0, 8)}`,
      type: 'sqs-queue',
      service: 'sqs',
      properties: {
        queueName: `${intent.name}-analytics-events`,
        messageRetentionPeriod: 1209600, // 14 days for analytics replay
      ...
      region,
      tags: { ...intent.tags, Tier: 'analytics', Role: 'ingestion' },
    });

    // CloudWatch dashboard for analytics metrics
    resources.push({
      id: `cw-dashboard-${randomUUID().slice(0, 8)}`,
      type: 'cloudwatch-dashboard',
      service: 'cloudwatch',
      properties: {
        dashboardName: `${intent.name}-analytics`,
        dashboardBody: {
          widgets: [
            {
              type: 'metric',
              properties: {
                ...
                metrics: [
                  ['AWS/Lambda', 'Invocations', 'FunctionName', `${intent.name}-etl-processor`],
                  ['AWS/Lambda', 'Errors', 'FunctionName', `${intent.name}-etl-processor`],
                  ['AWS/Lambda', 'Duration', 'FunctionName', `${intent.name}-etl-processor`],
                ],
                period: 300,
                stat: 'Sum',
              },
            },
            {
              ...
              properties: {
                ...
                metrics: [
                  ['AWS/SQS', 'NumberOfMessagesSent', 'QueueName', `${intent.name}-analytics-events`],
                  ['AWS/SQS', 'ApproximateNumberOfMessagesVisible', 'QueueName', `${intent.name}-analytics-events`],
                ],
                period: 300,
                stat: 'Sum',
              },
            },
            {
              ...
              properties: {
                ...
                metrics: [
                  ['AWS/S3', 'BucketSizeBytes', 'BucketName', `${intent.name}-analytics-${Date.now()}`, 'StorageType', 'StandardStorage'],
                ],
                period: 86400,
                stat: 'Average',
              },
      ...
      dependencies: [etlLambdaId],
      estimatedCostUsd: 3.0, // $3/month per dashboard
      region,
      tags: { ...intent.tags, Tier: 'analytics', Role: 'monitoring' },
    });
   ...
   *
   * When automaticFailover is enabled, creates real multi-region failover
   * infrastructure: cross-region VPCs, RDS read replicas, ALBs, Route 53
   * health checks, and failover DNS records. Otherwise, only backup plans.
   */
  private async compileDisasterRecovery(
    ...
    // --- Always create AWS Backup plan ---
    const backupPlanId = `backup-plan-${randomUUID().slice(0, 8)}`;
    resources.push({
      id: backupPlanId,
      type: 'backup-plan',
      service: 'backup',
  ...
  /**
   * Generate full failover infrastructure across additional regions:
  ...
  private async compileAutomaticFailover(
    ...
    // Health check interval: aggressive for tight RTO, relaxed otherwise
    const healthCheckIntervalSeconds = rtoMinutes <= 5 ? 10 : 30;

    // Primary region ALB health check (always needed for failover DNS)
    const primaryHealthCheckId = `r53-hc-primary-${randomUUID().slice(0, 8)}`;
    if (hasWebOrApiTier) {
      resources.push({
        id: primaryHealthCheckId,
        type: 'route53-health-check',
        service: 'route53',
        properties: {
          type: 'HTTPS',
          fullyQualifiedDomainName: `${intent.name}-alb.${intent.primaryRegion}.elb.amazonaws.com`,
          port: 443,
          resourcePath: '/health',
        ...
        dependencies: [],
        estimatedCostUsd: 1.0, // ~$0.75/health check/month
        region: intent.primaryRegion,
        tags: { ...intent.tags, Role: 'dr-health-check', FailoverRole: 'PRIMARY' },
      });
    }

    for (const drRegion of intent.additionalRegions!) {
      // Cross-region VPC
      const drVpcId = `vpc-dr-${drRegion}-${randomUUID().slice(0, 8)}`;
      resources.push({
        id: drVpcId,
        type: 'vpc',
        service: 'ec2',
        properties: {
          cidrBlock: '10.1.0.0/16', // Different CIDR from primary
          name: `${intent.name}-dr-${drRegion}`,
          enableDnsHostnames: true,
          enableDnsSupport: true,
        },
        dependencies: [],
        estimatedCostUsd: 0,
        region: drRegion,
        tags: { ...intent.tags, Role: 'dr-vpc', FailoverRegion: drRegion },
      });

      // DR subnets (at least 2 AZs for ALB)
      const drSubnetIds: string[] = [];
      for (let i = 0; i < 2; i++) {
        const subnetId = `subnet-dr-${drRegion}-${i}-${randomUUID().slice(0, 8)}`;
        drSubnetIds.push(subnetId);
        resources.push({
          id: subnetId,
          type: 'subnet',
          service: 'ec2',
          properties: {
            vpcId: drVpcId,
            cidrBlock: `10.1.${i * 16}.0/20`,
            availabilityZone: `${drRegion}${String.fromCharCode(97 + i)}`, // e.g. us-west-2a
            name: `${intent.name}-dr-${drRegion}-${i}`,
            mapPublicIpOnLaunch: false,
          },
          dependencies: [drVpcId],
          estimatedCostUsd: 0,
          region: drRegion,
          tags: { ...intent.tags, Role: 'dr-subnet' },
        });
      }

      // DR security group (mirrors primary)
      const drSgId = `sg-dr-${drRegion}-${randomUUID().slice(0, 8)}`;
      resources.push({
        id: drSgId,
        type: 'security-group',
        service: 'ec2',
        properties: {
          name: `${intent.name}-dr-sg-${drRegion}`,
          description: `DR security group in ${drRegion}`,
          vpcId: drVpcId,
          ingressRules: [
            { protocol: 'tcp', port: 443, cidr: '0.0.0.0/0', description: 'HTTPS' },
            { protocol: 'tcp', port: 80, cidr: '0.0.0.0/0', description: 'HTTP' },
        ...
        estimatedCostUsd: 0,
        region: drRegion,
        tags: { ...intent.tags, Role: 'dr-security-group' },
      });

      // Cross-region RDS read replica (standby that can be promoted)
      if (hasDatabaseTier) {
        resources.push({
          id: `rds-replica-${drRegion}-${randomUUID().slice(0, 8)}`,
          type: 'rds-read-replica',
          service: 'rds',
          properties: {
            sourceDBInstanceIdentifier: `${intent.name}-db`, // References primary RDS
            instanceClass: 'db.t3.medium', // DR can use smaller instance
      ...
      // Cross-region ALB (receives traffic when primary fails)
      let drAlbId: string | undefined;
      if (hasWebOrApiTier) {
        drAlbId = `alb-dr-${drRegion}-${randomUUID().slice(0, 8)}`;
        resources.push({
          id: drAlbId,
          type: 'alb',
          service: 'elbv2',
          properties: {
            ...
            scheme: 'internet-facing',
            subnets: drSubnetIds,
            securityGroups: [drSgId],
            idleTimeout: 60,
          },
          dependencies: [...drSubnetIds, drSgId],
          estimatedCostUsd: 22.0, // ~$16 ALB + LCU costs
          region: drRegion,
          tags: { ...intent.tags, Role: 'dr-load-balancer', FailoverRegion: drRegion },
        });

        // DR target group
        resources.push({
          id: `tg-dr-${drRegion}-${randomUUID().slice(0, 8)}`,
          type: 'target-group',
          service: 'elbv2',
          properties: {
            name: `${intent.name}-dr-tg-${drRegion}`,
            protocol: 'HTTPS',
            port: 443,
            vpcId: drVpcId,
            targetType: 'ip',
          ...
          estimatedCostUsd: 0,
          region: drRegion,
          tags: { ...intent.tags, Role: 'dr-target-group' },
        });

        // Route 53 health check for DR ALB
        const drHealthCheckId = `r53-hc-${drRegion}-${randomUUID().slice(0, 8)}`;
        resources.push({
          id: drHealthCheckId,
          type: 'route53-health-check',
          service: 'route53',
          properties: {
            type: 'HTTPS',
            fullyQualifiedDomainName: `${intent.name}-dr-${drRegion}.${drRegion}.elb.amazonaws.com`,
            port: 443,
            resourcePath: '/health',
            requestInterval: healthCheckIntervalSeconds,
            failureThreshold: 3,
          },
          dependencies: [drAlbId],
          estimatedCostUsd: 1.0,
          region: drRegion,
          tags: { ...intent.tags, Role: 'dr-health-check', FailoverRole: 'SECONDARY' },
        });

        // Route 53 failover record — PRIMARY
        resources.push({
          id: `r53-failover-primary-${randomUUID().slice(0, 8)}`,
          type: 'route53-failover-record',
          service: 'route53',
          properties: {
            name: `${intent.name}.example.com`, // Placeholder; real hosted zone configured externally
            type: 'A',
            aliasTarget: `${intent.name}-alb.${intent.primaryRegion}.elb.amazonaws.com`,
            failover: 'PRIMARY',
            setIdentifier: `${intent.name}-primary`,
            healthCheckId: primaryHealthCheckId,
          },
          dependencies: [primaryHealthCheckId],
          estimatedCostUsd: 0.50,
          region: intent.primaryRegion,
          tags: { ...intent.tags, Role: 'dr-failover-dns', FailoverRole: 'PRIMARY' },
        });

        // Route 53 failover record — SECONDARY
        resources.push({
          id: `r53-failover-${drRegion}-${randomUUID().slice(0, 8)}`,
          type: 'route53-failover-record',
          service: 'route53',
          properties: {
            name: `${intent.name}.example.com`,
            type: 'A',
            aliasTarget: `${intent.name}-dr-${drRegion}.${drRegion}.elb.amazonaws.com`,
            failover: 'SECONDARY',
            setIdentifier: `${intent.name}-dr-${drRegion}`,
            healthCheckId: drHealthCheckId,
          },
          dependencies: [drHealthCheckId, drAlbId],
          estimatedCostUsd: 0.50,
          region: drRegion,
          tags: { ...intent.tags, Role: 'dr-failover-dns', FailoverRole: 'SECONDARY' },
          rationale: `Failover DNS record for DR region ${drRegion}; Route 53 promotes this when primary health check fails`,
        });
      }
    }
