...
Official Voice Call plugin for **Espada**.
...
### Option A: install via Espada (recommended)
...
```bash
mkdir -p ~/.espada/extensions
cp -R extensions/voice-call ~/.espada/extensions/voice-call
cd ~/.espada/extensions/voice-call && pnpm install
```
