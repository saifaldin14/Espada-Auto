...
function resolveEspadaRoot(): string {
  if (coreRootCache) return coreRootCache;
  const override = process.env.ESPADA_ROOT?.trim() || process.env.ESPADA_ROOT?.trim();
  if (override) {
  ...
  for (const start of candidates) {
    for (const name of ["espada", "espada"]) {
  ...
  throw new Error(
    "Unable to resolve core root. Set ESPADA_ROOT (or legacy ESPADA_ROOT) to the package root.",
  );
}

async function importCoreModule<T>(relativePath: string): Promise<T> {
  const root = resolveEspadaRoot();
