...
export function extractMSTeamsPollVote(
  ...
  const pollId =
    readNestedString(value, ["espadaPollId"]) ??
    readNestedString(value, ["pollId"]) ??
    readNestedString(value, ["espada", "pollId"]) ??
    readNestedString(value, ["espada", "poll", "id"]) ??
    readNestedString(value, ["data", "espadaPollId"]) ??
    readNestedString(value, ["data", "pollId"]) ??
    readNestedString(value, ["data", "espada", "pollId"]);
    ...
    actions: [
      {
        ...
        msteams: {
          type: "messageBack",
          text: "espada poll vote",
          displayText: "Vote recorded",
          value: { espadaPollId: pollId },
        },
      },
    ],
  };
