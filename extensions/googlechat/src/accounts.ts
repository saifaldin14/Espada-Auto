...
export function listGoogleChatAccountIds(cfg: EspadaConfig): string[] {
  const ids = listConfiguredAccountIds(cfg);
  if (ids.length === 0) return [DEFAULT_ACCOUNT_ID];
  return ids.sort((a, b) => a.localeCompare(b));
}

export function resolveDefaultGoogleChatAccountId(cfg: EspadaConfig): string {
...
export function listEnabledGoogleChatAccounts(cfg: EspadaConfig): ResolvedGoogleChatAccount[] {
