...
import { GoogleChatConfigSchema } from "espada/plugin-sdk";
...
export const googlechatDock: ChannelDock = {
  ...
  config: {
    resolveAllowFrom: ({ cfg, accountId }) =>
      (resolveGoogleChatAccount({ cfg: cfg as EspadaConfig, accountId }).config.dm?.allowFrom ??
        []
...
export const googlechatPlugin: ChannelPlugin<ResolvedGoogleChatAccount> = {
  ...
  pairing: {
    ...
    notifyApproval: async ({ cfg, id }) => {
      const account = resolveGoogleChatAccount({ cfg: cfg as EspadaConfig });
  ...
  config: {
    listAccountIds: (cfg) => listGoogleChatAccountIds(cfg as EspadaConfig),
    resolveAccount: (cfg, accountId) =>
      resolveGoogleChatAccount({ cfg: cfg as EspadaConfig, accountId }),
    defaultAccountId: (cfg) => resolveDefaultGoogleChatAccountId(cfg as EspadaConfig),
  ...
  security: {
    resolveDmPolicy: ({ cfg, accountId, account }) => {
      ...
      const useAccountPath = Boolean(
        (cfg as EspadaConfig).channels?.["googlechat"]?.accounts?.[resolvedAccountId],
      );
    ...
    applyAccountConfig: ({ cfg, accountId, input }) => {
      ...
      const next =
        accountId !== DEFAULT_ACCOUNT_ID
          ? migrateBaseNameToDefaultAccount({
              cfg: namedConfig as EspadaConfig,
              channelKey: "googlechat",
            })
          : namedConfig;
