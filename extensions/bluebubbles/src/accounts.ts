...
export function listBlueBubblesAccountIds(cfg: EspadaConfig): string[] {
  const ids = listConfiguredAccountIds(cfg);
  if (ids.length === 0) return [DEFAULT_ACCOUNT_ID];
  return ids.sort((a, b) => a.localeCompare(b));
}

export function resolveDefaultBlueBubblesAccountId(cfg: EspadaConfig): string {
...
export function listEnabledBlueBubblesAccounts(cfg: EspadaConfig): ResolvedBlueBubblesAccount[] {
