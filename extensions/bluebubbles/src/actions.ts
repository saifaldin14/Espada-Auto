...
export const bluebubblesMessageActions: ChannelMessageActionAdapter = {
  listActions: ({ cfg }) => {
    const account = resolveBlueBubblesAccount({ cfg: cfg as EspadaConfig });
    if (!account.enabled || !account.configured) return [];
    const gate = createActionGate((cfg as EspadaConfig).channels?.bluebubbles?.actions);
  ...
  handleAction: async ({ action, params, cfg, accountId, toolContext }) => {
    ...
    const opts = { cfg: cfg as EspadaConfig, accountId: accountId ?? undefined };
