import type { ChannelAccountSnapshot, ChannelPlugin, EspadaConfig } from "espada/plugin-sdk";
import {
...
export const bluebubblesPlugin: ChannelPlugin<ResolvedBlueBubblesAccount> = {
  ...
  config: {
    listAccountIds: (cfg) => listBlueBubblesAccountIds(cfg as EspadaConfig),
    resolveAccount: (cfg, accountId) =>
      resolveBlueBubblesAccount({ cfg: cfg as EspadaConfig, accountId }),
    defaultAccountId: (cfg) => resolveDefaultBlueBubblesAccountId(cfg as EspadaConfig),
    ...
    resolveAllowFrom: ({ cfg, accountId }) =>
      (resolveBlueBubblesAccount({ cfg: cfg as EspadaConfig, accountId }).config.allowFrom ??
        []).map(
        (entry) => String(entry),
      ),
  ...
  security: {
    resolveDmPolicy: ({ cfg, accountId, account }) => {
      ...
      const useAccountPath = Boolean(
        (cfg as EspadaConfig).channels?.bluebubbles?.accounts?.[resolvedAccountId],
      );
