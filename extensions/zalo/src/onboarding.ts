...
async function noteZaloTokenHelp(prompter: WizardPrompter): Promise<void> {
  await prompter.note(
    [
      ...
      "Docs: https://docs.espada.dev/channels/zalo",
    ].join("\n"),
    "Zalo bot token",
  );
}
...
const dmPolicy: ChannelOnboardingDmPolicy = {
  ...
  setPolicy: (cfg, policy) => setZaloDmPolicy(cfg as EspadaConfig, policy),
  promptAllowFrom: async ({ cfg, prompter, accountId }) => {
    const id =
      accountId && normalizeAccountId(accountId)
        ? normalizeAccountId(accountId) ?? DEFAULT_ACCOUNT_ID
        : resolveDefaultZaloAccountId(cfg as EspadaConfig);
...
export const zaloOnboardingAdapter: ChannelOnboardingAdapter = {
  ...
  dmPolicy,
  getStatus: async ({ cfg }) => {
    const configured = listZaloAccountIds(cfg as EspadaConfig).some((accountId) =>
      Boolean(resolveZaloAccount({ cfg: cfg as EspadaConfig, accountId }).token),
    );
    return {
      channel,
      configured,
  ...
  configure: async ({ cfg, prompter, accountOverrides, shouldPromptAccountIds, forceAllowFrom }) => {
    ...
    const defaultZaloAccountId = resolveDefaultZaloAccountId(cfg as EspadaConfig);
