 ...
 *
 * Self-contained circuit breaker for GCP service API calls.
 * Prevents cascading failures when a specific GCP service
 * (e.g. Compute, Cloud SQL) is degraded or throttling heavily.
 *
...
export class CircuitBreaker {
  ...
  constructor(name: string, config?: CircuitBreakerConfig) {
    ...
    this.shouldTrip = config?.shouldTrip ?? shouldRetryGcpError;
...
export function getGcpServiceBreaker(service: string, projectId?: string): CircuitBreaker {
  const name = projectId ? `gcp:${service}:${projectId}` : `gcp:${service}`;
  ...
  if (!cb) {
    cb = new CircuitBreaker(name, {
      onStateChange: (event) => {
        ...
        logFn(
          `[circuit-breaker] GCP ${service}${projectId ? ` (${projectId})` : ""}: ${event.from} → ${event.to}` +
...
export async function withGcpCircuitBreaker<T>(
  ...
  const cb = getGcpServiceBreaker(service, opts?.projectId);
...
export function getGcpCircuitBreakerHealthSummary() {
  const snapshots = getGcpCircuitBreakerSnapshots();
  const open: string[] = [];
