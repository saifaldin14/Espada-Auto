...
Espada extension for Zalo Personal Account messaging via [zca-cli](https://zca-cli.dev).
...
```bash
espada channels login --channel zalouser
...
```bash
espada message send --channel zalouser --target <threadId> --message "Hello from Espada!"
```
...
```bash
espada channels login --channel zalouser              # Login via QR
espada channels login --channel zalouser --account work
espada channels status --probe
espada channels logout --channel zalouser
```

### Directory (IDs, contacts, groups)

```bash
espada directory self --channel zalouser
espada directory peers list --channel zalouser --query "name"
espada directory groups list --channel zalouser --query "work"
espada directory groups members --channel zalouser --group-id <id>
```
...
```bash
# Text
espada message send --channel zalouser --target <threadId> --message "message"

# Media (URL)
espada message send --channel zalouser --target <threadId> --message "caption" --media-url "https://example.com/img.jpg"
```

### Listener

The listener runs inside the Gateway when the channel is enabled. For debugging,
use `espada channels logs --channel zalouser` or run `zca listen` directly.
...
```bash
espada channels login --channel zalouser --account work
espada message send --channel zalouser --account work --target <id> --message "Hello"
