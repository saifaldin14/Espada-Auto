...
function resolveZalouserGroupToolPolicy(
  ...
  const account = resolveZalouserAccountSync({
    cfg: params.cfg as EspadaConfig,
...
export const zalouserDock: ChannelDock = {
  ...
  config: {
    resolveAllowFrom: ({ cfg, accountId }) =>
      (resolveZalouserAccountSync({ cfg: cfg as EspadaConfig, accountId }).config.allowFrom ?? []).map(
        (entry) => String(entry),
      ),
...
export const zalouserPlugin: ChannelPlugin<ResolvedZalouserAccount> = {
  ...
  config: {
    listAccountIds: (cfg) => listZalouserAccountIds(cfg as EspadaConfig),
    resolveAccount: (cfg, accountId) =>
      resolveZalouserAccountSync({ cfg: cfg as EspadaConfig, accountId }),
    defaultAccountId: (cfg) => resolveDefaultZalouserAccountId(cfg as EspadaConfig),
    setAccountEnabled: ({ cfg, accountId, enabled }) =>
      setAccountEnabledInConfigSection({
        cfg: cfg as EspadaConfig,
        sectionKey: "zalouser",
        accountId,
        enabled,
        allowTopLevel: true,
  ...
  security: {
    resolveDmPolicy: ({ cfg, accountId, account }) => {
      ...
      const useAccountPath = Boolean(
        (cfg as EspadaConfig).channels?.zalouser?.accounts?.[resolvedAccountId],
      );
  ...
  directory: {
    self: async ({ cfg, accountId, runtime }) => {
      ...
      const account = resolveZalouserAccountSync({ cfg: cfg as EspadaConfig, accountId });
  ...
  pairing: {
    ...
    notifyApproval: async ({ cfg, id }) => {
      const account = resolveZalouserAccountSync({ cfg: cfg as EspadaConfig });
  ...
  auth: {
    login: async ({ cfg, accountId, runtime }) => {
      ...
      if (!ok) {
        throw new Error(
          "Missing dependency: `zca` not found in PATH. See docs.espada.dev/channels/zalouser",
        );
      }
      runtime.log(
  ...
  gateway: {
    startAccount: async (ctx) => {
      ...
      return monitorZalouserProvider({
        account,
        config: ctx.cfg as EspadaConfig,
        runtime: ctx.runtime,
