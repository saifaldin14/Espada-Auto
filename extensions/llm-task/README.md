...
`espada.invoke --each`) without adding new Espada code per workflow.
...
This extension depends on Espada internal modules (the embedded agent runner).
It is intended to ship as a **bundled** Espada extension (like `lobster`) and
be enabled via `plugins.entries` + tool allowlists.

It is **not** currently designed to be copied into
`~/.espada/extensions` as a standalone plugin directory.
