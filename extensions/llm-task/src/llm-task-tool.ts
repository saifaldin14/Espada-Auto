...
// NOTE: This extension is intended to be bundled with Espada.
// When running from source (tests/dev), Espada internals live under src/.
...
export function createLlmTaskTool(api: EspadaPluginApi) {
  return {
    name: "llm-task",
    description:
      "Run a generic JSON-only LLM task and return schema-validated JSON. Designed for orchestration from Lobster workflows via espada.invoke.",
    ...
    async execute(_id: string, params: Record<string, unknown>) {
      ...
      try {
        tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "espada-llm-task-"));
