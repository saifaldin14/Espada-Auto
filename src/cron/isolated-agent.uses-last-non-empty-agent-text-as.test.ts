...
async function withTempHome<T>(fn: (home: string) => Promise<T>): Promise<T> {
  return withTempHomeBase(fn, { prefix: "espada-cron-" });
}

async function writeSessionStore(home: string) {
  const dir = path.join(home, ".espada", "sessions");
...
function makeCfg(
  home: string,
  storePath: string,
  overrides: Partial<EspadaConfig> = {},
): EspadaConfig {
  const base: EspadaConfig = {
    agents: {
      defaults: {
        model: "anthropic/claude-opus-4-5",
        workspace: path.join(home, "espada"),
      },
    },
...
describe("runCronIsolatedAgentTurn", () => {
  ...
  it("uses agentId for workspace, session key, and store paths", async () => {
    await withTempHome(async (home) => {
      ...
      const cfg = makeCfg(
        home,
        path.join(home, ".espada", "agents", "{agentId}", "sessions", "sessions.json"),
        {
          agents: {
