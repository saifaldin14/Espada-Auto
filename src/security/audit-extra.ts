...
import type { EspadaConfig, ConfigFileSnapshot } from "../config/config.js";
...
function summarizeGroupPolicy(cfg: EspadaConfig): {
  open: number;
  allowlist: number;
  other: number;
} {
...
export function collectAttackSurfaceSummaryFindings(cfg: EspadaConfig): SecurityAuditFinding[] {
...
export function collectSyncedFolderFindings(params: {
  ...
  if (isProbablySyncedPath(params.stateDir) || isProbablySyncedPath(params.configPath)) {
    findings.push({
      ...
      remediation: `Keep ESPADA_STATE_DIR on a local-only volume and re-run "${formatCliCommand("espada security audit --fix")}".`,
    });
  }
  return findings;
}
...
export function collectSecretsInConfigFindings(cfg: EspadaConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
  const password =
  ...
  if (password && !looksLikeEnvRef(password)) {
    findings.push({
      ...
      remediation:
        "Prefer ESPADA_GATEWAY_PASSWORD (env) and remove gateway.auth.password from disk.",
    });
  }
...
export function collectHooksHardeningFindings(cfg: EspadaConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
...
function collectModels(cfg: EspadaConfig): ModelRef[] {
  const out: ModelRef[] = [];
...
export function collectModelHygieneFindings(cfg: EspadaConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
...
function hasWebSearchKey(cfg: EspadaConfig, env: NodeJS.ProcessEnv): boolean {
...
function isWebSearchEnabled(cfg: EspadaConfig, env: NodeJS.ProcessEnv): boolean {
...
function isWebFetchEnabled(cfg: EspadaConfig): boolean {
  const enabled = cfg.tools?.web?.fetch?.enabled;
  if (enabled === false) return false;
  return true;
}

function isBrowserEnabled(cfg: EspadaConfig): boolean {
  try {
...
function listGroupPolicyOpen(cfg: EspadaConfig): string[] {
  const out: string[] = [];
  const channels = cfg.channels as Record<string, unknown> | undefined;
...
export function collectExposureMatrixFindings(cfg: EspadaConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
