...
vi.mock("./models-config.js", () => ({
  ensureEspadaModelsJson: vi.fn().mockResolvedValue({ agentDir: "/tmp", wrote: false }),
}));

vi.mock("./agent-paths.js", () => ({
  resolveEspadaAgentDir: () => "/tmp/espada",
}));
...
describe("loadModelCatalog", () => {
  ...
  it("returns partial results on discovery errors", async () => {
    ...
    const result = await loadModelCatalog({ config: {} as EspadaConfig });
