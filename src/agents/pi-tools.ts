...
function resolveExecConfig(cfg: EspadaConfig | undefined) {
...
export function createEspadaCodingTools(options?: {
  ...
  const base = (codingTools as unknown as AnyAgentTool[]).flatMap((tool) => {
    if (tool.name === readTool.name) {
      ...
      return [createEspadaReadTool(freshReadTool)];
    }
