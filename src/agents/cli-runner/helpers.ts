...
/**
 * Cleanup suspended Espada CLI processes that have accumulated.
...
function buildModelAliasLines(cfg?: EspadaConfig) {
...
export async function writeCliImages(
  ...
  const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "espada-cli-images-"));
  const paths: string[] = [];
