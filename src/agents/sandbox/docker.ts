...
export function buildSandboxCreateArgs(params: {
  ...
  args.push("--label", "espada.sandbox=1");
  args.push("--label", `espada.sessionKey=${params.scopeKey}`);
  args.push("--label", `espada.createdAtMs=${createdAtMs}`);
  if (params.configHash) {
    args.push("--label", `espada.configHash=${params.configHash}`);
  }
...
async function readContainerConfigHash(containerName: string): Promise<string | null> {
  const result = await execDocker(
    ["inspect", "-f", '{{ index .Config.Labels "espada.configHash" }}', containerName],
    { allowFailure: true },
  );
  if (result.code !== 0) return null;
...
function formatSandboxRecreateHint(params: { scope: SandboxConfig["scope"]; sessionKey: string }) {
  if (params.scope === "session") {
    return formatCliCommand(`espada sandbox recreate --session ${params.sessionKey}`);
  }
  if (params.scope === "agent") {
    const agentId = resolveSandboxAgentId(params.sessionKey) ?? "main";
    return formatCliCommand(`espada sandbox recreate --agent ${agentId}`);
  }
  return formatCliCommand("espada sandbox recreate --all");
}
