...
type WebSearchConfig = NonNullable<EspadaConfig["tools"]>["web"] extends infer Web
...
function resolveSearchConfig(cfg?: EspadaConfig): WebSearchConfig {
...
function missingSearchKeyPayload(provider: (typeof SEARCH_PROVIDERS)[number]) {
  ...
  return {
    ...
    message: `web_search needs a Brave Search API key. Run \`${formatCliCommand("espada configure --section web")}\` to store it, or set BRAVE_API_KEY in the Gateway environment.`,
    docs: "https://docs.espada.dev/tools/web",
  };
}
...
async function runPerplexitySearch(params: {
  ...
  const res = await fetch(endpoint, {
    ...
    headers: {
      ...
      "HTTP-Referer": "https://espada.dev",
      "X-Title": "Espada Web Search",
    },
    body: JSON.stringify({
      model: params.model,
      messages: [
