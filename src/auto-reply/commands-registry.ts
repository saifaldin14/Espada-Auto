import type { EspadaConfig } from "../config/types.js";
...
export function isCommandEnabled(cfg: EspadaConfig, commandKey: string): boolean {
...
function resolveDefaultCommandContext(cfg?: EspadaConfig): {
  provider: string;
  model: string;
} {
...
export function getCommandDetection(_cfg?: EspadaConfig): CommandDetection {
  const commands = getChatCommands();
...
export function maybeResolveTextAlias(raw: string, cfg?: EspadaConfig) {
