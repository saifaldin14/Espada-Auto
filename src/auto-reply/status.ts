...
type AgentConfig = Partial<NonNullable<NonNullable<EspadaConfig["agents"]>["defaults"]>>;
...
const readUsageFromSessionLog = (
  ...
  | undefined => {
  // Transcripts are stored at the session file path (fallback: ~/.espada/sessions/<SessionId>.jsonl)
...
export function buildHelpMessage(cfg?: EspadaConfig): string {
  const lines = ["ℹ️ Help", ""];
