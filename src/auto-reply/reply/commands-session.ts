...
import { scheduleGatewaySigusr1Restart, triggerEspadaRestart } from "../../infra/restart.js";
...
export const handleRestartCommand: CommandHandler = async (params, allowTextCommands) => {
  ...
  if (hasSigusr1Listener) {
    ...
    return {
      shouldContinue: false,
      reply: {
        text: "⚙️ Restarting espada in-process (SIGUSR1); back in a few seconds.",
      },
    };
  }
  const restartMethod = triggerEspadaRestart();
  if (!restartMethod.ok) {
  ...
  return {
    shouldContinue: false,
    reply: {
      text: `⚙️ Restarting espada via ${restartMethod.method}; give me a few seconds to come back online.`,
    },
  };
};
