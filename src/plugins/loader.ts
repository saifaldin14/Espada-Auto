...
import { discoverEspadaPlugins } from "./discovery.js";
...
function resolvePluginModuleExport(moduleExport: unknown): {
  definition?: EspadaPluginDefinition;
  register?: EspadaPluginDefinition["register"];
} {
  const resolved =
    moduleExport &&
  ...
  if (typeof resolved === "function") {
    return {
      register: resolved as EspadaPluginDefinition["register"],
    };
  }
  if (resolved && typeof resolved === "object") {
    const def = resolved as EspadaPluginDefinition;
...
export function loadEspadaPlugins(options: PluginLoadOptions = {}): PluginRegistry {
  ...
  const discovery = discoverEspadaPlugins({
    workspaceDir: options.workspaceDir,
  ...
  for (const candidate of discovery.candidates) {
    ...
    let mod: EspadaPluginModule | null = null;
    try {
      mod = jiti(candidate.source) as EspadaPluginModule;
    } catch (err) {
