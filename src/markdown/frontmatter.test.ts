...
`;
    ...
    const parsed = JSON5.parse(result.metadata ?? "") as { espada?: { emoji?: string } };
    expect(parsed.espada?.emoji).toBe("disk");
  });
...
`;
    ...
    const parsed = JSON5.parse(result.metadata ?? "") as { espada?: { events?: string[] } };
