...
(() => {
  ...
  const hasIOS = () =>
    !!(
      ...
      (window.webkit.messageHandlers.espadaCanvasA2UIAction ||
        window.webkit.messageHandlers.espadaCanvasA2UIAction)
    );
  const hasAndroid = () =>
    !!(
      (window.espadaCanvasA2UIAction &&
        typeof window.espadaCanvasA2UIAction.postMessage === "function") ||
      (window.espadaCanvasA2UIAction &&
        typeof window.espadaCanvasA2UIAction.postMessage === "function")
    );
  const legacySend = typeof window.espadaSendUserAction === "function" ? window.espadaSendUserAction : undefined;
  if (!window.espadaSendUserAction && legacySend) {
    window.espadaSendUserAction = legacySend;
  }
  if (!window.espadaSendUserAction && typeof window.espadaSendUserAction === "function") {
    window.espadaSendUserAction = window.espadaSendUserAction;
  }
  const hasHelper = () =>
    typeof window.espadaSendUserAction === "function" ||
    typeof window.espadaSendUserAction === "function";
  statusEl.innerHTML =
    "Bridge: " +
  ...
  window.addEventListener("espada:a2ui-action-status", (ev) => {
  ...
  function send(name, sourceComponentId) {
    if (!hasHelper()) {
      log("No action bridge found. Ensure you're viewing this on an iOS/Android Espada node canvas.");
      return;
    }
    const sendUserAction =
      typeof window.espadaSendUserAction === "function"
        ? window.espadaSendUserAction
        : window.espadaSendUserAction;
...
function isDisabledByEnv() {
  if (isTruthyEnvValue(process.env.ESPADA_SKIP_CANVAS_HOST)) return true;
...
export async function createCanvasHostHandler(
  ...
  const rootDir = resolveUserPath(opts.rootDir ?? path.join(os.homedir(), "espada", "canvas"));
    ...
    try {
      ...
      if (!opened) {
        if (urlPath === "/" || urlPath.endsWith("/")) {
          ...
          res.end(
            `<!doctype html><meta charset="utf-8" /><title>Espada Canvas</title><pre>Missing file.\nCreate ${rootDir}/index.html</pre>`,
          );
          return true;
        }
        res.statusCode = 404;
