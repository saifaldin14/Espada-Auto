...
vi.mock("../../agents/pi-embedded-runner/run/attempt.js", () => ({
  runEmbeddedAttempt: (params: unknown) => runEmbeddedAttemptMock(params),
}));

let runEmbeddedPiAgent: typeof import("../../agents/pi-embedded-runner.js").runEmbeddedPiAgent;

beforeAll(async () => {
  ({ runEmbeddedPiAgent } = await import("../../agents/pi-embedded-runner.js"));
});
...
const makeConfig = (opts?: { fallbacks?: string[]; apiKey?: string }): EspadaConfig =>
  ({
    agents: {
      defaults: {
        model: {
...
describe("runEmbeddedPiAgent auth profile rotation", () => {
  it("rotates for auto-pinned profiles", async () => {
    const agentDir = await fs.mkdtemp(path.join(os.tmpdir(), "espada-agent-"));
    const workspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "espada-workspace-"));
    try {
