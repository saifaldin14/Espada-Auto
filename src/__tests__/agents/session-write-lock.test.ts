...
import { __testing, acquireSessionWriteLock } from "../../agents/session-write-lock.js";
...
describe("acquireSessionWriteLock", () => {
  it("reuses locks across symlinked session paths", async () => {
    ...
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "espada-lock-"));
    try {
  ...
  it("removes held locks on termination signals", async () => {
    ...
    for (const signal of signals) {
      const root = await fs.mkdtemp(path.join(os.tmpdir(), "espada-lock-cleanup-"));
      try {
        const sessionFile = path.join(root, "sessions.json");
        const lockPath = `${sessionFile}.lock`;
