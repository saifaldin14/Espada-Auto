...
import { createEspadaTools } from "../../agents/espada-tools.js";
import { __testing, createEspadaCodingTools } from "../../agents/pi-tools.js";
import { createSandboxedReadTool } from "../../agents/pi-tools.read.js";
...
describe("createEspadaCodingTools", () => {
  ...
  it("does not expose provider-specific message tools", () => {
    const tools = createEspadaCodingTools({ messageProvider: "discord" });
  ...
  it("applies sandbox path guards to file_path alias", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "espada-sbx-"));
    const outsidePath = path.join(os.tmpdir(), "espada-outside.txt");
