...
import { CHUTES_TOKEN_ENDPOINT, type ChutesStoredOAuth } from "../../agents/chutes-oauth.js";
...
describe("auth-profiles (chutes)", () => {
  ...
  afterEach(async () => {
    ...
    if (previousAgentDir === undefined) delete process.env.ESPADA_AGENT_DIR;
    else process.env.ESPADA_AGENT_DIR = previousAgentDir;
  ...
  it("refreshes expired Chutes OAuth credentials", async () => {
    tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "espada-chutes-"));
    process.env.ESPADA_STATE_DIR = tempDir;
    process.env.ESPADA_AGENT_DIR = path.join(tempDir, "agents", "main", "agent");
