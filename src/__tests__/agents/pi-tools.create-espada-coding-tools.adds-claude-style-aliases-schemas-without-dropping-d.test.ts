...
const defaultTools = createEspadaCodingTools();

describe("createEspadaCodingTools", () => {
  it("keeps read tool image metadata intact", async () => {
    const readTool = defaultTools.find((tool) => tool.name === "read");
    expect(readTool).toBeDefined();

    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "espada-read-"));
    try {
  ...
  it("returns text content without image blocks for text files", async () => {
    const tools = createEspadaCodingTools();
    ...
    try {
      ...
      const contents = "Hello from espada read tool.";
  ...
  it("filters tools by sandbox policy", () => {
    const sandbox = {
      ...
      workspaceDir: path.join(os.tmpdir(), "espada-sandbox"),
      agentWorkspaceDir: path.join(os.tmpdir(), "espada-workspace"),
    ...
    const tools = createEspadaCodingTools({ sandbox });
