...
import { createCacheTrace } from "../../agents/cache-trace.js";
...
describe("createCacheTrace", () => {
  ...
  it("honors diagnostics cache trace config and expands file paths", () => {
    ...
    const trace = createCacheTrace({
      cfg: {
        diagnostics: {
          cacheTrace: {
            enabled: true,
            filePath: "~/.espada/logs/cache-trace.jsonl",
          },
        },
      },
      env: {},
    ...
    expect(trace?.filePath).toBe(resolveUserPath("~/.espada/logs/cache-trace.jsonl"));
