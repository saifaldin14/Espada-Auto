...
describe("browser extension install", () => {
  it("installs into the state dir (never node_modules)", async () => {
    const tmp = fs.mkdtempSync(path.join(os.tmpdir(), "espada-ext-"));
    const { installChromeExtension } = await import("../../cli/browser-cli-extension.js");
  ...
  it("copies extension path to clipboard", async () => {
    ...
    const tmp = fs.mkdtempSync(path.join(os.tmpdir(), "espada-ext-path-"));
    ...
    try {
      ...
      const { registerBrowserExtensionCommands } = await import("../../cli/browser-cli-extension.js");
      ...
      if (prev === undefined) delete process.env.ESPADA_STATE_DIR;
      else process.env.ESPADA_STATE_DIR = prev;
    }
  });
});
