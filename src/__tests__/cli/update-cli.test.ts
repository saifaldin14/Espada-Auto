...
vi.mock("../../infra/espada-root.js", () => ({
  resolveEspadaPackageRoot: vi.fn(),
}));
...
describe("update-cli", () => {
  ...
  beforeEach(async () => {
    vi.clearAllMocks();
    const { resolveEspadaPackageRoot } = await import("../../infra/espada-root.js");
    const { readConfigFileSnapshot } = await import("../../config/config.js");
    const { checkUpdateStatus, fetchNpmTagVersion, resolveNpmChannelTag } =
      await import("../../infra/update-check.js");
    const { runCommandWithTimeout } = await import("../../process/exec.js");
    vi.mocked(resolveEspadaPackageRoot).mockResolvedValue(process.cwd());
  ...
  it("exports updateCommand and registerUpdateCli", async () => {
    const { updateCommand, registerUpdateCli, updateWizardCommand } =
      await import("../../cli/update-cli.js");
  ...
  it("updateCommand runs update and outputs result", async () => {
    ...
    const { updateCommand } = await import("../../cli/update-cli.js");
  ...
  it("updateStatusCommand prints table output", async () => {
    const { defaultRuntime } = await import("../../runtime.js");
    const { updateStatusCommand } = await import("../../cli/update-cli.js");

    await updateStatusCommand({ json: false });

    const logs = vi.mocked(defaultRuntime.log).mock.calls.map((call) => call[0]);
    expect(logs.join("\n")).toContain("Espada update status");
  });
  ...
  it("defaults to stable channel for package installs when unset", async () => {
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "espada-update-"));
    try {
      await fs.writeFile(
        path.join(tempDir, "package.json"),
        JSON.stringify({ name: "espada", version: "1.0.0" }),
        "utf-8",
      );
      ...
      vi.mocked(resolveEspadaPackageRoot).mockResolvedValue(tempDir);
      vi.mocked(checkUpdateStatus).mockResolvedValue({
        root: tempDir,
        installKind: "package",
        packageManager: "npm",
  ...
  it("updateCommand restarts daemon by default", async () => {
    const { runGatewayUpdate } = await import("../../infra/update-runner.js");
    const { runDaemonRestart } = await import("../../cli/daemon-cli.js");
    const { updateCommand } = await import("../../cli/update-cli.js");
  ...
  it("requires confirmation on downgrade when non-interactive", async () => {
    ...
    try {
      ...
      await fs.writeFile(
        path.join(tempDir, "package.json"),
        JSON.stringify({ name: "espada", version: "2.0.0" }),
        "utf-8",
      );
  ...
  it("updateWizardCommand requires a TTY", async () => {
    const { defaultRuntime } = await import("../../runtime.js");
    const { updateWizardCommand } = await import("../../cli/update-cli.js");
  ...
  it("updateWizardCommand offers dev checkout and forwards selections", async () => {
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "espada-update-wizard-"));
    const previousGitDir = process.env.ESPADA_GIT_DIR;
    try {
      setTty(true);
      process.env.ESPADA_GIT_DIR = tempDir;
      ...
      process.env.ESPADA_GIT_DIR = previousGitDir;
      await fs.rm(tempDir, { recursive: true, force: true });
    }
  });
});
