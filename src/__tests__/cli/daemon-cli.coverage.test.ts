...
describe("daemon-cli coverage", () => {
  const originalEnv = {
    ESPADA_STATE_DIR: process.env.ESPADA_STATE_DIR,
    ESPADA_CONFIG_PATH: process.env.ESPADA_CONFIG_PATH,
    ESPADA_GATEWAY_PORT: process.env.ESPADA_GATEWAY_PORT,
    ESPADA_PROFILE: process.env.ESPADA_PROFILE,
  };

  beforeEach(() => {
    process.env.ESPADA_STATE_DIR = "/tmp/espada-cli-state";
    process.env.ESPADA_CONFIG_PATH = "/tmp/espada-cli-state/espada.json";
    delete process.env.ESPADA_GATEWAY_PORT;
    delete process.env.ESPADA_PROFILE;
    serviceReadCommand.mockResolvedValue(null);
  });

  afterEach(() => {
    if (originalEnv.ESPADA_STATE_DIR !== undefined)
      process.env.ESPADA_STATE_DIR = originalEnv.ESPADA_STATE_DIR;
    else delete process.env.ESPADA_STATE_DIR;

    if (originalEnv.ESPADA_CONFIG_PATH !== undefined)
      process.env.ESPADA_CONFIG_PATH = originalEnv.ESPADA_CONFIG_PATH;
    else delete process.env.ESPADA_CONFIG_PATH;

    if (originalEnv.ESPADA_GATEWAY_PORT !== undefined)
      process.env.ESPADA_GATEWAY_PORT = originalEnv.ESPADA_GATEWAY_PORT;
    else delete process.env.ESPADA_GATEWAY_PORT;

    if (originalEnv.ESPADA_PROFILE !== undefined)
      process.env.ESPADA_PROFILE = originalEnv.ESPADA_PROFILE;
    else delete process.env.ESPADA_PROFILE;
  });
  ...
  it("probes gateway status by default", async () => {
    ...
    const { registerDaemonCli } = await import("../../cli/daemon-cli.js");
  ...
  it("derives probe URL from service args + env (json)", async () => {
    ...
    serviceReadCommand.mockResolvedValueOnce({
      ...
      environment: {
        ESPADA_PROFILE: "dev",
        ESPADA_STATE_DIR: "/tmp/espada-daemon-state",
        ESPADA_CONFIG_PATH: "/tmp/espada-daemon-state/espada.json",
        ESPADA_GATEWAY_PORT: "19001",
      },
