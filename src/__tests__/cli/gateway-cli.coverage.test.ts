...
describe("gateway-cli coverage", () => {
  it("registers call/health commands and routes to callGateway", async () => {
    ...
    const { registerGatewayCli } = await import("../../cli/gateway-cli.js");
  ...
  it("registers gateway discover and prints human output with details on new lines", async () => {
    ...
    discoverGatewayBeacons.mockResolvedValueOnce([
      {
        instanceName: "Studio (Espada)",
        displayName: "Studio",
        domain: "espada.internal.",
        host: "studio.espada.internal",
        lanHost: "studio.local",
        tailnetDns: "studio.tailnet.ts.net",
        gatewayPort: 18789,
        sshPort: 22,
    ...
    expect(out).toContain("- Studio espada.internal.");
    expect(out).toContain("  tailnet: studio.tailnet.ts.net");
    expect(out).toContain("  host: studio.espada.internal");
