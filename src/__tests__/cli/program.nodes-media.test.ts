...
vi.mock("../../cli/channel-auth.js", () => ({ runChannelLogin, runChannelLogout }));
...
describe("cli program (nodes media)", () => {
  ...
  it("runs nodes camera clip and prints one MEDIA path", async () => {
    ...
    expect(mediaPath).toMatch(/espada-camera-clip-front-.*\.mp4$/);
  ...
  it("runs nodes canvas snapshot and prints MEDIA path", async () => {
    ...
    const out = String(runtime.log.mock.calls[0]?.[0] ?? "");
    const mediaPath = out.replace(/^MEDIA:/, "").trim();
    expect(mediaPath).toMatch(/espada-canvas-snapshot-.*\.png$/);
