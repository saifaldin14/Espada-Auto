...
vi.mock("../../infra/update-check.js", async () => {
  const actual = await vi.importActual<typeof import("../../infra/update-check.js")>("./update-check.js");
  return {
    ...actual,
...
describe("update-startup", () => {
  ...
  beforeEach(async () => {
    ...
    tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "espada-update-check-"));
  ...
  it("logs update hint for npm installs when newer tag exists", async () => {
    ...
    const { checkUpdateStatus, resolveNpmChannelTag } = await import("../../infra/update-check.js");
    const { runGatewayUpdateCheck } = await import("../../infra/update-startup.js");

    vi.mocked(resolveEspadaPackageRoot).mockResolvedValue("/opt/espada");
