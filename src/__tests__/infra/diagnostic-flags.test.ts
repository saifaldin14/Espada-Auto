...
import { isDiagnosticFlagEnabled, resolveDiagnosticFlags } from "../../infra/diagnostic-flags.js";
...
describe("diagnostic flags", () => {
  ...
  it("treats env true as wildcard", () => {
    const env = { ESPADA_DIAGNOSTICS: "1" } as NodeJS.ProcessEnv;
    expect(isDiagnosticFlagEnabled("anything.here", undefined, env)).toBe(true);
  });

  it("treats env false as disabled", () => {
    const env = { ESPADA_DIAGNOSTICS: "0" } as NodeJS.ProcessEnv;
