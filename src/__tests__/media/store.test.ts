...
describe("media store", () => {
  ...
  const snapshotEnv = () => {
    for (const key of ["HOME", "USERPROFILE", "HOMEDRIVE", "HOMEPATH", "ESPADA_STATE_DIR"]) {
  ...
  beforeAll(async () => {
    snapshotEnv();
    home = await fs.mkdtemp(path.join(os.tmpdir(), "espada-test-home-"));
    process.env.HOME = home;
    process.env.USERPROFILE = home;
    process.env.ESPADA_STATE_DIR = path.join(home, ".espada");
    ...
    await fs.mkdir(path.join(home, ".espada"), { recursive: true });
  ...
  async function withTempStore<T>(
    fn: (store: typeof import("../../media/store.js"), home: string) => Promise<T>,
  ): Promise<T> {
  ...
  it("creates and returns media directory", async () => {
    await withTempStore(async (store, home) => {
      ...
      expect(path.normalize(dir)).toContain(`${path.sep}.espada${path.sep}media`);
