...
import { loadEspadaPlugins } from "../../plugins/loader.js";

type TempPlugin = { dir: string; file: string; id: string };

const tempDirs: string[] = [];
const prevBundledDir = process.env.ESPADA_BUNDLED_PLUGINS_DIR;
const EMPTY_PLUGIN_SCHEMA = { type: "object", additionalProperties: false, properties: {} };

function makeTempDir() {
  const dir = path.join(os.tmpdir(), `espada-plugin-${randomUUID()}`);
...
function writePlugin(params: {
  ...
  fs.writeFileSync(
    path.join(dir, "espada.plugin.json"),
    JSON.stringify(
      {
        id: params.id,
...
afterEach(() => {
  ...
  if (prevBundledDir === undefined) {
    ...
    process.env.ESPADA_BUNDLED_PLUGINS_DIR = prevBundledDir;
  }
});

describe("loadEspadaPlugins", () => {
  it("disables bundled plugins by default", () => {
    ...
    process.env.ESPADA_BUNDLED_PLUGINS_DIR = bundledDir;
    ...
    const enabledRegistry = loadEspadaPlugins({
      cache: false,
      config: {
        plugins: {
          allow: ["bundled"],
