...
function makeTempDir() {
  const dir = path.join(os.tmpdir(), `espada-plugin-install-${randomUUID()}`);
...
describe("installPluginFromArchive", () => {
  it("installs into ~/.espada/extensions and uses unscoped id", async () => {
    ...
    fs.writeFileSync(
      ...
      JSON.stringify({
        ...
        version: "0.0.1",
        espada: { extensions: ["./dist/index.js"] },
      }),
      "utf-8",
    );
    ...
    const { installPluginFromArchive } = await import("../../plugins/install.js");
  ...
  it("rejects packages without espada.extensions", async () => {
    const stateDir = makeTempDir();
    const workDir = makeTempDir();
    const pkgDir = path.join(workDir, "package");
    ...
    fs.writeFileSync(
      path.join(pkgDir, "package.json"),
      JSON.stringify({ name: "@espada/nope", version: "0.0.1" }),
      "utf-8",
    );
    ...
    if (result.ok) return;
    expect(result.error).toContain("espada.extensions");
  });
});
