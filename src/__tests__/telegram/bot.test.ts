...
let createTelegramBot: typeof import("../../telegram/bot.js").createTelegramBot;
let getTelegramSequentialKey: typeof import("../../telegram/bot.js").getTelegramSequentialKey;
...
const { sessionStorePath } = vi.hoisted(() => ({
  sessionStorePath: `/tmp/espada-telegram-bot-${Math.random().toString(16).slice(2)}.json`,
}));
...
vi.mock("../../telegram/sent-message-cache.js", () => ({
  wasSentByBot,
...
describe("createTelegramBot", () => {
  beforeEach(async () => {
    ...
    ({ createTelegramBot, getTelegramSequentialKey } = await import("../../telegram/bot.js"));
    ...
    await handler({
      message: {
        ...
        reply_to_message: {
          message_id: 42,
          text: "original reply",
          from: { id: 999, first_name: "Espada" },
        },
      },
      me: { id: 999, username: "espada_bot" },
      getFile: async () => ({ download: async () => new Uint8Array() }),
    });
  ...
  it("honors routed group activation from session store", async () => {
    ...
    const replySpy = replyModule.__replySpy as unknown as ReturnType<typeof vi.fn>;
    replySpy.mockReset();
    const storeDir = fs.mkdtempSync(path.join(os.tmpdir(), "espada-telegram-"));
