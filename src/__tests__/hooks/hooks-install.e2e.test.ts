...
async function makeTempDir() {
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "espada-hooks-e2e-"));
  tempDirs.push(dir);
  return dir;
}
...
describe("hooks install (e2e)", () => {
  ...
  beforeEach(async () => {
    ...
    prevStateDir = process.env.ESPADA_STATE_DIR;
    prevBundledDir = process.env.ESPADA_BUNDLED_HOOKS_DIR;
    process.env.ESPADA_STATE_DIR = path.join(baseDir, "state");
    process.env.ESPADA_BUNDLED_HOOKS_DIR = path.join(baseDir, "bundled-none");
    vi.resetModules();
  });
  ...
  afterEach(async () => {
    ...
    if (prevBundledDir === undefined) {
      delete process.env.ESPADA_BUNDLED_HOOKS_DIR;
    } else {
      process.env.ESPADA_BUNDLED_HOOKS_DIR = prevBundledDir;
    }
  ...
  it("installs a hook pack and triggers the handler", async () => {
    ...
    await fs.writeFile(
      ...
      [
        ...
        'metadata: {"espada":{"events":["command:new"]}}',
        "---",
        "",
        "# Hello Hook",
        "",
    ...
    const { installHooksFromPath } = await import("../../hooks/install.js");
