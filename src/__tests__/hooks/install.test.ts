...
function makeTempDir() {
  const dir = path.join(os.tmpdir(), `espada-hook-install-${randomUUID()}`);
...
describe("installHooksFromArchive", () => {
  it("installs hook packs from zip archives", async () => {
    ...
    zip.file(
      ...
      JSON.stringify({
        name: "@espada/zip-hooks",
        version: "0.0.1",
        espada: { hooks: ["./hooks/zip-hook"] },
      }),
    );
    zip.file(
    ...
    const { installHooksFromArchive } = await import("../../hooks/install.js");
  ...
  it("installs hook packs from tar archives", async () => {
    ...
    fs.writeFileSync(
      ...
      JSON.stringify({
        name: "@espada/tar-hooks",
        version: "0.0.1",
        espada: { hooks: ["./hooks/tar-hook"] },
      }),
      "utf-8",
    );
    fs.writeFileSync(
