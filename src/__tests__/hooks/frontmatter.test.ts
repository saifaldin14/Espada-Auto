...
import {
  parseFrontmatter,
  resolveEspadaMetadata,
  resolveHookInvocationPolicy,
} from "../../hooks/frontmatter.js";
...
`;
    ...
    expect(parsed.espada.emoji).toBe("💾");
    expect(parsed.espada.events).toEqual(["command:new"]);
  });
...
`;
    ...
    const parsed = JSON.parse(result.metadata as string);
    expect(parsed.espada.emoji).toBe("📝");
    expect(parsed.espada.events).toEqual(["command"]);
    expect(parsed.espada.requires.config).toEqual(["workspace.dir"]);
    expect(parsed.espada.install[0].kind).toBe("bundled");
  });

  it("handles single-line metadata (inline JSON)", () => {
    const content = `---
name: simple-hook
metadata: {"espada": {"events": ["test"]}}
---
`;
    const result = parseFrontmatter(content);
    expect(result.name).toBe("simple-hook");
    expect(result.metadata).toBe('{"espada": {"events": ["test"]}}');
  });
...
describe("resolveEspadaMetadata", () => {
  it("extracts espada metadata from parsed frontmatter", () => {
    const frontmatter = {
      name: "test-hook",
    ...
    const result = resolveEspadaMetadata(frontmatter);
  ...
  it("returns undefined when espada key is missing", () => {
    const frontmatter = {
  ...
  it("handles install specs", () => {
    const frontmatter = {
      metadata: JSON.stringify({
        espada: {
          events: ["command"],
          install: [
            { id: "bundled", kind: "bundled", label: "Bundled with Espada" },
            { id: "npm", kind: "npm", package: "@espada/hook" },
          ],
        },
      }),
    };
    ...
    expect(result?.install?.[1].package).toBe("@espada/hook");
  });
...
`;
    ...
    const espada = resolveEspadaMetadata(frontmatter);
    expect(espada).toBeDefined();
    expect(espada?.emoji).toBe("💾");
    expect(espada?.events).toEqual(["command:new"]);
    expect(espada?.requires?.config).toEqual(["workspace.dir"]);
    expect(espada?.install?.[0].kind).toBe("bundled");
  });
