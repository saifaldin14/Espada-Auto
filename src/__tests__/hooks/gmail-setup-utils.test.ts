...
describe("resolvePythonExecutablePath", () => {
  itUnix(
    ...
    async () => {
      const tmp = await fs.mkdtemp(path.join(os.tmpdir(), "espada-python-"));
      ...
      try {
        ...
        const { resolvePythonExecutablePath } = await import("../../hooks/gmail-setup-utils.js");
...
describe("ensureTailscaleEndpoint", () => {
  it("includes stdout and exit code when tailscale serve fails", async () => {
    ...
    const { ensureTailscaleEndpoint } = await import("../../hooks/gmail-setup-utils.js");
