...
import { listChannelPairingRequests, upsertChannelPairingRequest } from "../../pairing/pairing-store.js";

async function withTempStateDir<T>(fn: (stateDir: string) => Promise<T>) {
  const previous = process.env.ESPADA_STATE_DIR;
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "espada-pairing-"));
