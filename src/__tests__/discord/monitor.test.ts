...
} from "../../discord/monitor.js";
import { DiscordMessageListener } from "../../discord/monitor/listeners.js";
...
describe("DiscordMessageListener", () => {
  it("returns before the handler finishes", async () => {
    ...
    await listener.handle(
      {} as unknown as import("../../discord/monitor/listeners.js").DiscordMessageEvent,
...
describe("discord allowlist helpers", () => {
  it("normalizes slugs", () => {
    expect(normalizeDiscordSlug("Friends of Espada")).toBe("friends-of-espada");
...
describe("discord guild/channel resolution", () => {
  it("resolves guild entry by id", () => {
    const guildEntries = makeEntries({
      "123": { slug: "friends-of-espada" },
    });
    const resolved = resolveDiscordGuildEntry({
      guild: fakeGuild("123", "Friends of Espada"),
      guildEntries,
    });
    expect(resolved?.id).toBe("123");
    expect(resolved?.slug).toBe("friends-of-espada");
  });

  it("resolves guild entry by slug key", () => {
    const guildEntries = makeEntries({
      "friends-of-espada": { slug: "friends-of-espada" },
    });
    const resolved = resolveDiscordGuildEntry({
      guild: fakeGuild("123", "Friends of Espada"),
      guildEntries,
  ...
  it("matches group DM allowlist", () => {
    expect(
      resolveGroupDmAllow({
        channels: ["espada-dm"],
        channelId: "1",
        channelName: "Espada DM",
        channelSlug: "espada-dm",
      }),
    ).toBe(true);
    expect(
      resolveGroupDmAllow({
