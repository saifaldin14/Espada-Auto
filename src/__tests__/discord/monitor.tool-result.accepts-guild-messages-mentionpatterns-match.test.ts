...
import { __resetDiscordChannelInfoCacheForTest } from "../../discord/monitor/message-utils.js";
...
vi.mock("../../config/sessions.js", async (importOriginal) => {
  ...
  return {
    ...actual,
    resolveStorePath: vi.fn(() => "/tmp/espada-sessions.json"),
...
describe("discord tool result dispatch", () => {
  it("accepts guild messages when mentionPatterns match", async () => {
    const { createDiscordMessageHandler } = await import("../../discord/monitor.js");
    const cfg = {
      agents: {
        defaults: {
          model: "anthropic/claude-opus-4-5",
          workspace: "/tmp/espada",
        },
      },
      session: { store: "/tmp/espada-sessions.json" },
      channels: {
        discord: {
      ...
      messages: {
        responsePrefix: "PFX",
        groupChat: { mentionPatterns: ["\\bespada\\b"] },
      },
    ...
    await handler(
      {
        message: {
          ...
          referencedMessage: {
            ...
            mentionedUsers: [],
            mentionedRoles: [],
            author: { id: "bot-id", bot: true, username: "Espada" },
          },
        },
        author: { id: "u1", bot: false, username: "Ada" },
        member: { nickname: "Ada" },
  ...
  it("treats forum threads as distinct sessions without channel payloads", async () => {
    ...
    const cfg = {
      agent: { model: "anthropic/claude-opus-4-5", workspace: "/tmp/espada" },
      session: { store: "/tmp/espada-sessions.json" },
      channels: {
        discord: {
          dm: { enabled: true, policy: "open" },
