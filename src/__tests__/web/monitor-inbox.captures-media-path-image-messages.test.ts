...
import { monitorWebInbox, resetWebInboundDedupe } from "../../web/inbound.js";
...
describe("web monitor inbox", () => {
  beforeEach(() => {
    ...
    authDir = fsSync.mkdtempSync(path.join(os.tmpdir(), "espada-auth-"));
  });
  ...
  it("logs inbound bodies to file", async () => {
    const logPath = path.join(os.tmpdir(), `espada-log-test-${crypto.randomUUID()}.log`);
