...
import { monitorWebChannel, SILENT_REPLY_TOKEN } from "../../web/auto-reply.js";
import { resetBaileysMocks, resetLoadConfigMock, setLoadConfigMock } from "../../web/test-helpers.js";
...
beforeEach(async () => {
  ...
  tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "espada-web-home-"));
...
const makeSessionStore = async (
  ...
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "espada-session-"));
...
describe("web auto-reply", () => {
  ...
  it("supports always-on group activation with silent token and clears pending history", async () => {
    ...
    setLoadConfigMock(() => ({
      messages: {
        groupChat: { mentionPatterns: ["@espada"] },
      },
      session: { store: storePath },
    }));

    let capturedOnMessage:
      | ((msg: import("../../web/inbound.js").WebInboundMessage) => Promise<void>)
      | undefined;
    const listenerFactory = async (opts: {
      onMessage: (msg: import("../../web/inbound.js").WebInboundMessage) => Promise<void>;
    }) => {
  ...
  it("ignores JID mentions in self-chat mode (group chats)", async () => {
    ...
    // Text-based mentionPatterns still work (user can type "espada" explicitly).
    await capturedOnMessage?.({
      body: "espada ping",
      from: "123@g.us",
      conversationId: "123@g.us",
  ...
  it("emits heartbeat logs with connection metadata", async () => {
    vi.useFakeTimers();
    const logPath = `/tmp/espada-heartbeat-${crypto.randomUUID()}.log`;
  ...
  it("logs outbound replies to file", async () => {
    const logPath = `/tmp/espada-log-test-${crypto.randomUUID()}.log`;
    setLoggerOverride({ level: "trace", file: logPath });
