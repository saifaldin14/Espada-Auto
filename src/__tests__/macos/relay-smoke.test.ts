...
import { parseRelaySmokeTest, runRelaySmokeTest } from "../../macos/relay-smoke.js";
...
describe("parseRelaySmokeTest", () => {
  ...
  it("parses env var smoke mode only when no args", () => {
    expect(parseRelaySmokeTest([], { ESPADA_SMOKE_QR: "1" })).toBe("qr");
    expect(parseRelaySmokeTest(["send"], { ESPADA_SMOKE_QR: "1" })).toBe(null);
  });
