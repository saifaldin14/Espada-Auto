...
import { CANVAS_HOST_PATH, CANVAS_WS_PATH, injectCanvasLiveReload } from "../../canvas-host/a2ui.js";
...
describe("canvas host", () => {
  it("injects live reload script", () => {
    ...
    expect(out).toContain("espadaCanvasA2UIAction");
    expect(out).toContain("espadaSendUserAction");
  });

  it("creates a default index.html when missing", async () => {
    const dir = await fs.mkdtemp(path.join(os.tmpdir(), "espada-canvas-"));
    ...
    try {
      ...
      expect(html).toContain("espadaSendUserAction");
  ...
  it("serves the gateway-hosted A2UI scaffold", async () => {
    ...
    try {
      const res = await fetch(`http://127.0.0.1:${server.port}/__espada__/a2ui/`);
      const html = await res.text();
      expect(res.status).toBe(200);
      expect(html).toContain("espada-a2ui-host");
      expect(html).toContain("espadaCanvasA2UIAction");

      const bundleRes = await fetch(
        `http://127.0.0.1:${server.port}/__espada__/a2ui/a2ui.bundle.js`,
      );
      const js = await bundleRes.text();
      expect(bundleRes.status).toBe(200);
      expect(js).toContain("espadaA2UI");
    } finally {
      await server.close();
      await fs.rm(dir, { recursive: true, force: true });
    }
