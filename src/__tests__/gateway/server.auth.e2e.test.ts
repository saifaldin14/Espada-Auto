...
import { getHandshakeTimeoutMs } from "../../gateway/server-constants.js";
...
describe("gateway server auth/connect", () => {
  describe("default auth (token)", () => {
    ...
    test("closes silent handshakes after timeout", { timeout: 60_000 }, async () => {
      vi.useRealTimers();
      const prevHandshakeTimeout = process.env.ESPADA_TEST_HANDSHAKE_TIMEOUT_MS;
      process.env.ESPADA_TEST_HANDSHAKE_TIMEOUT_MS = "50";
      try {
        ...
        if (prevHandshakeTimeout === undefined) {
          delete process.env.ESPADA_TEST_HANDSHAKE_TIMEOUT_MS;
        } else {
          process.env.ESPADA_TEST_HANDSHAKE_TIMEOUT_MS = prevHandshakeTimeout;
        }
      }
    });
  ...
  describe("token auth", () => {
    ...
    beforeAll(async () => {
      prevToken = process.env.ESPADA_GATEWAY_TOKEN;
  ...
  test("requires pairing for scope upgrades", async () => {
    ...
    const { buildDeviceAuthPayload } = await import("../../gateway/device-auth.js");
    const { loadOrCreateDeviceIdentity, publicKeyRawBase64UrlFromPem, signDevicePayload } =
      await import("../../infra/device-identity.js");
    const { approveDevicePairing, getPairedDevice, listDevicePairing } =
      await import("../../infra/device-pairing.js");
    ...
    const { server, ws, port, prevToken } = await startServerWithClient("secret");
    const identityDir = await mkdtemp(join(tmpdir(), "espada-device-scope-"));
