...
import { resolveEspadaAgentDir } from "../../agents/agent-paths.js";
...
import { ensureEspadaModelsJson } from "../../agents/models-config.js";
import { loadConfig } from "../../config/config.js";
import type { EspadaConfig, ModelProviderConfig } from "../../config/types.js";
...
const GATEWAY_LIVE = isTruthyEnvValue(process.env.ESPADA_LIVE_GATEWAY);
const ZAI_FALLBACK = isTruthyEnvValue(process.env.ESPADA_LIVE_GATEWAY_ZAI_FALLBACK);
const PROVIDERS = parseFilter(process.env.ESPADA_LIVE_GATEWAY_PROVIDERS);
...
function sanitizeAuthConfig(params: {
  ...
  let profiles: NonNullable<EspadaConfig["auth"]>["profiles"] | undefined;
  if (auth.profiles) {
    profiles = {};
...
async function runGatewayModelSuite(params: GatewayModelSuiteParams) {
  const previous = {
    ...
    agentDir: process.env.ESPADA_AGENT_DIR,
    piAgentDir: process.env.PI_CODING_AGENT_DIR,
    stateDir: process.env.ESPADA_STATE_DIR,
  };
  ...
  const hostAgentDir = resolveEspadaAgentDir();
  ...
  tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "espada-live-state-"));
  ...
  process.env.ESPADA_AGENT_DIR = tempAgentDir;
  ...
  const toolProbePath = path.join(workspaceDir, `.espada-live-tool-probe.${nonceA}.txt`);
  await fs.writeFile(toolProbePath, `nonceA=${nonceA}\nnonceB=${nonceB}\n`);

  const agentDir = resolveEspadaAgentDir();
  const sanitizedCfg: EspadaConfig = {
    ...params.cfg,
  ...
  try {
        ...
        try {
          ...
          const toolProbe = await client.request<AgentFinalPayload>(
            "agent",
            {
              ...
              message:
                "Espada live tool probe (local, safe): " +
    ...
    process.env.ESPADA_CONFIG_PATH = previous.configPath;
    process.env.ESPADA_GATEWAY_TOKEN = previous.token;
    process.env.ESPADA_SKIP_CHANNELS = previous.skipChannels;
    process.env.ESPADA_SKIP_GMAIL_WATCHER = previous.skipGmail;
    process.env.ESPADA_SKIP_CRON = previous.skipCron;
    process.env.ESPADA_SKIP_CANVAS_HOST = previous.skipCanvas;
    process.env.ESPADA_AGENT_DIR = previous.agentDir;
    process.env.PI_CODING_AGENT_DIR = previous.piAgentDir;
    process.env.ESPADA_STATE_DIR = previous.stateDir;
  }
}
...
describeLive("gateway live (dev agent, profile keys)", () => {
  it(
    ...
    async () => {
      ...
      await ensureEspadaModelsJson(cfg);
      ...
      const rawModels = process.env.ESPADA_LIVE_GATEWAY_MODELS?.trim();
  ...
  it("z.ai fallback handles anthropic tool history", async () => {
    ...
    const nonceA = randomUUID();
    const nonceB = randomUUID();
    const toolProbePath = path.join(workspaceDir, `.espada-live-zai-fallback.${nonceA}.txt`);
    await fs.writeFile(toolProbePath, `nonceA=${nonceA}\nnonceB=${nonceB}\n`);
