...
import { agentCommand, getFreePort, installGatewayTestHooks } from "../../gateway/test-helpers.js";
  ...
  it("handles OpenResponses request parsing and validation", async () => {
    ...
    try {
      ...
      const resMissingAuth = await fetch(`http://127.0.0.1:${port}/v1/responses`, {
        ...
        body: JSON.stringify({ model: "espada", input: "hi" }),
      });
      ...
      const resHeader = await postResponses(
        port,
        { model: "espada", input: "hi" },
        { "x-espada-agent-id": "beta" },
      );
      ...
      mockAgentOnce([{ text: "hello" }]);
      const resModel = await postResponses(port, { model: "espada:beta", input: "hi" });
