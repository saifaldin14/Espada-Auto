...
vi.mock("../../gateway/server-browser.js", () => ({
...
vi.mock("../../gateway/config-reload.js", () => ({
...
describe("gateway hot reload", () => {
  ...
  beforeEach(() => {
    prevSkipChannels = process.env.ESPADA_SKIP_CHANNELS;
    prevSkipGmail = process.env.ESPADA_SKIP_GMAIL_WATCHER;
    process.env.ESPADA_SKIP_CHANNELS = "0";
    delete process.env.ESPADA_SKIP_GMAIL_WATCHER;
  });

  afterEach(() => {
    if (prevSkipChannels === undefined) {
      delete process.env.ESPADA_SKIP_CHANNELS;
    } else {
      process.env.ESPADA_SKIP_CHANNELS = prevSkipChannels;
    }
    if (prevSkipGmail === undefined) {
      delete process.env.ESPADA_SKIP_GMAIL_WATCHER;
    } else {
      process.env.ESPADA_SKIP_GMAIL_WATCHER = prevSkipGmail;
    }
  });
