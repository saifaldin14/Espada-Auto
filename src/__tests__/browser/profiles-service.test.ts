...
import { createBrowserProfilesService } from "../../browser/profiles-service.js";
import type { BrowserRouteContext, BrowserServerState } from "../../browser/server-context.js";
...
vi.mock("../../browser/chrome.js", () => ({
  resolveEspadaUserDataDir: vi.fn(() => "/tmp/espada-test/espada/user-data"),
}));

import { loadConfig, writeConfigFile } from "../../config/config.js";
import { resolveEspadaUserDataDir } from "../../browser/chrome.js";
...
describe("BrowserProfilesService", () => {
  ...
  it("deletes local profiles and moves data to Trash", async () => {
    ...
    const tempDir = fs.mkdtempSync(path.join("/tmp", "espada-profile-"));
    const userDataDir = path.join(tempDir, "work", "user-data");
    fs.mkdirSync(path.dirname(userDataDir), { recursive: true });
    vi.mocked(resolveEspadaUserDataDir).mockReturnValue(userDataDir);
