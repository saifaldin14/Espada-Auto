...
vi.mock("../../browser/chrome.js", () => ({
  ...
  launchEspadaChrome: vi.fn(async () => {
...
describe("browser server-context tab selection state", () => {
  it("updates lastTargetId when openTab is created via CDP", async () => {
    ...
    vi.doMock("./cdp.js", async () => {
      const actual = await vi.importActual<typeof import("../../browser/cdp.js")>("./cdp.js");
      return {
        ...actual,
    ...
    const { createBrowserRouteContext } = await import("../../browser/server-context.js");
    const state = makeState("espada");
    const ctx = createBrowserRouteContext({ getState: () => state });
    const espada = ctx.forProfile("espada");

    const opened = await espada.openTab("https://created.example");
    expect(opened.targetId).toBe("CREATED");
    expect(state.profiles.get("espada")?.lastTargetId).toBe("CREATED");
  });
});
