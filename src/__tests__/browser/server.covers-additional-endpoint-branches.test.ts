  ...
  return {
    ...
    loadConfig: () => ({
      browser: {
        ...
        headless: true,
        defaultProfile: "espada",
        profiles: {
          espada: { cdpPort: testPort + 1, color: "#FF4500" },
        },
      },
    }),
...
vi.mock("../../browser/chrome.js", () => ({
  ...
  launchEspadaChrome: vi.fn(async (_resolved: unknown, profile: { cdpPort: number }) => {
    launchCalls.push({ port: profile.cdpPort });
    reachable = true;
    return {
      pid: 123,
      exe: { kind: "chrome", path: "/fake/chrome" },
      userDataDir: "/tmp/espada",
      cdpPort: profile.cdpPort,
      startedAt: Date.now(),
      proc,
    };
  }),
  resolveEspadaUserDataDir: vi.fn(() => "/tmp/espada"),
  stopEspadaChrome: vi.fn(async () => {
    reachable = false;
  }),
}));
...
vi.mock("../../browser/pw-ai.js", () => pwMocks);
...
describe("browser control server", () => {
  beforeEach(async () => {
    ...
    prevGatewayPort = process.env.ESPADA_GATEWAY_PORT;
    process.env.ESPADA_GATEWAY_PORT = String(testPort - 2);
  ...
  afterEach(async () => {
    ...
    if (prevGatewayPort === undefined) {
      ...
      process.env.ESPADA_GATEWAY_PORT = prevGatewayPort;
    }
    const { stopBrowserControlServer } = await import("../../browser/server.js");
    await stopBrowserControlServer();
  });

  it("covers additional endpoint branches", async () => {
    const { startBrowserControlServerFromConfig } = await import("../../browser/server.js");
...
describe("backward compatibility (profile parameter)", () => {
  ...
  it("GET / without profile uses default profile", async () => {
    ...
    // Should use default profile (espada)
    expect(status.profile).toBe("espada");
  });
  ...
  it("POST /start without profile uses default profile", async () => {
    ...
    expect(result.profile).toBe("espada");
  });
  ...
  it("GET /profiles returns list of profiles", async () => {
    ...
    // Should at least have the default espada profile
    expect(result.profiles.some((p) => p.name === "espada")).toBe(true);
  });

  it("GET /tabs?profile=espada returns tabs for specified profile", async () => {
    const { startBrowserControlServerFromConfig } = await import("../../browser/server.js");
    await startBrowserControlServerFromConfig();
    const base = `http://127.0.0.1:${testPort}`;
    ...
    const result = (await realFetch(`${base}/tabs?profile=espada`).then((r) => r.json())) as {
      running: boolean;
      tabs: unknown[];
    };
    expect(result.running).toBe(true);
  ...
  it("POST /tabs/open?profile=espada opens tab in specified profile", async () => {
    const { startBrowserControlServerFromConfig } = await import("../../browser/server.js");
    await startBrowserControlServerFromConfig();
    const base = `http://127.0.0.1:${testPort}`;
    ...
    const result = (await realFetch(`${base}/tabs/open?profile=espada`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ url: "https://example.com" }),
    }).then((r) => r.json())) as { targetId?: string };
