...
import { maybeRemoveDeprecatedCliAuthProfiles } from "../../commands/doctor-auth.js";
...
beforeEach(() => {
  originalAgentDir = process.env.ESPADA_AGENT_DIR;
  originalPiAgentDir = process.env.PI_CODING_AGENT_DIR;
  tempAgentDir = fs.mkdtempSync(path.join(os.tmpdir(), "espada-auth-"));
...
afterEach(() => {
  if (originalAgentDir === undefined) {
    ...
    process.env.ESPADA_AGENT_DIR = originalAgentDir;
  }
