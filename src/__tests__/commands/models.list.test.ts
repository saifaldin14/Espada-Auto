...
const ensureEspadaModelsJson = vi.fn().mockResolvedValue(undefined);
const resolveEspadaAgentDir = vi.fn().mockReturnValue("/tmp/espada-agent");
const ensureAuthProfileStore = vi.fn().mockReturnValue({ version: 1, profiles: {} });
const listProfilesForProvider = vi.fn().mockReturnValue([]);
const resolveAuthProfileDisplayLabel = vi.fn(({ profileId }: { profileId: string }) => profileId);
const resolveAuthStorePathForDisplay = vi
  .fn()
  .mockReturnValue("/tmp/espada-agent/auth-profiles.json");
...
vi.mock("../../config/config.js", () => ({
  ...
  STATE_DIR: "/tmp/espada-state",
  loadConfig,
}));

vi.mock("../../agents/models-config.js", () => ({
  ensureEspadaModelsJson,
}));

vi.mock("../../agents/agent-paths.js", () => ({
  resolveEspadaAgentDir,
}));
...
describe("models list/status", () => {
  it("models status resolves z.ai alias to canonical zai", async () => {
    ...
    const { modelsStatusCommand } = await import("../../commands/models/list.js");
  ...
  it("models list outputs canonical zai key for configured z.ai model", async () => {
    ...
    const { modelsListCommand } = await import("../../commands/models/list.js");
