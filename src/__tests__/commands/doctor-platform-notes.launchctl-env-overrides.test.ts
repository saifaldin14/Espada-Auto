...
import { noteMacLaunchctlGatewayEnvOverrides } from "../../commands/doctor-platform-notes.js";
...
describe("noteMacLaunchctlGatewayEnvOverrides", () => {
  it("prints clear unsetenv instructions for token override", async () => {
    ...
    const getenv = vi.fn(async (name: string) =>
      name === "ESPADA_GATEWAY_TOKEN" ? "launchctl-token" : undefined,
    );
    const cfg = {
      gateway: {
        auth: {
    ...
    expect(message).toContain("ESPADA_GATEWAY_TOKEN");
    expect(message).toContain("launchctl unsetenv ESPADA_GATEWAY_TOKEN");
    expect(message).not.toContain("ESPADA_GATEWAY_PASSWORD");
  });
