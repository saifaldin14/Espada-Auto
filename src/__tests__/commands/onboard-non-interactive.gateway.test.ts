...
describe("onboard (non-interactive): gateway and remote auth", () => {
  const prev = {
    ...
    skipBrowser: process.env.ESPADA_SKIP_BROWSER_CONTROL_SERVER,
    token: process.env.ESPADA_GATEWAY_TOKEN,
    password: process.env.ESPADA_GATEWAY_PASSWORD,
  };
  ...
  const initStateDir = async (prefix: string) => {
    ...
    process.env.ESPADA_STATE_DIR = stateDir;
  ...
  beforeAll(async () => {
    ...
    tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "espada-onboard-"));
  ...
  afterAll(async () => {
    ...
    process.env.ESPADA_STATE_DIR = prev.stateDir;
    process.env.ESPADA_CONFIG_PATH = prev.configPath;
    process.env.ESPADA_SKIP_CHANNELS = prev.skipChannels;
    process.env.ESPADA_SKIP_GMAIL_WATCHER = prev.skipGmail;
    process.env.ESPADA_SKIP_CRON = prev.skipCron;
    process.env.ESPADA_SKIP_CANVAS_HOST = prev.skipCanvas;
    process.env.ESPADA_SKIP_BROWSER_CONTROL_SERVER = prev.skipBrowser;
    process.env.ESPADA_GATEWAY_TOKEN = prev.token;
    process.env.ESPADA_GATEWAY_PASSWORD = prev.password;
  });
  ...
  it("writes gateway token auth into config and gateway enforces it", async () => {
    ...
    const workspace = path.join(stateDir, "espada");

    const { runNonInteractiveOnboarding } = await import("../../commands/onboard-non-interactive.js");
  ...
  it("auto-generates token auth when binding LAN and persists the token", async () => {
    ...
    process.env.ESPADA_STATE_DIR = stateDir;
    process.env.ESPADA_CONFIG_PATH = path.join(stateDir, "espada.json");
