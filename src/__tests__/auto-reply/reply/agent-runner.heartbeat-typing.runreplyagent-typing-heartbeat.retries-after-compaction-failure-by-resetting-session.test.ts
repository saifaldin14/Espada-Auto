...
vi.mock("../../../auto-reply/reply/queue.js", async () => {
  const actual = await vi.importActual<typeof import("../../../auto-reply/reply/queue.js")>("./queue.js");
  return {
    ...actual,
...
describe("runReplyAgent typing (heartbeat)", () => {
  ...
  it("retries after compaction failure by resetting the session", async () => {
    ...
    const stateDir = await fs.mkdtemp(path.join(tmpdir(), "espada-session-compaction-reset-"));
  ...
  it("retries after context overflow payload by resetting the session", async () => {
    const prevStateDir = process.env.ESPADA_STATE_DIR;
    const stateDir = await fs.mkdtemp(path.join(tmpdir(), "espada-session-overflow-reset-"));
    process.env.ESPADA_STATE_DIR = stateDir;
    try {
      const sessionId = "session";
      const storePath = path.join(stateDir, "sessions", "sessions.json");
  ...
  it("resets the session after role ordering payloads", async () => {
    const prevStateDir = process.env.ESPADA_STATE_DIR;
    const stateDir = await fs.mkdtemp(path.join(tmpdir(), "espada-session-role-ordering-"));
    process.env.ESPADA_STATE_DIR = stateDir;
    try {
      const sessionId = "session";
      const storePath = path.join(stateDir, "sessions", "sessions.json");
