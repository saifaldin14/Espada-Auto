...
import { isAbortTrigger, tryFastAbortFromMessage } from "../../../auto-reply/reply/abort.js";
import { enqueueFollowupRun, getFollowupQueueDepth, type FollowupRun } from "../../../auto-reply/reply/queue.js";
...
describe("abort detection", () => {
  it("triggerBodyNormalized extracts /stop from RawBody for abort detection", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "espada-abort-"));
  ...
  it("fast-aborts even when text commands are disabled", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "espada-abort-"));
    const storePath = path.join(root, "sessions.json");
    const cfg = { session: { store: storePath }, commands: { text: false } } as EspadaConfig;
