...
import { resetBashChatCommandForTests } from "../../../auto-reply/reply/bash-command.js";
...
vi.mock("../../../auto-reply/reply/commands-context-report.js", () => ({
...
beforeAll(async () => {
  testWorkspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "espada-commands-"));
...
function buildParams(commandBody: string, cfg: EspadaConfig, ctxOverrides?: Partial<MsgContext>) {
  const ctx = {
    Body: commandBody,
