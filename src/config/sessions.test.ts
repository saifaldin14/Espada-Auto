...
describe("sessions", () => {
  ...
  it("builds discord display name with guild+channel slugs", () => {
    expect(
      buildGroupDisplayName({
        ...
        space: "friends-of-espada",
        id: "123",
        key: "discord:group:123",
      }),
    ).toBe("discord:friends-of-espada#general");
  });
  ...
  it("updateLastRoute persists channel and target", async () => {
    ...
    const dir = await fs.mkdtemp(path.join(os.tmpdir(), "espada-sessions-"));
  ...
  it("derives session transcripts dir from ESPADA_STATE_DIR", () => {
    const dir = resolveSessionTranscriptsDir(
      { ESPADA_STATE_DIR: "/custom/state" } as NodeJS.ProcessEnv,
      () => "/home/ignored",
    );
  ...
  it("includes topic ids in session transcript filenames", () => {
    ...
    process.env.ESPADA_STATE_DIR = "/custom/state";
    try {
