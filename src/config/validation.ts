...
import type { EspadaConfig, ConfigValidationIssue } from "./types.js";
...
function validateIdentityAvatar(config: EspadaConfig): ConfigValidationIssue[] {
...
export function validateConfigObject(
  raw: unknown,
): { ok: true; config: EspadaConfig } | { ok: false; issues: ConfigValidationIssue[] } {
  ...
  const validated = EspadaSchema.safeParse(raw);
  ...
  const duplicates = findDuplicateAgentDirs(validated.data as EspadaConfig);
  ...
  const avatarIssues = validateIdentityAvatar(validated.data as EspadaConfig);
  ...
  return {
    ...
    config: applyModelDefaults(
      applyAgentDefaults(applySessionDefaults(validated.data as EspadaConfig)),
    ),
  };
}
