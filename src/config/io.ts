...
import type { EspadaConfig, ConfigFileSnapshot, LegacyConfigIssue } from "./types.js";
import { validateConfigObjectWithPlugins } from "./validation.js";
import { compareEspadaVersions } from "./version.js";
...
const SHELL_ENV_EXPECTED_KEYS = [
  ...
  "SLACK_BOT_TOKEN",
  "SLACK_APP_TOKEN",
  "ESPADA_GATEWAY_TOKEN",
  "ESPADA_GATEWAY_PASSWORD",
];
...
function coerceConfig(value: unknown): EspadaConfig {
  if (!value || typeof value !== "object" || Array.isArray(value)) {
    return {};
  }
  return value as EspadaConfig;
}
...
function stampConfigVersion(cfg: EspadaConfig): EspadaConfig {
...
function warnIfConfigFromFuture(cfg: EspadaConfig, logger: Pick<typeof console, "warn">): void {
  const touched = cfg.meta?.lastTouchedVersion;
  if (!touched) return;
  const cmp = compareEspadaVersions(VERSION, touched);
  if (cmp === null) return;
  if (cmp < 0) {
    logger.warn(
      `Config was last written by a newer Espada (${touched}); current version is ${VERSION}.`,
    );
  }
}

function applyConfigEnv(cfg: EspadaConfig, env: NodeJS.ProcessEnv): void {
...
export function createConfigIO(overrides: ConfigIoDeps = {}) {
  ...
  function loadConfig(): EspadaConfig {
    try {
      ...
      if (resolved && typeof resolved === "object" && "env" in resolved) {
        applyConfigEnv(resolved as EspadaConfig, deps.env);
      }
      ...
      const preValidationDuplicates = findDuplicateAgentDirs(resolvedConfig as EspadaConfig, {
        env: deps.env,
  ...
  async function writeConfigFile(cfg: EspadaConfig) {
    clearConfigCache();
...
// module scope. `ESPADA_CONFIG_PATH` (and friends) are expected to work even
...
function resolveConfigCacheMs(env: NodeJS.ProcessEnv): number {
  const raw = env.ESPADA_CONFIG_CACHE_MS?.trim();
...
function shouldUseConfigCache(env: NodeJS.ProcessEnv): boolean {
  if (env.ESPADA_DISABLE_CONFIG_CACHE?.trim()) return false;
...
export function loadConfig(): EspadaConfig {
...
export async function writeConfigFile(cfg: EspadaConfig): Promise<void> {
  clearConfigCache();
