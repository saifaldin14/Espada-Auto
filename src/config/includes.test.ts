...
const ETC_ESPADA_DIR = path.join(ROOT_DIR, "etc", "espada");
const SHARED_DIR = path.join(ROOT_DIR, "shared");

const DEFAULT_BASE_PATH = path.join(CONFIG_DIR, "espada.json");

function configPath(...parts: string[]) {
  return path.join(CONFIG_DIR, ...parts);
}

function etcEspadaPath(...parts: string[]) {
  return path.join(ETC_ESPADA_DIR, ...parts);
}
...
describe("resolveConfigIncludes", () => {
  ...
  it("resolves absolute path $include", () => {
    const absolute = etcEspadaPath("agents.json");
  ...
  it("resolves parent directory references", () => {
    ...
    expect(resolve(obj, files, configPath("sub", "espada.json"))).toEqual({
      shared: true,
    });
  });
});
