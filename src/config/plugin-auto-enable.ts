import type { EspadaConfig } from "./config.js";
import {
  getChatChannelMeta,
  listChatChannels,
...
function isTelegramConfigured(cfg: EspadaConfig, env: NodeJS.ProcessEnv): boolean {
...
function isDiscordConfigured(cfg: EspadaConfig, env: NodeJS.ProcessEnv): boolean {
...
function isSlackConfigured(cfg: EspadaConfig, env: NodeJS.ProcessEnv): boolean {
  if (
...
function isSignalConfigured(cfg: EspadaConfig): boolean {
...
function isIMessageConfigured(cfg: EspadaConfig): boolean {
...
function isWhatsAppConfigured(cfg: EspadaConfig): boolean {
...
function isGenericChannelConfigured(cfg: EspadaConfig, channelId: string): boolean {
...
function collectModelRefs(cfg: EspadaConfig): string[] {
  const refs: string[] = [];
...
function isProviderConfigured(cfg: EspadaConfig, providerId: string): boolean {
...
function isPluginExplicitlyDisabled(cfg: EspadaConfig, pluginId: string): boolean {
  const entry = cfg.plugins?.entries?.[pluginId];
  return entry?.enabled === false;
}

function isPluginDenied(cfg: EspadaConfig, pluginId: string): boolean {
...
function ensureAllowlisted(cfg: EspadaConfig, pluginId: string): EspadaConfig {
...
function enablePluginEntry(cfg: EspadaConfig, pluginId: string): EspadaConfig {
  const entries = {
    ...cfg.plugins?.entries,
    [pluginId]: {
