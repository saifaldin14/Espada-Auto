...
describe("hasAnyWhatsAppAuth", () => {
  ...
  beforeEach(() => {
    previousOauthDir = process.env.ESPADA_OAUTH_DIR;
    tempOauthDir = fs.mkdtempSync(path.join(os.tmpdir(), "espada-oauth-"));
    process.env.ESPADA_OAUTH_DIR = tempOauthDir;
  });
  ...
  afterEach(() => {
    if (previousOauthDir === undefined) {
      ...
      process.env.ESPADA_OAUTH_DIR = previousOauthDir;
    }
    if (tempOauthDir) {
  ...
  it("includes authDir overrides", () => {
    const customDir = fs.mkdtempSync(path.join(os.tmpdir(), "espada-wa-auth-"));
    try {
