...
function listConfiguredAccountIds(cfg: EspadaConfig): string[] {
  const accounts = cfg.channels?.whatsapp?.accounts;
  if (!accounts || typeof accounts !== "object") return [];
  return Object.keys(accounts).filter(Boolean);
}

export function listWhatsAppAuthDirs(cfg: EspadaConfig): string[] {
...
export function hasAnyWhatsAppAuth(cfg: EspadaConfig): boolean {
  return listWhatsAppAuthDirs(cfg).some((authDir) => hasWebCredsSync(authDir));
}

export function listWhatsAppAccountIds(cfg: EspadaConfig): string[] {
  const ids = listConfiguredAccountIds(cfg);
  if (ids.length === 0) return [DEFAULT_ACCOUNT_ID];
  return ids.sort((a, b) => a.localeCompare(b));
}

export function resolveDefaultWhatsAppAccountId(cfg: EspadaConfig): string {
...
export function resolveWhatsAppAuthDir(params: { cfg: EspadaConfig; accountId: string }): {
  authDir: string;
  isLegacy: boolean;
} {
...
export function listEnabledWhatsAppAccounts(cfg: EspadaConfig): ResolvedWhatsAppAccount[] {
