...
describe("docker-setup.sh", () => {
  it("handles unset optional env vars under strict mode", async () => {
    ...
    const rootDir = await mkdtemp(join(tmpdir(), "espada-docker-setup-"));
    ...
    expect(envFile).toContain("ESPADA_DOCKER_APT_PACKAGES=");
    expect(envFile).toContain("ESPADA_EXTRA_MOUNTS=");
    expect(envFile).toContain("ESPADA_HOME_VOLUME=");
  });

  it("plumbs ESPADA_DOCKER_APT_PACKAGES into .env and docker build args", async () => {
    const assocCheck = spawnSync("bash", ["-c", "declare -A _t=()"], {
      encoding: "utf8",
    });
    if (assocCheck.status !== 0) {
    ...
    const envFile = await readFile(join(rootDir, ".env"), "utf8");
    expect(envFile).toContain("ESPADA_DOCKER_APT_PACKAGES=ffmpeg build-essential");

    const log = await readFile(logPath, "utf8");
    expect(log).toContain("--build-arg ESPADA_DOCKER_APT_PACKAGES=ffmpeg build-essential");
  });
