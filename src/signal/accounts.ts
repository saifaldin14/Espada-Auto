...
export function listSignalAccountIds(cfg: EspadaConfig): string[] {
  const ids = listConfiguredAccountIds(cfg);
  if (ids.length === 0) return [DEFAULT_ACCOUNT_ID];
  return ids.sort((a, b) => a.localeCompare(b));
}

export function resolveDefaultSignalAccountId(cfg: EspadaConfig): string {
...
function mergeSignalAccountConfig(cfg: EspadaConfig, accountId: string): SignalAccountConfig {
...
export function listEnabledSignalAccounts(cfg: EspadaConfig): ResolvedSignalAccount[] {
