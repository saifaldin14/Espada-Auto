...
import type { EspadaConfig, GatewayBindMode } from "../config/config.js";
...
export async function noteSecurityWarnings(cfg: EspadaConfig) {
  const warnings: string[] = [];
  const auditHint = `- Run: ${formatCliCommand("espada security audit --deep")}`;
    ...
    if (!hasSharedSecret) {
      const authFixLines =
        resolvedAuth.mode === "password"
          ? [
              `  Fix: ${formatCliCommand("espada configure")} to set a password`,
              `  Or switch to token: ${formatCliCommand("espada config set gateway.auth.mode token")}`,
            ]
          : [
              `  Fix: ${formatCliCommand("espada doctor --fix")} to generate a token`,
      ...
      warnings.push(
        ...
        `  Fix: ${formatCliCommand("espada config set gateway.bind loopback")}`,
        ...authFixLines,
      );
    } else {
