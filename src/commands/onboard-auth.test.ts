...
const requireAgentDir = () => {
  const agentDir = process.env.ESPADA_AGENT_DIR;
  if (!agentDir) throw new Error("ESPADA_AGENT_DIR not set");
  return agentDir;
};
...
describe("writeOAuthCredentials", () => {
  ...
  afterEach(async () => {
    ...
    delete process.env.ESPADA_OAUTH_DIR;
  });

  it("writes auth-profiles.json under ESPADA_AGENT_DIR when set", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "espada-oauth-"));
    process.env.ESPADA_STATE_DIR = tempStateDir;
    process.env.ESPADA_AGENT_DIR = path.join(tempStateDir, "agent");
    process.env.PI_CODING_AGENT_DIR = process.env.ESPADA_AGENT_DIR;
...
describe("setMinimaxApiKey", () => {
  ...
  it("writes to ESPADA_AGENT_DIR when set", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "espada-minimax-"));
    process.env.ESPADA_STATE_DIR = tempStateDir;
    process.env.ESPADA_AGENT_DIR = path.join(tempStateDir, "custom-agent");
    process.env.PI_CODING_AGENT_DIR = process.env.ESPADA_AGENT_DIR;
