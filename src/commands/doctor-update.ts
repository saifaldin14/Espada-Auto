...
async function detectEspadaGitCheckout(root: string): Promise<"git" | "not-git" | "unknown"> {
...
export async function maybeOfferUpdateBeforeDoctor(params: {
  ...
  const updateInProgress = isTruthyEnvValue(process.env.ESPADA_UPDATE_IN_PROGRESS);
  ...
  const git = await detectEspadaGitCheckout(params.root);
  if (git === "git") {
    const shouldUpdate = await params.confirm({
      message: "Update Espada from git before running doctor?",
      initialValue: true,
    });
  ...
  if (git === "not-git") {
    note(
      [
        ...
        `Run \`${formatCliCommand("espada update")}\` to update via your package manager (npm/pnpm), then rerun doctor.`,
      ].join("\n"),
      "Update",
    );
  }
