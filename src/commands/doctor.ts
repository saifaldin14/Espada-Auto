...
function resolveMode(cfg: EspadaConfig): "local" | "remote" {
...
export async function doctorCommand(
  ...
  const root = await resolveEspadaPackageRoot({
  ...
  let cfg: EspadaConfig = configResult.cfg;

  const configPath = configResult.path ?? CONFIG_PATH;
  if (!cfg.gateway?.mode) {
    const lines = [
      "gateway.mode is unset; gateway start will be blocked.",
      `Fix: run ${formatCliCommand("espada configure")} and set Gateway mode (local/remote).`,
      `Or set directly: ${formatCliCommand("espada config set gateway.mode local")}`,
    ];
    if (!fs.existsSync(configPath)) {
      lines.push(`Missing config: run ${formatCliCommand("espada setup")} first.`);
    }
    note(lines.join("\n"), "Gateway");
  }
  ...
  if (shouldWriteConfig) {
    ...
    runtime.log(`Run "${formatCliCommand("espada doctor --fix")}" to apply changes.`);
  }
