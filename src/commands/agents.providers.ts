...
function resolveDefaultAccountId(cfg: EspadaConfig, provider: ChannelId): string {
  const plugin = getChannelPlugin(provider);
  if (!plugin) return DEFAULT_ACCOUNT_ID;
  return resolveChannelDefaultAccountId({ plugin, cfg });
}

function shouldShowProviderEntry(entry: ProviderAccountStatus, cfg: EspadaConfig): boolean {
...
export function summarizeBindings(cfg: EspadaConfig, bindings: AgentBinding[]): string[] {
