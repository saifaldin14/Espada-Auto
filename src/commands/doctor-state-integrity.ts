...
function findOtherStateDirs(stateDir: string): string[] {
  ...
  for (const root of roots) {
    ...
    for (const entry of entries) {
      ...
      const candidate = path.resolve(root, entry.name, ".espada");
...
export async function noteStateIntegrity(
  ...
  const defaultStateDir = path.join(homedir(), ".espada");
...
export function noteWorkspaceBackupTip(workspaceDir: string) {
  ...
  note(
    [
      ...
      "- Keep ~/.espada out of git; it contains credentials and session history.",
