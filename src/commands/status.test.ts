...
beforeAll(() => {
  previousProfile = process.env.ESPADA_PROFILE;
  process.env.ESPADA_PROFILE = "isolated";
});
...
afterAll(() => {
  if (previousProfile === undefined) {
    ...
    process.env.ESPADA_PROFILE = previousProfile;
  }
});
...
vi.mock("../infra/espada-root.js", () => ({
  resolveEspadaPackageRoot: vi.fn().mockResolvedValue("/tmp/espada"),
}));
...
vi.mock("../infra/update-check.js", () => ({
  checkUpdateStatus: vi.fn().mockResolvedValue({
    ...
    deps: {
      manager: "pnpm",
      status: "ok",
      lockfilePath: "/tmp/espada/pnpm-lock.yaml",
      markerPath: "/tmp/espada/node_modules/.modules.yaml",
    },
...
describe("statusCommand", () => {
  ...
  it("prints formatted lines otherwise", async () => {
    ...
    expect(logs.some((l) => l.includes("Espada status"))).toBe(true);
    ...
    expect(
      logs.some(
        (l) =>
          l.includes("espada status --all") ||
          l.includes("espada --profile isolated status --all") ||
          l.includes("espada status --all") ||
          l.includes("espada --profile isolated status --all"),
      ),
    ).toBe(true);
  ...
  it("shows gateway auth when reachable", async () => {
    ...
    process.env.ESPADA_GATEWAY_TOKEN = "abcd1234";
    try {
