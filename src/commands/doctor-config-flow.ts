...
function stripUnknownConfigKeys(config: EspadaConfig): {
  config: EspadaConfig;
  removed: string[];
} {
  const parsed = EspadaSchema.safeParse(config);
  if (parsed.success) {
    return { config, removed: [] };
  }

  const next = structuredClone(config) as EspadaConfig;
...
function noteOpencodeProviderOverrides(cfg: EspadaConfig) {
...
function hasExplicitConfigPath(env: NodeJS.ProcessEnv): boolean {
  return Boolean(env.ESPADA_CONFIG_PATH?.trim() || env.ESPADA_CONFIG_PATH?.trim());
}
...
export async function loadAndMaybeMigrateDoctorConfig(params: {
  ...
  let cfg: EspadaConfig = baseCfg;
  let candidate = structuredClone(baseCfg) as EspadaConfig;
  ...
  if (normalized.changes.length > 0) {
    ...
    if (shouldRepair) {
      ...
      fixHints.push(`Run "${formatCliCommand("espada doctor --fix")}" to apply these changes.`);
    }
  }
  ...
  if (unknown.removed.length > 0) {
    ...
    if (shouldRepair) {
      ...
      fixHints.push('Run "espada doctor --fix" to remove these keys.');
    }
  }
