...
export async function noteMacLaunchAgentOverrides() {
  ...
  const markerPath = path.join(resolveHomeDir(), ".espada", "disable-launchagent");
...
function hasConfigGatewayCreds(cfg: EspadaConfig): boolean {
  const localToken =
...
export async function noteMacLaunchctlGatewayEnvOverrides(
  ...
  const envToken = await getenv("ESPADA_GATEWAY_TOKEN");
  const envPassword = await getenv("ESPADA_GATEWAY_PASSWORD");
  if (!envToken && !envPassword) return;

  const lines = [
    "- launchctl environment overrides detected (can cause confusing unauthorized errors).",
    envToken ? "- `ESPADA_GATEWAY_TOKEN` is set; it overrides config tokens." : undefined,
    envPassword
      ? "- `ESPADA_GATEWAY_PASSWORD` is set; it overrides config passwords."
      : undefined,
    "- Clear overrides and restart the app/gateway:",
    envToken ? "  launchctl unsetenv ESPADA_GATEWAY_TOKEN" : undefined,
    envPassword ? "  launchctl unsetenv ESPADA_GATEWAY_PASSWORD" : undefined,
  ].filter((line): line is string => Boolean(line));
