...
function resolveConfiguredModelRaw(cfg: EspadaConfig): string {
  const raw = cfg.agents?.defaults?.model as { primary?: string } | string | undefined;
  if (typeof raw === "string") return raw.trim();
  return raw?.primary?.trim() ?? "";
}

function resolveConfiguredModelKeys(cfg: EspadaConfig): string[] {
...
export function applyPrimaryModel(cfg: EspadaConfig, model: string): EspadaConfig {
...
export function applyModelAllowlist(cfg: EspadaConfig, models: string[]): EspadaConfig {
  const defaults = cfg.agents?.defaults;
