...
const mocks = vi.hoisted(() => {
  ...
  return {
    store,
    resolveEspadaAgentDir: vi.fn().mockReturnValue("/tmp/espada-agent"),
...
vi.mock("../../agents/agent-paths.js", () => ({
  resolveEspadaAgentDir: mocks.resolveEspadaAgentDir,
}));
...
describe("modelsStatusCommand auth overview", () => {
  it("includes masked auth sources in JSON output", async () => {
    ...
    expect(payload.auth.storePath).toBe("/tmp/espada-agent/auth-profiles.json");
