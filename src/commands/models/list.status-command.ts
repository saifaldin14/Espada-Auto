...
export async function modelsStatusCommand(
  ...
  if (missingProvidersInUse.length > 0) {
    ...
    for (const provider of missingProvidersInUse) {
      const hint =
        provider === "anthropic"
          ? `Run \`claude setup-token\`, then \`${formatCliCommand("espada models auth setup-token")}\` or \`${formatCliCommand("espada configure")}\`.`
          : `Run \`${formatCliCommand("espada configure")}\` or set an API key env var.`;
