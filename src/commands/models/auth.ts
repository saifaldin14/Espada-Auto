...
import { readConfigFileSnapshot, type EspadaConfig } from "../../config/config.js";
...
function applyDefaultModel(cfg: EspadaConfig, model: string): EspadaConfig {
...
export async function modelsAuthLoginCommand(opts: LoginOptions, runtime: RuntimeEnv) {
  ...
  if (providers.length === 0) {
    throw new Error(
      `No provider plugins found. Install one via \`${formatCliCommand("espada plugins install")}\`.`,
    );
  }
