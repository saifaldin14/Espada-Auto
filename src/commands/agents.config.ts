...
export function listAgentEntries(cfg: EspadaConfig): AgentEntry[] {
...
function resolveAgentName(cfg: EspadaConfig, agentId: string) {
...
function resolveAgentModel(cfg: EspadaConfig, agentId: string) {
  const entry = listAgentEntries(cfg).find(
    (agent) => normalizeAgentId(agent.id) === normalizeAgentId(agentId),
  );
  if (entry?.model) {
...
export function buildAgentSummaries(cfg: EspadaConfig): AgentSummary[] {
