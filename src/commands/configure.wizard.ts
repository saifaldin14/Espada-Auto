    ...
    await select({
      ...
      options: [
        ...
        },
        {
          ...
          hint: "Delete channel tokens/settings from espada.json",
        },
      ],
...
export async function runConfigureWizard(
  ...
  try {
    ...
    intro(opts.command === "update" ? "Espada update wizard" : "Espada configure");
    ...
    if (snapshot.exists) {
      ...
      if (!snapshot.valid) {
        outro(
          `Config invalid. Run \`${formatCliCommand("espada doctor")}\` to repair it, then re-run configure.`,
        );
        runtime.exit(1);
        return;
      }
    ...
    const newPassword = nextConfig.gateway?.auth?.password ?? process.env.ESPADA_GATEWAY_PASSWORD;
    const oldPassword = baseConfig.gateway?.auth?.password ?? process.env.ESPADA_GATEWAY_PASSWORD;
    const token = nextConfig.gateway?.auth?.token ?? process.env.ESPADA_GATEWAY_TOKEN;
