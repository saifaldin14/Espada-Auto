...
beforeEach(() => {
  ...
  resolveEspadaPackageRoot.mockReset().mockResolvedValue(null);
  ...
  setStdinTty(true);
  originalStateDir = process.env.ESPADA_STATE_DIR;
  originalUpdateInProgress = process.env.ESPADA_UPDATE_IN_PROGRESS;
  process.env.ESPADA_UPDATE_IN_PROGRESS = "1";
  tempStateDir = fs.mkdtempSync(path.join(os.tmpdir(), "espada-doctor-state-"));
...
afterEach(() => {
  ...
  if (originalStateDir === undefined) {
    ...
    process.env.ESPADA_STATE_DIR = originalStateDir;
  }
  if (originalUpdateInProgress === undefined) {
    delete process.env.ESPADA_UPDATE_IN_PROGRESS;
  } else {
    process.env.ESPADA_UPDATE_IN_PROGRESS = originalUpdateInProgress;
  }
  if (tempStateDir) {
...
const resolveEspadaPackageRoot = vi.fn().mockResolvedValue(null);
...
vi.mock("../plugins/loader.js", () => ({
  loadEspadaPlugins: () => ({ plugins: [], diagnostics: [] }),
}));
...
vi.mock("../infra/espada-root.js", () => ({
  resolveEspadaPackageRoot,
}));
...
describe("doctor command", () => {
  it("warns when the state directory is missing", async () => {
    ...
    const missingDir = fs.mkdtempSync(path.join(os.tmpdir(), "espada-missing-state-"));
    fs.rmSync(missingDir, { recursive: true, force: true });
    process.env.ESPADA_STATE_DIR = missingDir;
    note.mockClear();
  ...
  it("skips gateway auth warning when ESPADA_GATEWAY_TOKEN is set", async () => {
    readConfigFileSnapshot.mockResolvedValue({
      path: "/tmp/espada.json",
      exists: true,
      raw: "{}",
    ...
    process.env.ESPADA_GATEWAY_TOKEN = "env-token-1234567890";
    note.mockClear();
