...
export async function statusCommand(
  ...
  runtime.log(theme.heading("Espada status"));
  runtime.log("");
  ...
  runtime.log(theme.muted(`Full report: ${formatCliCommand("espada security audit")}`));
  runtime.log(theme.muted(`Deep probe: ${formatCliCommand("espada security audit --deep")}`));
  ...
  runtime.log("");
  runtime.log("FAQ: https://docs.espada.dev/faq");
  runtime.log("Troubleshooting: https://docs.espada.dev/troubleshooting");
  runtime.log("");
  ...
  runtime.log(`  Need to share?      ${formatCliCommand("espada status --all")}`);
  runtime.log(`  Need to debug live? ${formatCliCommand("espada logs --follow")}`);
  if (gatewayReachable) {
    runtime.log(`  Need to test channels? ${formatCliCommand("espada status --deep")}`);
  } else {
    runtime.log(`  Fix reachability first: ${formatCliCommand("espada gateway probe")}`);
  }
}
