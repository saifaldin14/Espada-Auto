...
const execHostEnforced = process.env.ESPADA_NODE_EXEC_HOST?.trim().toLowerCase() === "app";
const execHostFallbackAllowed =
  process.env.ESPADA_NODE_EXEC_FALLBACK?.trim().toLowerCase() !== "0";
...
function ensureNodePathEnv(): string {
  ensureEspadaCliOnPath({ pathEnv: process.env.PATH ?? "" });
