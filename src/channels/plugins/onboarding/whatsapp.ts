...
function setWhatsAppDmPolicy(cfg: EspadaConfig, dmPolicy: DmPolicy): EspadaConfig {
  return mergeWhatsAppConfig(cfg, { dmPolicy });
}

function setWhatsAppAllowFrom(cfg: EspadaConfig, allowFrom?: string[]): EspadaConfig {
  return mergeWhatsAppConfig(cfg, { allowFrom }, { unsetOnUndefined: ["allowFrom"] });
}

function setWhatsAppSelfChatMode(cfg: EspadaConfig, selfChatMode: boolean): EspadaConfig {
...
async function detectWhatsAppLinked(cfg: EspadaConfig, accountId: string): Promise<boolean> {
...
async function promptWhatsAppAllowFrom(
  ...
  if (options?.forceAllowlist) {
    await prompter.note(
      "We need the sender/owner number so Espada can allowlist you.",
      "WhatsApp number",
    );
  ...
  const phoneMode = (await prompter.select({
    ...
    options: [
      ...
      { value: "separate", label: "Separate phone just for Espada" },
    ],
...
export const whatsappOnboardingAdapter: ChannelOnboardingAdapter = {
  ...
  configure: async ({
    ...
    if (wantsLink) {
      ...
      await prompter.note(
        `Run \`${formatCliCommand("espada channels login")}\` later to link WhatsApp.`,
        "WhatsApp",
      );
    }
