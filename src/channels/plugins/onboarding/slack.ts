...
function setSlackDmPolicy(cfg: EspadaConfig, dmPolicy: DmPolicy) {
  const allowFrom =
...
function buildSlackManifest(botName: string) {
  const safeName = botName.trim() || "Espada";
  const manifest = {
    display_information: {
      name: safeName,
      description: `${safeName} connector for Espada`,
    },
    features: {
      bot_user: {
...
function setSlackAllowFrom(cfg: EspadaConfig, allowFrom: string[]): EspadaConfig {
  return {
    ...cfg,
    channels: {
      ...cfg.channels,
