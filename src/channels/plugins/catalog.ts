...
import { discoverEspadaPlugins } from "../../plugins/discovery.js";
import type { PluginOrigin } from "../../plugins/types.js";
import type { EspadaPackageManifest } from "../../plugins/manifest.js";
...
const ENV_CATALOG_PATHS = ["ESPADA_PLUGIN_CATALOG_PATHS", "ESPADA_MPM_CATALOG_PATHS"];
...
function toChannelMeta(params: {
  channel: NonNullable<EspadaPackageManifest["channel"]>;
  id: string;
}): ChannelMeta | null {
...
function resolveInstallInfo(params: {
  manifest: EspadaPackageManifest;
  packageName?: string;
  packageDir?: string;
  workspaceDir?: string;
...
function buildCatalogEntry(candidate: {
  ...
  packageDir?: string;
  workspaceDir?: string;
  packageEspada?: EspadaPackageManifest;
}): ChannelPluginCatalogEntry | null {
  const manifest = candidate.packageEspada;
...
function buildExternalCatalogEntry(entry: ExternalCatalogEntry): ChannelPluginCatalogEntry | null {
  const manifest = entry.espada ?? entry[LEGACY_MANIFEST_KEY];
  return buildCatalogEntry({
    packageName: entry.name,
    packageEspada: manifest,
  });
}
...
export function listChannelPluginCatalogEntries(
  ...
  const discovery = discoverEspadaPlugins({ workspaceDir: options.workspaceDir });
