...
export function resolveEspadaUserDataDir(profileName = DEFAULT_ESPADA_BROWSER_PROFILE_NAME) {
...
export async function launchEspadaChrome(
  ...
  const userDataDir = resolveEspadaUserDataDir(profile.name);
  fs.mkdirSync(userDataDir, { recursive: true });

  const needsDecorate = !isProfileDecorated(
    userDataDir,
    profile.name,
    (profile.color ?? DEFAULT_ESPADA_BROWSER_COLOR).toUpperCase(),
  );
  ...
  if (needsDecorate) {
    try {
      decorateEspadaProfile(userDataDir, {
        name: profile.name,
        color: profile.color,
      });
      log.info(`🦞 espada browser profile decorated (${profile.color})`);
    } catch (err) {
      log.warn(`espada browser profile decoration failed: ${String(err)}`);
    }
  }
  ...
  try {
    ...
    log.warn(`espada browser clean-exit prefs failed: ${String(err)}`);
  }
  ...
  log.info(
    `🦞 espada browser started (${exe.kind}) profile "${profile.name}" on 127.0.0.1:${profile.cdpPort} (pid ${pid})`,
  );
...
export async function stopEspadaChrome(running: RunningChrome, timeoutMs = 2500) {
