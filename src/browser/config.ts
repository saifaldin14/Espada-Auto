import type { BrowserConfig, BrowserProfileConfig, EspadaConfig } from "../config/config.js";
import {
...
import {
  DEFAULT_ESPADA_BROWSER_COLOR,
  DEFAULT_ESPADA_BROWSER_ENABLED,
  DEFAULT_BROWSER_EVALUATE_ENABLED,
  DEFAULT_BROWSER_DEFAULT_PROFILE_NAME,
  DEFAULT_ESPADA_BROWSER_PROFILE_NAME,
} from "./constants.js";
...
function normalizeHexColor(raw: string | undefined) {
  ...
  if (!value) return DEFAULT_ESPADA_BROWSER_COLOR;
  const normalized = value.startsWith("#") ? value : `#${value}`;
  if (!/^#[0-9a-fA-F]{6}$/.test(normalized)) return DEFAULT_ESPADA_BROWSER_COLOR;
...
/**
 * Ensure the default "espada" profile exists in the profiles map.
...
function ensureDefaultProfile(
  ...
  if (!result[DEFAULT_ESPADA_BROWSER_PROFILE_NAME]) {
    result[DEFAULT_ESPADA_BROWSER_PROFILE_NAME] = {
 ...
 *
 * Note: this is an Espada browser profile (routing config), not a Chrome user profile.
...
function ensureDefaultChromeExtensionProfile(
  ...
  // (legacy single-profile configs may use controlPort+1 for espada CDP).
...
export function resolveBrowserConfig(
  ...
  rootConfig?: EspadaConfig,
): ResolvedBrowserConfig {
  const enabled = cfg?.enabled ?? DEFAULT_ESPADA_BROWSER_ENABLED;
...
export function resolveProfile(
  ...
  let cdpUrl = "";
  const driver = profile.driver === "extension" ? "extension" : "espada";
