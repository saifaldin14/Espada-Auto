...
export function listSlackAccountIds(cfg: EspadaConfig): string[] {
  const ids = listConfiguredAccountIds(cfg);
  if (ids.length === 0) return [DEFAULT_ACCOUNT_ID];
  return ids.sort((a, b) => a.localeCompare(b));
}

export function resolveDefaultSlackAccountId(cfg: EspadaConfig): string {
...
function mergeSlackAccountConfig(cfg: EspadaConfig, accountId: string): SlackAccountConfig {
...
export function listEnabledSlackAccounts(cfg: EspadaConfig): ResolvedSlackAccount[] {
