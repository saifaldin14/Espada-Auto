...
export function listIMessageAccountIds(cfg: EspadaConfig): string[] {
  const ids = listConfiguredAccountIds(cfg);
  if (ids.length === 0) return [DEFAULT_ACCOUNT_ID];
  return ids.sort((a, b) => a.localeCompare(b));
}

export function resolveDefaultIMessageAccountId(cfg: EspadaConfig): string {
...
function mergeIMessageAccountConfig(cfg: EspadaConfig, accountId: string): IMessageAccountConfig {
...
export function listEnabledIMessageAccounts(cfg: EspadaConfig): ResolvedIMessageAccount[] {
