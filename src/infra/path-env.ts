...
function candidateBinDirs(opts: EnsureEspadaPathOpts): string[] {
  ...
  // Bundled macOS app: `espada` lives next to the executable (process.execPath).
  try {
    const execDir = path.dirname(execPath);
    const siblingEspada = path.join(execDir, "espada");
    if (isExecutable(siblingEspada)) candidates.push(execDir);
  } catch {
    // ignore
  }

  // Project-local installs (best effort): if a `node_modules/.bin/espada` exists near cwd,
  // include it. This helps when running under launchd or other minimal PATH environments.
  const localBinDir = path.join(cwd, "node_modules", ".bin");
  if (isExecutable(path.join(localBinDir, "espada"))) candidates.push(localBinDir);
...
/**
 * Best-effort PATH bootstrap so skills that require the `espada` CLI can run
 * under launchd/minimal environments (and inside the macOS app bundle).
 */
export function ensureEspadaCliOnPath(opts: EnsureEspadaPathOpts = {}) {
  if (isTruthyEnvValue(process.env.ESPADA_PATH_BOOTSTRAPPED)) return;
  process.env.ESPADA_PATH_BOOTSTRAPPED = "1";
