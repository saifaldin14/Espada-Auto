...
export class DirectoryCache<T> {
  ...
  private lastConfigRef: EspadaConfig | null = null;

  constructor(private readonly ttlMs: number) {}

  get(key: string, cfg: EspadaConfig): T | undefined {
  ...
  set(key: string, value: T, cfg: EspadaConfig): void {
    this.resetIfConfigChanged(cfg);
  ...
  clear(cfg?: EspadaConfig): void {
    this.cache.clear();
    if (cfg) this.lastConfigRef = cfg;
  }

  private resetIfConfigChanged(cfg: EspadaConfig): void {
