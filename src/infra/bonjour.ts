...
function isDisabledByEnv() {
  if (isTruthyEnvValue(process.env.ESPADA_DISABLE_BONJOUR)) return true;
...
function safeServiceName(name: string) {
  ...
  return trimmed.length > 0 ? trimmed : "Espada";
}

function prettifyInstanceName(name: string) {
  const normalized = name.trim().replace(/\s+/g, " ");
  return normalized.replace(/\s+\(Espada\)\s*$/i, "").trim() || normalized;
}
