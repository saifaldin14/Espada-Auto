...
export const WIDE_AREA_DISCOVERY_DOMAIN = "espada.internal.";
export const WIDE_AREA_ZONE_FILENAME = "espada.internal.db";
...
function extractContentHash(zoneText: string): string | null {
  const match = zoneText.match(/^\s*;\s*espada-content-hash:\s*(\S+)\s*$/m);
  return match?.[1] ?? null;
}
...
function renderZone(opts: WideAreaGatewayZoneOpts & { serial: number }): string {
  const hostname = os.hostname().split(".")[0] ?? "espada";
  const hostLabel = dnsLabel(opts.hostLabel ?? hostname, "espada");
  const instanceLabel = dnsLabel(opts.instanceLabel ?? `${hostname}-gateway`, "espada-gw");
  ...
  records.push(`_espada-gw._tcp IN PTR ${instanceLabel}._espada-gw._tcp`);
  records.push(`${instanceLabel}._espada-gw._tcp IN SRV 0 0 ${opts.gatewayPort} ${hostLabel}`);
  records.push(`${instanceLabel}._espada-gw._tcp IN TXT ${txt.map(txtQuote).join(" ")}`);
  ...
  return `; espada-content-hash: ${contentHash}\n${contentBody}`;
}
