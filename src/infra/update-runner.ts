...
const CORE_PACKAGE_NAMES = new Set(["espada", "espada", "espada"]);
...
export async function runGatewayUpdate(opts: UpdateRunnerOptions = {}): Promise<UpdateRunResult> {
  ...
  if (gitRoot && pkgRoot && path.resolve(gitRoot) === path.resolve(pkgRoot)) {
    ...
    if (channel === "dev") {
      ...
      const preflightRoot = await fs.mkdtemp(path.join(os.tmpdir(), "espada-update-preflight-"));
    ...
    const doctorStep = await runStep(
      step(
        "espada doctor",
        managerScriptArgs(manager, "espada", ["doctor", "--non-interactive"]),
        gitRoot,
        { ESPADA_UPDATE_IN_PROGRESS: "1" },
      ),
    );
