...
describe("buildServiceEnvironment", () => {
  it("sets minimal PATH and gateway vars", () => {
    ...
    expect(env.ESPADA_GATEWAY_PORT).toBe("18789");
    expect(env.ESPADA_GATEWAY_TOKEN).toBe("secret");
    expect(env.ESPADA_SERVICE_MARKER).toBe("espada");
    expect(env.ESPADA_SERVICE_KIND).toBe("gateway");
    expect(typeof env.ESPADA_SERVICE_VERSION).toBe("string");
    expect(env.ESPADA_SYSTEMD_UNIT).toBe("espada-gateway.service");
    if (process.platform === "darwin") {
      expect(env.ESPADA_LAUNCHD_LABEL).toBe("bot.molt.gateway");
    }
  });

  it("uses profile-specific unit and label", () => {
    const env = buildServiceEnvironment({
      env: { HOME: "/home/user", ESPADA_PROFILE: "work" },
      port: 18789,
    });
    expect(env.ESPADA_SYSTEMD_UNIT).toBe("espada-gateway-work.service");
    if (process.platform === "darwin") {
      expect(env.ESPADA_LAUNCHD_LABEL).toBe("bot.molt.work");
    }
  });
});
