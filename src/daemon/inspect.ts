...
const EXTRA_MARKERS = ["espada"];
const execFileAsync = promisify(execFile);

export function renderGatewayServiceCleanupHints(
  env: Record<string, string | undefined> = process.env as Record<string, string | undefined>,
): string[] {
  const profile = env.ESPADA_PROFILE;
...
function hasGatewayServiceMarker(content: string): boolean {
  const lower = content.toLowerCase();
  return (
    lower.includes("espada_service_marker") &&
    lower.includes(GATEWAY_SERVICE_MARKER.toLowerCase()) &&
    lower.includes("espada_service_kind") &&
    lower.includes(GATEWAY_SERVICE_KIND.toLowerCase())
  );
}

function isEspadaGatewayLaunchdService(label: string, contents: string): boolean {
  if (hasGatewayServiceMarker(contents)) return true;
  const lowerContents = contents.toLowerCase();
  if (!lowerContents.includes("gateway")) return false;
  return label.startsWith("bot.molt.") || label.startsWith("com.espada.");
}

function isEspadaGatewaySystemdService(name: string, contents: string): boolean {
  if (hasGatewayServiceMarker(contents)) return true;
  if (!name.startsWith("espada-gateway")) return false;
  return contents.toLowerCase().includes("gateway");
}

function isEspadaGatewayTaskName(name: string): boolean {
  const normalized = name.trim().toLowerCase();
  if (!normalized) return false;
  const defaultName = resolveGatewayWindowsTaskName().toLowerCase();
  return normalized === defaultName || normalized.startsWith("espada gateway");
}
...
function isIgnoredLaunchdLabel(label: string): boolean {
  return (
    ...
    resolveLegacyGatewayLaunchAgentLabels(process.env.ESPADA_PROFILE).includes(label)
  );
}
...
async function scanLaunchdDir(params: {
  ...
  for (const entry of entries) {
    ...
    if (isEspadaGatewayLaunchdService(label, contents)) continue;
    results.push({
      platform: "darwin",
      label,
...
async function scanSystemdDir(params: {
  ...
  for (const entry of entries) {
    ...
    if (!containsMarker(contents)) continue;
    if (isEspadaGatewaySystemdService(name, contents)) continue;
    results.push({
      platform: "linux",
      label: entry,
...
export async function findExtraGatewayServices(
  ...
  if (process.platform === "win32") {
    ...
    for (const task of tasks) {
      ...
      if (!name) continue;
      if (isEspadaGatewayTaskName(name)) continue;
