...
async function withLaunchctlStub(
  ...
  const originalLogPath = process.env.ESPADA_TEST_LAUNCHCTL_LOG;
  const originalListOutput = process.env.ESPADA_TEST_LAUNCHCTL_LIST_OUTPUT;

  const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "espada-launchctl-test-"));
  try {
    ...
    await fs.writeFile(
      ...
      [
        ...
        "const logPath = process.env.ESPADA_TEST_LAUNCHCTL_LOG;",
        "if (logPath) {",
        '  fs.appendFileSync(logPath, JSON.stringify(args) + "\\n", "utf8");',
        "}",
        'if (args[0] === "list") {',
        '  const output = process.env.ESPADA_TEST_LAUNCHCTL_LIST_OUTPUT || "";',
    ...
    process.env.ESPADA_TEST_LAUNCHCTL_LOG = logPath;
    process.env.ESPADA_TEST_LAUNCHCTL_LIST_OUTPUT = options.listOutput ?? "";
    process.env.PATH = `${binDir}${path.delimiter}${originalPath ?? ""}`;

    await run({
      env: {
        HOME: homeDir,
        ESPADA_PROFILE: "default",
      },
      logPath,
    });
  } finally {
    ...
    if (originalLogPath === undefined) {
      delete process.env.ESPADA_TEST_LAUNCHCTL_LOG;
    } else {
      process.env.ESPADA_TEST_LAUNCHCTL_LOG = originalLogPath;
    }
    if (originalListOutput === undefined) {
      delete process.env.ESPADA_TEST_LAUNCHCTL_LIST_OUTPUT;
    } else {
      process.env.ESPADA_TEST_LAUNCHCTL_LIST_OUTPUT = originalListOutput;
    }
...
describe("resolveLaunchAgentPlistPath", () => {
  it("uses default label when ESPADA_PROFILE is default", () => {
  ...
  it("uses default label when ESPADA_PROFILE is unset", () => {
  ...
  it("uses profile-specific label when ESPADA_PROFILE is set to a custom value", () => {
    const env = { HOME: "/Users/test", ESPADA_PROFILE: "jbphoenix" };
    expect(resolveLaunchAgentPlistPath(env)).toBe(
      "/Users/test/Library/LaunchAgents/bot.molt.jbphoenix.plist",
    );
  });

  it("prefers ESPADA_LAUNCHD_LABEL over ESPADA_PROFILE", () => {
    const env = {
      HOME: "/Users/test",
      ESPADA_PROFILE: "jbphoenix",
      ESPADA_LAUNCHD_LABEL: "com.custom.label",
    };
    expect(resolveLaunchAgentPlistPath(env)).toBe(
  ...
  it("trims whitespace from ESPADA_LAUNCHD_LABEL", () => {
    const env = {
      HOME: "/Users/test",
      ESPADA_LAUNCHD_LABEL: "  com.custom.label  ",
    };
  ...
  it("ignores empty ESPADA_LAUNCHD_LABEL and falls back to profile", () => {
    const env = {
      HOME: "/Users/test",
      ESPADA_PROFILE: "myprofile",
      ESPADA_LAUNCHD_LABEL: "   ",
    };
    expect(resolveLaunchAgentPlistPath(env)).toBe(
  ...
  it("trims whitespace from ESPADA_PROFILE", () => {
    const env = { HOME: "/Users/test", ESPADA_PROFILE: "  myprofile  " };
    expect(resolveLaunchAgentPlistPath(env)).toBe(
      "/Users/test/Library/LaunchAgents/bot.molt.myprofile.plist",
    );
  });
