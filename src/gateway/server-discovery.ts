...
export function formatBonjourInstanceName(displayName: string) {
  ...
  if (!trimmed) return "Espada";
  if (/espada/i.test(trimmed)) return trimmed;
  return `${trimmed} (Espada)`;
}

export function resolveBonjourCliPath(opts: ResolveBonjourCliPathOptions = {}): string | undefined {
  const env = opts.env ?? process.env;
  const envPath = env.ESPADA_CLI_PATH?.trim();
  ...
  const siblingCli = path.join(execDir, "espada");
  ...
  const binCli = path.join(cwd, "bin", "espada.js");
...
export async function resolveTailnetDnsHint(opts?: {
  ...
  const env = opts?.env ?? process.env;
  const envRaw = env.ESPADA_TAILNET_DNS?.trim();
