...
describe("gateway e2e", () => {
  it(
    ...
    async () => {
      ...
      const tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "espada-gw-mock-home-"));
      ...
      const toolProbePath = path.join(workspaceDir, `.espada-tool-probe.${nonceA}.txt`);
  ...
  it("runs wizard over ws and writes auth token config", { timeout: 90_000 }, async () => {
    ...
    const tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "espada-wizard-home-"));
    process.env.HOME = tempHome;
