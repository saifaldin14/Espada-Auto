...
describe("gateway server models + voicewake", () => {
  const setTempHome = (homeDir: string) => {
    ...
    process.env.ESPADA_STATE_DIR = path.join(homeDir, ".espada");
    ...
    return () => {
      ...
      if (prevStateDir === undefined) {
        ...
        process.env.ESPADA_STATE_DIR = prevStateDir;
      }
  ...
  test(
    ...
    { timeout: 60_000 },
    async () => {
      const homeDir = await fs.mkdtemp(path.join(os.tmpdir(), "espada-home-"));
      const restoreHome = setTempHome(homeDir);

      const initial = await rpcReq<{ triggers: string[] }>(ws, "voicewake.get");
      expect(initial.ok).toBe(true);
      expect(initial.payload?.triggers).toEqual(["espada", "claude", "computer"]);
      ...
      const onDisk = JSON.parse(
        await fs.readFile(path.join(homeDir, ".espada", "settings", "voicewake.json"), "utf8"),
...
describe("gateway server misc", () => {
  test("hello-ok advertises the gateway port for canvas host", async () => {
    ...
    const prevCanvasPort = process.env.ESPADA_CANVAS_HOST_PORT;
    process.env.ESPADA_GATEWAY_TOKEN = "secret";
    testTailnetIPv4.value = "100.64.0.1";
    testState.gatewayBind = "lan";
    const canvasPort = await getFreePort();
    testState.canvasHostPort = canvasPort;
    process.env.ESPADA_CANVAS_HOST_PORT = String(canvasPort);
    ...
    if (prevToken === undefined) {
      ...
      process.env.ESPADA_GATEWAY_TOKEN = prevToken;
    }
    if (prevCanvasPort === undefined) {
      delete process.env.ESPADA_CANVAS_HOST_PORT;
    } else {
      process.env.ESPADA_CANVAS_HOST_PORT = prevCanvasPort;
    }
  });
  ...
  test("auto-enables configured channel plugins on startup", async () => {
    const configPath = process.env.ESPADA_CONFIG_PATH;
    if (!configPath) throw new Error("Missing ESPADA_CONFIG_PATH");
