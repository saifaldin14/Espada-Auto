...
async function setupGatewayTestHome() {
  ...
  previousStateDir = process.env.ESPADA_STATE_DIR;
  previousConfigPath = process.env.ESPADA_CONFIG_PATH;
  previousSkipBrowserControl = process.env.ESPADA_SKIP_BROWSER_CONTROL_SERVER;
  previousSkipGmailWatcher = process.env.ESPADA_SKIP_GMAIL_WATCHER;
  previousSkipCanvasHost = process.env.ESPADA_SKIP_CANVAS_HOST;
  tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "espada-gateway-home-"));
  process.env.HOME = tempHome;
  process.env.USERPROFILE = tempHome;
  process.env.ESPADA_STATE_DIR = path.join(tempHome, ".espada");
  delete process.env.ESPADA_CONFIG_PATH;
}

function applyGatewaySkipEnv() {
  process.env.ESPADA_SKIP_BROWSER_CONTROL_SERVER = "1";
...
async function resetGatewayTestState(options: { uniqueConfigRoot: boolean }) {
  ...
  tempConfigRoot = options.uniqueConfigRoot
    ? await fs.mkdtemp(path.join(tempHome, "espada-test-"))
    : path.join(tempHome, ".espada-test");
...
async function cleanupGatewayTestHome(options: { restoreEnv: boolean }) {
  ...
  if (options.restoreEnv) {
    ...
    if (previousStateDir === undefined) delete process.env.ESPADA_STATE_DIR;
    else process.env.ESPADA_STATE_DIR = previousStateDir;
    if (previousConfigPath === undefined) delete process.env.ESPADA_CONFIG_PATH;
    else process.env.ESPADA_CONFIG_PATH = previousConfigPath;
    if (previousSkipBrowserControl === undefined)
      delete process.env.ESPADA_SKIP_BROWSER_CONTROL_SERVER;
    else process.env.ESPADA_SKIP_BROWSER_CONTROL_SERVER = previousSkipBrowserControl;
    if (previousSkipGmailWatcher === undefined) delete process.env.ESPADA_SKIP_GMAIL_WATCHER;
    else process.env.ESPADA_SKIP_GMAIL_WATCHER = previousSkipGmailWatcher;
    if (previousSkipCanvasHost === undefined) delete process.env.ESPADA_SKIP_CANVAS_HOST;
    else process.env.ESPADA_SKIP_CANVAS_HOST = previousSkipCanvasHost;
  }
...
export async function startServerWithClient(token?: string, opts?: GatewayServerOptions) {
  ...
  const prev = process.env.ESPADA_GATEWAY_TOKEN;
  ...
  if (fallbackToken === undefined) {
    ...
    process.env.ESPADA_GATEWAY_TOKEN = fallbackToken;
  }
