...
beforeAll(async () => {
  ...
  prevGatewayToken = process.env.ESPADA_GATEWAY_TOKEN;
  gatewayPort = await getFreePort();
  process.env.ESPADA_GATEWAY_PORT = String(gatewayPort);
  process.env.ESPADA_GATEWAY_TOKEN = "test-token";
...
afterAll(async () => {
  ...
  if (prevGatewayToken === undefined) {
    ...
    process.env.ESPADA_GATEWAY_TOKEN = prevGatewayToken;
  }
});
...
describe("sessions_send gateway loopback", () => {
  it("returns reply when lifecycle ends before agent.wait", async () => {
    ...
    const tool = createEspadaTools().find((candidate) => candidate.name === "sessions_send");
