...
export async function handleToolsInvokeHttpRequest(
  ...
  const messageChannel = normalizeMessageChannel(getHeader(req, "x-espada-message-channel") ?? "");
  const accountId = getHeader(req, "x-espada-account-id")?.trim() || undefined;
  ...
  const allTools = createEspadaTools({
