...
const SANDBOX_EXAMPLES = {
  main: [
    ["espada sandbox list", "List all sandbox containers."],
    ["espada sandbox list --browser", "List only browser containers."],
    ["espada sandbox recreate --all", "Recreate all containers."],
    ["espada sandbox recreate --session main", "Recreate a specific session."],
    ["espada sandbox recreate --agent mybot", "Recreate agent containers."],
    ["espada sandbox explain", "Explain effective sandbox config."],
  ],
  list: [
    ["espada sandbox list", "List all sandbox containers."],
    ["espada sandbox list --browser", "List only browser containers."],
  ...
  recreate: [
    ["espada sandbox recreate --all", "Recreate all containers."],
    ["espada sandbox recreate --session main", "Recreate a specific session."],
    ["espada sandbox recreate --agent mybot", "Recreate a specific agent (includes sub-agents)."],
    ["espada sandbox recreate --browser --all", "Recreate only browser containers."],
    ["espada sandbox recreate --all --force", "Skip confirmation."],
  ],
  explain: [
    ["espada sandbox explain", "Show effective sandbox config."],
    ["espada sandbox explain --session agent:main:main", "Explain a specific session."],
    ["espada sandbox explain --agent work", "Explain an agent sandbox."],
    ["espada sandbox explain --json", "JSON output."],
  ],
} as const;
...
export function registerSandboxCli(program: Command) {
  const sandbox = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/sandbox", "docs.espada.dev/cli/sandbox")}\n`,
    )
    .action(() => {
