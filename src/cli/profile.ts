...
function resolveProfileStateDir(profile: string, homedir: () => string): string {
  ...
  return path.join(homedir(), `.espada${suffix}`);
}
...
export function applyCliProfileEnv(params: {
  ...
  env.ESPADA_PROFILE = profile;

  const stateDir = env.ESPADA_STATE_DIR?.trim() || resolveProfileStateDir(profile, homedir);
  if (!env.ESPADA_STATE_DIR?.trim()) env.ESPADA_STATE_DIR = stateDir;

  if (!env.ESPADA_CONFIG_PATH?.trim()) {
    env.ESPADA_CONFIG_PATH = path.join(stateDir, "espada.json");
  }

  if (profile === "dev" && !env.ESPADA_GATEWAY_PORT?.trim()) {
    env.ESPADA_GATEWAY_PORT = "19001";
  }
}
