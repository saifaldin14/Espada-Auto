...
function renderNodeServiceStartHints(): string[] {
  const base = [formatCliCommand("espada node install"), formatCliCommand("espada node start")];
...
export async function runNodeDaemonInstall(opts: NodeDaemonInstallOptions) {
  ...
  if (loaded && !opts.force) {
    ...
    if (!json) {
      ...
      defaultRuntime.log(`Reinstall with: ${formatCliCommand("espada node install --force")}`);
    }
    return;
  }
...
export async function runNodeDaemonStatus(opts: NodeDaemonStatusOptions = {}) {
  ...
  const hintEnv = {
    ...baseEnv,
    ESPADA_LOG_PREFIX: baseEnv.ESPADA_LOG_PREFIX ?? "node",
