...
function applySlotSelectionForPlugin(
  config: EspadaConfig,
  pluginId: string,
): { config: EspadaConfig; warnings: string[] } {
...
export function registerPluginsCli(program: Command) {
  const plugins = program
    .command("plugins")
    .description("Manage Espada plugins/extensions")
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/plugins", "docs.espada.dev/cli/plugins")}\n`,
    );
  ...
  plugins
    ...
    .action(() => {
      ...
      const docs = formatDocsLink("/plugin", "docs.espada.dev/plugin");
      lines.push("");
