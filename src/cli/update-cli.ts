...
import { resolveEspadaPackageRoot } from "../infra/package-root.js";
import {
  checkUpdateStatus,
...
const STEP_LABELS: Record<string, string> = {
  ...
  "espada doctor": "Running doctor checks",
...
const DEFAULT_PACKAGE_NAME = "espada";
const CORE_PACKAGE_NAMES = new Set([DEFAULT_PACKAGE_NAME, "espada"]);
const CLI_NAME = resolveCliName();
const ESPADA_REPO_URL = "https://github.com/espada/espada.git";
const DEFAULT_GIT_DIR = path.join(os.homedir(), "espada");

function normalizeTag(value?: string | null): string | null {
  if (!value) return null;
  const trimmed = value.trim();
  if (!trimmed) return null;
  if (trimmed.startsWith("espada@")) return trimmed.slice("espada@".length);
...
function resolveGitInstallDir(): string {
  const override = process.env.ESPADA_GIT_DIR?.trim();
...
async function ensureGitCheckout(params: {
  ...
  if (!dirExists) {
    return await runUpdateStep({
      name: "git clone",
      argv: ["git", "clone", ESPADA_REPO_URL, params.dir],
      timeoutMs: params.timeoutMs,
  ...
  if (!(await isGitCheckout(params.dir))) {
    ...
    if (!empty) {
      throw new Error(
        `ESPADA_GIT_DIR points at a non-git directory: ${params.dir}. Set ESPADA_GIT_DIR to an empty folder or a espada checkout.`,
      );
    }
    return await runUpdateStep({
      name: "git clone",
  ...
  if (!(await isCorePackage(params.dir))) {
    throw new Error(`ESPADA_GIT_DIR does not look like a core checkout: ${params.dir}.`);
  }
...
export async function updateStatusCommand(opts: UpdateStatusOptions): Promise<void> {
  ...
  const root =
    (await resolveEspadaPackageRoot({
  ...
  defaultRuntime.log(theme.heading("Espada update status"));
  defaultRuntime.log("");
  defaultRuntime.log(
    renderTable({
      width: tableWidth,
...
export async function updateCommand(opts: UpdateCommandOptions): Promise<void> {
  ...
  if (!opts.json) {
    defaultRuntime.log(theme.heading("Updating Espada..."));
    defaultRuntime.log("");
  }
  ...
  if (result.status === "skipped") {
    ...
    if (result.reason === "not-git-install") {
      defaultRuntime.log(
        theme.warn(
          `Skipped: this Espada install isn't a git checkout, and the package manager couldn't be detected. Update via your package manager, then run \`${replaceCliName(formatCliCommand("espada doctor"), CLI_NAME)}\` and \`${replaceCliName(formatCliCommand("espada gateway restart"), CLI_NAME)}\`.`,
        ),
      );
      defaultRuntime.log(
        theme.muted(
          `Examples: \`${replaceCliName("npm i -g espada@latest", CLI_NAME)}\` or \`${replaceCliName("pnpm add -g espada@latest", CLI_NAME)}\``,
        ),
      );
    }
    defaultRuntime.exit(0);
  ...
  if (shouldRestart) {
    ...
    try {
      ...
      if (!opts.json && restarted) {
        ...
        defaultRuntime.log("");
        process.env.ESPADA_UPDATE_IN_PROGRESS = "1";
        try {
          ...
          delete process.env.ESPADA_UPDATE_IN_PROGRESS;
        }
      }
    } catch (err) {
      if (!opts.json) {
        ...
        defaultRuntime.log(
          theme.muted(
            `You may need to restart the service manually: ${replaceCliName(formatCliCommand("espada gateway restart"), CLI_NAME)}`,
          ),
        );
      }
    }
    ...
    if (result.mode === "npm" || result.mode === "pnpm") {
      defaultRuntime.log(
        theme.muted(
          `Tip: Run \`${replaceCliName(formatCliCommand("espada doctor"), CLI_NAME)}\`, then \`${replaceCliName(formatCliCommand("espada gateway restart"), CLI_NAME)}\` to apply updates to a running gateway.`,
        ),
      );
    } else {
      defaultRuntime.log(
        theme.muted(
          `Tip: Run \`${replaceCliName(formatCliCommand("espada gateway restart"), CLI_NAME)}\` to apply updates to a running gateway.`,
        ),
      );
    }
  }
...
export async function updateWizardCommand(opts: UpdateWizardOptions = {}): Promise<void> {
  if (!process.stdin.isTTY) {
    defaultRuntime.error(
      "Update wizard requires a TTY. Use `espada update --channel <stable|beta|dev>` instead.",
    );
    defaultRuntime.exit(1);
    return;
  }
  ...
  if (requestedChannel === "dev" && updateStatus.installKind !== "git") {
    ...
    if (!hasGit) {
      ...
      if (dirExists) {
        ...
        if (!empty) {
          defaultRuntime.error(
            `ESPADA_GIT_DIR points at a non-git directory: ${gitDir}. Set ESPADA_GIT_DIR to an empty folder or a espada checkout.`,
          );
          defaultRuntime.exit(1);
          return;
        }
      ...
      const ok = await confirm({
        message: stylePromptMessage(
          `Create a git checkout at ${gitDir}? (override via ESPADA_GIT_DIR)`,
        ),
        initialValue: true,
      });
      if (isCancel(ok) || ok === false) {
...
export function registerUpdateCli(program: Command) {
  const update = program
    .command("update")
    .description("Update Espada to the latest version")
    ...
    .addHelpText("after", () => {
      const examples = [
        ["espada update", "Update a source checkout (git)"],
        ["espada update --channel beta", "Switch to beta channel (git + npm)"],
        ["espada update --channel dev", "Switch to dev channel (git + npm)"],
        ["espada update --tag beta", "One-off update to a dist-tag or version"],
        ["espada update --no-restart", "Update without restarting the service"],
        ["espada update --json", "Output result as JSON"],
        ["espada update --yes", "Non-interactive (accept downgrade prompts)"],
        ["espada update wizard", "Interactive update wizard"],
        ["espada --update", "Shorthand for espada update"],
      ] as const;
...
${theme.heading("Switch channels:")}
  ...
  - Run espada update status to see the active channel and source
...
${theme.muted("Docs:")} ${formatDocsLink("/cli/update", "docs.espada.dev/cli/update")}`;
    })
    .action(async (opts) => {
      try {
        await updateCommand({
  ...
  update
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["espada update status", "Show channel + version status."],
          ["espada update status --json", "JSON output."],
          ["espada update status --timeout 10", "Custom timeout."],
        ])}\n\n${theme.heading("Notes:")}\n${theme.muted(
          "- Shows current update channel (stable/beta/dev) and source",
        )}\n${theme.muted("- Includes git tag/branch/SHA for source checkouts")}\n\n${theme.muted(
          "Docs:",
        )} ${formatDocsLink("/cli/update", "docs.espada.dev/cli/update")}`,
    )
    .action(async (opts) => {
      try {
