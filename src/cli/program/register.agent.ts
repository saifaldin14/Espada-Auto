...
${formatHelpExamples([
  ['espada agent --to +15555550123 --message "status update"', "Start a new session."],
  ['espada agent --agent ops --message "Summarize logs"', "Use a specific agent."],
  [
  ...
  ['espada agent --to +15555550123 --message "Summon reply" --deliver', "Deliver reply."],
  [
...
${theme.muted("Docs:")} ${formatDocsLink("/cli/agent", "docs.espada.dev/cli/agent")}`,
    )
    .action(async (opts) => {
  ...
  const agents = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/agents", "docs.espada.dev/cli/agents")}\n`,
    );
