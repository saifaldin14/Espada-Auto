...
async function main() {
  ...
  const { ensureEspadaCliOnPath } = await import("../infra/path-env.js");
  ...
  process.on("uncaughtException", (error) => {
    console.error("[espada] Uncaught exception:", formatUncaughtError(error));
    process.exit(1);
  });
...
void main().catch((err) => {
  console.error("[espada] Relay failed:", err instanceof Error ? (err.stack ?? err.message) : err);
  process.exit(1);
});
