...
homepage: https://docs.espada.dev/hooks#command-logger
metadata:
  {
    "espada":
      {
...
2. **Appends to log file** - Writes a JSON line to `~/.espada/logs/commands.log`
...
1. **Manual rotation**:
   ...
   ```bash
   mv ~/.espada/logs/commands.log ~/.espada/logs/commands.log.old
   ```

2. **Use logrotate** (Linux):
   Create `/etc/logrotate.d/espada`:
   ```
   /home/username/.espada/logs/commands.log {
       weekly
       rotate 4
       compress
       missingok
