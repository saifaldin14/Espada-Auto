...
This directory contains hooks that ship with Espada. These hooks are automatically discovered and can be enabled/disabled via CLI or configuration.
...
**Docs**: https://docs.espada.dev/hooks/soul-evil
...
metadata:
  { "espada": { "emoji": "🔗", "events": ["command:new"], "requires": { "bins": ["node"] } } }
---
# Hook Title
...
Hooks can be configured in `~/.espada/espada.json`:
...
2. Restart gateway: `pkill -9 -f 'espada.*gateway' && pnpm espada gateway`
3. Enable the hook: `espada hooks enable my-hook`
4. Trigger the event (e.g., send `/new` command)
5. Check gateway logs for hook execution

## Documentation

Full documentation: https://docs.espada.dev/hooks
